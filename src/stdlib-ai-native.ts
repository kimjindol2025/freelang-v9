 ...
 */
export function createAiNativeModule(): Record<string, Function> {
  return {
    // ── AI 모델 직접 호출 ────────────────────────────────────────────
    "ai-call": async (model: string, prompt: any): Promise<string> => {
      ...
      const promptStr =
        typeof prompt === "string"
          ...
          ? String((prompt as any).prompt || JSON.stringify(prompt))
          : String(prompt);
      ...
      if (anthropicKey && (model.startsWith("claude") || model.startsWith("anthropic"))) {
        try {
          const contextStr =
            typeof prompt === "object" && prompt !== null && (prompt as any).context
              ? `\n\nContext:\n${JSON.stringify((prompt as any).context)}`
              : "";
          const maxTokens =
            typeof prompt === "object" && prompt !== null
              ? Number((prompt as any)["max-tokens"] || (prompt as any).maxTokens || 1024)
              : 1024;
    ...
    // ── RAG 검색 ────────────────────────────────────────────────────
    "rag-search": (query: string, opts: any): Array<{ content: string; score: number; source: string }> => {
    ...
    "embed": async (text: string): Promise<number[]> => {
      ...
      return Array.from({ length: 10 }, (_, i) =>
        Math.sin((text.charCodeAt(i % text.length) + i * 7) * 0.1)
      );
    },
    ...
    // ── 텍스트 청킹 (RAG 준비용) ─────────────────────────────────────
    "chunk-text": (text: string, size: number = 512, overlap: number = 50): string[] => {
    ...
    // ── 프롬프트 템플릿 변수 치환 ────────────────────────────────────
    "prompt-template": (template: string, vars: Record<string, any>): string => {
