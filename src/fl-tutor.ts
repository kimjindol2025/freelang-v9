...
export interface FLExample {
  concept: string;      // 개념 이름
  code: string;         // FL 코드 예제
...
export interface LessonResult {
  ...
  examples: FLExample[];
  explanation: string;
  exercise: string;     // 연습 문제 (FL 코드 템플릿)
}

// 내장 FL 예제 라이브러리
export const FL_EXAMPLES: FLExample[] = [
  // 기초
...
export class FLTutor {
  private examples: FLExample[];

  constructor(examples: FLExample[] = FL_EXAMPLES) {
    // 딥 카피로 독립성 보장
  ...
  size(): number { return this.examples.length; }
...
export const globalTutor = new FLTutor();
