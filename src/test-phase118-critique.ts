...
import { CritiqueAgent, CritiquePoint, CritiqueResult, CritiqueConfig, defaultFinders, globalCritique, severityWeight } from "./critique";
...
test("3. run() critical → approved=false", () => {
  agent = new CritiqueAgent();
  const criticalFinder = (output: any): CritiquePoint[] => [
...
test("4. points 배열 반환", () => {
  agent = new CritiqueAgent();
  const finder = (output: any): CritiquePoint[] => [
...
test("5. overallRisk 계산 (0~1 범위)", () => {
  ...
  const result = agent.run("출력", { finders: [finder] });
  assert(result.overallRisk >= 0 && result.overallRisk <= 1,
    `overallRisk=${result.overallRisk}`);
  // major weight = 0.7
  assert(Math.abs(result.overallRisk - 0.7) < 0.01,
    `expected ~0.7, got ${result.overallRisk}`);
});
...
test("6. riskThreshold 기본값 0.5", () => {
  ...
  const minorFinder = (output: any): CritiquePoint[] => [
...
test("9. defaultFinders 빈 출력 감지 (null)", () => {
  agent = new CritiqueAgent();
  const result = agent.run(null, { finders: defaultFinders });
...
test("10. defaultFinders 짧은 출력 감지 (3글자)", () => {
  agent = new CritiqueAgent();
  const result = agent.run("abc", { finders: defaultFinders });
...
test("11. defaultFinders 에러 객체 감지", () => {
  agent = new CritiqueAgent();
  const result = agent.run(new Error("테스트 오류"), { finders: defaultFinders });
...
test("12. severity 가중치 순서 확인", () => {
  assert(severityWeight('critical') > severityWeight('major'));
  assert(severityWeight('major') > severityWeight('minor'));
  assert(severityWeight('minor') > severityWeight('suggestion'));
  assert(severityWeight('critical') === 1.0);
  assert(severityWeight('suggestion') === 0.1);
});

// 13. finder 예외 무시
test("13. finder 예외 무시 (에러 발생 시 빈 배열 반환)", () => {
  agent = new CritiqueAgent();
  const errorFinder = (output: any): CritiquePoint[] => {
...
test("14. 여러 finder 합산", () => {
  agent = new CritiqueAgent();
  const f1 = (output: any): CritiquePoint[] => [
    { type: 'weakness', description: '문제 A', severity: 'minor' }
  ];
  const f2 = (output: any): CritiquePoint[] => [
...
test("16. globalCritique 싱글톤 사용", () => {
  const result = globalCritique.run("정상 출력 텍스트", { finders: defaultFinders });
...
test("22. Error 출력 → major 포함", () => {
  agent = new CritiqueAgent();
  const result = agent.run(new Error("오류"), { finders: defaultFinders });
...
test("23. 정상 긴 출력 → approved=true", () => {
  agent = new CritiqueAgent();
  const result = agent.run("이것은 충분히 길고 정상적인 출력 문자열입니다", { finders: defaultFinders });
  assert(result.approved === true, `approved=${result.approved}`);
  assert(result.points.length === 0);
});
...
test("25. suggestion 포함 포인트 확인", () => {
  agent = new CritiqueAgent();
  // 짧은 출력 → suggestion 포함
  const result = agent.run("ab", { finders: defaultFinders });
...
test("26. undefined 출력 → critical", () => {
  agent = new CritiqueAgent();
  const result = agent.run(undefined, { finders: defaultFinders });
  const hasCritical = result.points.some(p => p.severity === 'critical');
...
test("27. _tag=Err 객체 → major 감지", () => {
  ...
  const result = agent.run(errVal, { finders: defaultFinders });
  const hasMajor = result.points.some(p => p.severity === 'major');
