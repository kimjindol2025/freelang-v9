...
import { ASTNode, Block, Literal, Variable, SExpr } from "./ast";
...
// 헬퍼: 리터럴 노드 생성
function lit(type: "number" | "string" | "boolean" | "null", value: any): Literal {
  return { kind: "literal", type, value };
}

function numLit(v: number): Literal {
  return lit("number", v);
}

function strLit(v: string): Literal {
  return lit("string", v);
}

function boolLit(v: boolean): Literal {
...
// 헬퍼: FUNC 블록 생성
function funcBlock(name: string, params: string[], body: ASTNode): Block {
...
test("JSCodegen 인스턴스 생성 가능", () => {
  ...
  assert(cg instanceof JSCodegen, "JSCodegen 생성 실패");
});
...
test("null 리터럴 → 'null'", () => {
  const cg = new JSCodegen();
  const node: Literal = { kind: "literal", type: "null", value: null };
