...
export interface DocEntry {
  ...
  params: string[];
  doc: string;        // ;;; 주석 (함수 바로 위에 붙은 것)
  examples: string[]; // ;;; @example 태그 뒤 코드
  source: string;     // 원본 소스 스니펫
}
...
export function extractDocs(src: string): DocEntry[] {
...
function collectCommentAbove(lines: string[], lineIdx: number): string[] {
  const result: string[] = [];
  let j = lineIdx - 1;
  // 빈 줄 하나는 허용, 그 이상은 끊기
  while (j >= 0) {
