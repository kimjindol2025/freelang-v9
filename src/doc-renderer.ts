...
import { DocEntry } from "./doc-extractor";

// ─────────────────────────────────────────
// 전체 Markdown 렌더링
// ─────────────────────────────────────────

export function renderMarkdown(entries: DocEntry[], title?: string): string {
  const lines: string[] = [];
...
export function renderSummary(entries: DocEntry[]): string {
  if (entries.length === 0) {
    return "(항목 없음)";
  }
  return entries
...
function kindBadge(kind: DocEntry["kind"]): string {
  switch (kind) {
...
function kindLabel(kind: DocEntry["kind"]): string {
  switch (kind) {
