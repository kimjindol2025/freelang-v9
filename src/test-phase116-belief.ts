...
import { BeliefSystem, globalBeliefs } from "./belief";
...
test("18. globalBeliefs 싱글톤 — 동일 인스턴스", () => {
  return globalBeliefs === globalBeliefs;
});
...
test("20. 반복 update로 수렴 — 긍정 증거 반복 → 높은 확신", () => {
  const bs = new BeliefSystem();
  bs.set("converge", 0.5);
  for (let i = 0; i < 10; i++) bs.update("converge", 0.9);
...
test("21. 반복 negate → 낮은 확신", () => {
  const bs = new BeliefSystem();
  bs.set("fade", 0.9);
  for (let i = 0; i < 8; i++) bs.negate("fade");
...
test("27. belief-certain 내장함수", () => {
  globalBeliefs.set("fl-certain-high", 0.95);
  const result = run(`(belief-certain 0.9)`);
  return Array.isArray(result) && result.some((b: any) => b.claim === "fl-certain-high");
});

test("28. belief-strongest 내장함수 → 가장 강한 claim 문자열", () => {
  globalBeliefs.set("fl-strongest-candidate", 0.999);
  const result = run(`(belief-strongest)`);
  return typeof result === "string" && result.length > 0;
});

test("29. belief-forget 내장함수 → 삭제 후 null", () => {
  globalBeliefs.set("fl-forget-me", 0.5);
  run(`(belief-forget "fl-forget-me")`);
  return globalBeliefs.get("fl-forget-me") === null;
});
