...
import { FLSDK, FLCodeBuilder, sdk } from "./fl-sdk";
...
test("31. sdk 싱글톤 내보내기", () => {
  assert(sdk instanceof FLSDK, "sdk should be FLSDK instance");
  assertEquals(sdk.version, '9.0.0');
});

test("32. sdk.block() FLCodeBlock 생성", () => {
  const b = sdk.block('expression', '(+ 1 2)', 'addition');
...
test("33. sdk.block() description 없이 생성", () => {
  const b = sdk.block('program', '(define x 1)');
