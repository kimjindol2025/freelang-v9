...
class FlChecker {
  ...
  private funcDefs: Map<string, number> = new Map(); // name → param count
  private funcUsed: Set<string> = new Set();

  check(src: string): CheckError[] {
    this.errors = [];
    ...
    let ast: any[];
    try {
      ast = convertTStoFL(parse(lex(src)));
    } catch (e: any) {
    ...
    for (const node of ast) {
      if (node.kind === "block" && node.type === "FUNC") {
        const params = node.fields instanceof Map
          ...
          : (node.fields as any).params;
        const paramCount = params?.items?.length ?? 0;
        // variadic check: last param is "&" rest
        const isVariadic = params?.items?.some((p: any) =>
          p.kind === "literal" && p.value === "&"
        );
    ...
    // Pass 2: 정적 검사
    for (const node of ast) {
      this.checkNode(node, new Set<string>());
    }
  ...
  private checkNode(node: any, scope: Set<string>): void {
    ...
    if (node.kind === "block") {
      if (node.type === "FUNC") {
        ...
        const body = node.fields instanceof Map
          ...
          : (node.fields as any).body;

        const funcScope = new Set(scope);
        if (params?.items) {
          for (const p of params.items) {
            if (p.kind === "variable") funcScope.add(p.name);
            else if (p.kind === "literal" && p.value !== "&") funcScope.add(p.value);
          }
        }

        if (Array.isArray(body)) {
          for (const expr of body) this.checkNode(expr, funcScope);
        } else {
    ...
    if (node.kind === "sexpr") {
        ...
        if (bindings?.kind === "array") {
          for (const binding of bindings.items || []) {
            if (binding.kind === "array" && binding.items?.length >= 2) {
              ...
              if (nameNode.kind === "variable") letScope.add(nameNode.name);
              else if (nameNode.kind === "array") {
                // destructuring: add each element
                for (const item of nameNode.items || []) {
                  if (item.kind === "variable") letScope.add(item.name);
                }
              }
            }
          }
        }
        for (const expr of letBody) this.checkNode(expr, letScope);
        return;
      }
      ...
      if (op === "define" || op === "set!") {
        ...
        if (args[1]) this.checkNode(args[1], scope);
        return;
      }
      ...
      for (const arg of args) this.checkNode(arg, scope);
      return;
    }
    ...
    for (const val of Object.values(node)) {
      if (Array.isArray(val)) {
        for (const item of val) this.checkNode(item, scope);
...
(export add)`;
  ...
  if (errs.length > 0) throw new Error(`unexpected errors: ${errs.map(e => e.message).join(", ")}`);
});

test("구문 오류 감지", () => {
  const src = `[FUNC add :params [$a $b :body ((+ $a $b))]`;
  const errs = checker.check(src);
  if (errs.length === 0) throw new Error("should detect parse error");
...
(export test)`;
  ...
  if (!arityErr) throw new Error(`no arity error found. Errors: ${errs.map(e=>e.message).join(",")}`);
...
(export test)`;
  const errs = checker.check(src).filter(e => e.type === "error");
  const arityErr = errs.find(e => e.message.includes("Arity"));
  if (!arityErr) throw new Error("no arity error found");
  console.log(`    → 감지: ${arityErr.message}`);
});
...
(export test)`;
  const errs = checker.check(src).filter(e => e.type === "error");
  if (errs.length > 0) throw new Error(`unexpected: ${errs.map(e=>e.message).join(",")}`);
});
...
(export bad)`;
  ...
  if (!varErr) throw new Error("no undefined variable error");
...
(export used)`;
  ...
  if (!unusedWarn) throw new Error(`no unused warning. Warnings: ${warns.map(e=>e.message).join(",")}`);
...
(export f)`;
  ...
  if (warns.length > 0) throw new Error(`unexpected warning for exported: ${warns[0].message}`);
});
...
(export main)`;
  ...
  if (warns.length > 0) throw new Error(`helper falsely marked unused`);
});
