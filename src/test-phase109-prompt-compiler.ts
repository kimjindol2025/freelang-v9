...
function assertIncludes(str: string, sub: string, msg?: string) {
  if (!str.includes(sub)) throw new Error(msg ?? `Expected "${sub}" in "${str.slice(0, 100)}"`);
}
...
test("1. PromptCompiler 기본 생성 (claude)", () => {
  const compiler = new PromptCompiler('claude');
...
test("11. compile() target='gpt' → [System]/[User] 포함", () => {
  const compiler = new PromptCompiler('gpt');
  const sections: PromptSection[] = [
...
test("19. globalCompiler 싱글톤 — 동일 인스턴스", () => {
  ...
  assert(globalCompiler instanceof PromptCompiler);
  // 두 번 임포트해도 같은 참조
  const { globalCompiler: gc2 } = require("./prompt-compiler");
  assert(globalCompiler === gc2);
});
...
test("20. prompt-compile 내장함수", () => {
  ...
  globalCompiler.setTarget('claude');
...
test("24. 빈 sections 처리 — target generic", () => {
  const compiler = new PromptCompiler('generic');
...
test("28. compile() CompileResult 구조 검증", () => {
  ...
  const result: CompileResult = compiler.compile(sections, 'instruction');
