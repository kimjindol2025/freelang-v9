...
import { evalEthicsCheck } from "./eval-builtins";
...
// 더미 인터프리터
const dummyInterp = {
  callFunctionValue: (_fn: any, _args: any[]) => null,
};
...
test("EthicsChecker 생성", () => {
  const checker = new EthicsChecker();
  assert(checker !== null, "EthicsChecker 인스턴스 생성 실패");
  assert(checker instanceof EthicsChecker, "EthicsChecker 타입 확인");
});
...
test("기본 원칙 5개 자동 등록", () => {
  ...
  // 5개 프레임워크가 모두 있어야 함
  const frameworks = ["utilitarian", "deontological", "virtue", "care", "fairness"] as EthicsFramework[];
...
test("addPrinciple — 커스텀 원칙 추가", () => {
  const checker = new EthicsChecker();
  const customPrinciple: EthicsPrinciple = {
    id: "custom-test",
    name: "테스트 원칙",
    description: "커스텀 테스트 원칙",
    framework: "virtue",
    check: (subject: string, _ctx: Record<string, unknown>) => ({
...
test("ethics-check 빌트인 — 안전한 내용", () => {
  const result = evalEthicsCheck(dummyInterp as any, "ethics-check", ["도움이 되는 내용"]);
...
test("ethics-check-framework 빌트인", () => {
  const result = evalEthicsCheck(dummyInterp as any, "ethics-check-framework", ["안전한 내용", "utilitarian"]);
  assert(result instanceof Map, "결과가 Map이 아님");
...
test("ethics-is-ethical 빌트인", () => {
  const trueResult = evalEthicsCheck(dummyInterp as any, "ethics-is-ethical", ["안전한 내용"]);
  assertEquals(trueResult, true, "안전한 내용이 is-ethical=false");

  const falseResult = evalEthicsCheck(dummyInterp as any, "ethics-is-ethical", ["murder"]);
  assertEquals(falseResult, false, "해로운 내용이 is-ethical=true");
});

// 23. ethics-suggest 빌트인
test("ethics-suggest 빌트인", () => {
  const violation = new Map<string, any>([
    ["principle", "해악 금지"],
    ["severity", "high"],
    ["description", "해악 발견"],
    ["suggestion", "해악 제거"],
  ...
  const result = evalEthicsCheck(dummyInterp as any, "ethics-suggest", ["해로운 내용", [violation]]);
...
test("ethics-risk 빌트인", () => {
  // 안전한 결과: none
  const safeResultMap = new Map<string, any>([
    ["subject", "안전한 내용"],
    ["passed", true],
    ["violations", []],
    ["score", 1.0],
  ...
  const noneLevel = evalEthicsCheck(dummyInterp as any, "ethics-risk", [safeResultMap]);
  ...
  const critResultMap = new Map<string, any>([
    ["subject", "위험한 내용"],
    ["passed", false],
  ...
  const critLevel = evalEthicsCheck(dummyInterp as any, "ethics-risk", [critResultMap]);
...
test("ethics-score 빌트인", () => {
  ...
  const score = evalEthicsCheck(dummyInterp as any, "ethics-score", [resultMap]);
...
test("ethics-violations 빌트인", () => {
  ...
  const violations = evalEthicsCheck(dummyInterp as any, "ethics-violations", [resultMap]);
...
test("ethics-add-principle 빌트인", () => {
  const result = evalEthicsCheck(dummyInterp as any, "ethics-add-principle", [
    ":id", "test-principle",
    ":name", "테스트 원칙",
    ":framework", "virtue",
    ":description", "테스트용 원칙",
...
test("globalEthics 전역 인스턴스", () => {
  assert(globalEthics instanceof EthicsChecker, "globalEthics가 EthicsChecker 인스턴스 아님");
  const result = globalEthics.check("전역 인스턴스 테스트");
