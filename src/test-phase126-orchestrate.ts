...
test("2. register() 에이전트 등록", () => {
  const orch = new Orchestrator();
  orch.register({ id: "agent-a", run: (x: any) => x });
  assert(orch.list().includes("agent-a"), "agent-a가 목록에 없음");
});

test("3. list() 목록 반환", () => {
  const orch = new Orchestrator();
  orch.register({ id: "x", run: (v: any) => v });
  orch.register({ id: "y", run: (v: any) => v });
  const lst = orch.list();
...
test("4. run() 단일 태스크", () => {
  const orch = new Orchestrator();
  orch.register({ id: "t1", run: (x: any) => x * 2 });
...
test("5. run() 여러 태스크", () => {
  const orch = new Orchestrator();
  orch.register({ id: "a", run: (x: any) => x + 1 });
  orch.register({ id: "b", run: (x: any) => x + 2 });
...
test("6. outputs taskId 키 구조", () => {
  const orch = new Orchestrator();
  orch.register({ id: "task1", run: (x: any) => "result-" + x });
...
test("7. order 배열 반환", () => {
  const orch = new Orchestrator();
  orch.register({ id: "m", run: (x: any) => x });
...
test("8. duration 기록", () => {
  const orch = new Orchestrator();
  orch.register({ id: "d", run: (x: any) => x });
...
test("9. success=true 정상 실행", () => {
  const orch = new Orchestrator();
  orch.register({ id: "s", run: (x: any) => x });
...
test("10. 의존성 없는 태스크 — 등록 순서대로 실행", () => {
  const orch = new Orchestrator();
  const exec: string[] = [];
  orch.register({ id: "p", run: (x: any) => { exec.push("p"); return x; } });
  orch.register({ id: "q", run: (x: any) => { exec.push("q"); return x; } });
...
test("11. dependsOn → 선행 후 실행", () => {
  const orch = new Orchestrator();
  const exec: string[] = [];
  orch.register({ id: "first", run: (x: any) => { exec.push("first"); return x + 10; } });
  orch.register({ id: "second", run: (x: any) => { exec.push("second"); return x; } });
  orch.run([
    { id: "first", input: 5 },
...
test("12. depOutputs input에 전달", () => {
  const orch = new Orchestrator();
  orch.register({ id: "src", run: (_: any) => 99 });
  orch.register({ id: "dst", run: (x: any) => x.deps[0] + 1 });
  const result = orch.run([
    { id: "src", input: null },
...
test("13. 위상 정렬 순서 검증 (A→B→C)", () => {
  const orch = new Orchestrator();
  const exec: string[] = [];
  orch.register({ id: "A", run: (x: any) => { exec.push("A"); return x; } });
  orch.register({ id: "B", run: (x: any) => { exec.push("B"); return x; } });
  orch.register({ id: "C", run: (x: any) => { exec.push("C"); return x; } });
  orch.run([
    { id: "A", input: 1 },
...
test("14. 에러 → success=false", () => {
  const orch = new Orchestrator();
  orch.register({ id: "err", run: (_: any) => { throw new Error("의도적 에러"); } });
  const result = orch.run([{ id: "err", input: null }]);
  assertEq(result.success, false, "에러 시 success=false 예상");
});

test("15. globalOrchestrator 싱글톤", () => {
  assertDefined(globalOrchestrator, "globalOrchestrator가 undefined");
  assert(globalOrchestrator instanceof Orchestrator, "globalOrchestrator가 Orchestrator 인스턴스가 아님");
});

// ─── 4. 내장 함수 ────────────────────────────────────────────────────────

test("16. orchestrate-register 내장함수", () => {
  const freshOrch = new Orchestrator();
  // createOrchestrateBuiltins가 globalOrchestrator에 등록하므로
  // 별도로 로컬 등록 검증
  freshOrch.register({ id: "builtin-test", run: (x: any) => x });
  assert(freshOrch.list().includes("builtin-test"), "builtin-test 등록 실패");
});

test("17. orchestrate 내장함수 (tasks → outputs)", () => {
  const interp = new Interpreter();
  const builtins = createOrchestrateBuiltins(interp as any);
  // 미리 globalOrchestrator에 등록
  globalOrchestrator.register({ id: "fn-a", run: (x: any) => (x ?? 0) + 100 });
...
test("19. orchestrate-list 내장함수", () => {
  const interp = new Interpreter();
  const builtins = createOrchestrateBuiltins(interp as any);
  globalOrchestrator.register({ id: "list-test-agent", run: (x: any) => x });
...
test("23. 순환 의존성 — 방문 체크로 무한루프 방지", () => {
  const orch = new Orchestrator();
  orch.register({ id: "X", run: (x: any) => x });
  orch.register({ id: "Y", run: (x: any) => x });
...
test("24. 선행 output이 다음 input의 deps[0]에 포함", () => {
  const orch = new Orchestrator();
  orch.register({ id: "producer", run: (_: any) => { return { value: 777 }; } });
  orch.register({
    id: "consumer",
    run: (x: any) => {
...
test("25. 통합: 3단계 파이프라인 (파싱→변환→포맷)", () => {
  ...
  orch.register({
    id: "parse",
    run: (input: any) => {
      return (input.raw as string).split(",").map(Number);
    }
  });
  ...
  orch.register({
    id: "transform",
    run: (input: any) => {
      const nums: number[] = input.deps[0];
      return nums.map((n: number) => n * n);
    }
  });
  ...
  orch.register({
    id: "format",
    run: (input: any) => {
      const squared: number[] = input.deps[0];
      const total = squared.reduce((a: number, b: number) => a + b, 0);
      return `total=${total}`;
    }
  });
