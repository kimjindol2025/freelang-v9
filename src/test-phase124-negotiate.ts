...
import { Negotiator, NegotiationPosition, globalNegotiator } from "./negotiate";
...
function assertClose(a: number, b: number, tol = 0.05, msg?: string) {
...
test("11. flexibility 높으면 빠른 합의", () => {
  const n = new Negotiator();
  const highFlex: NegotiationPosition[] = [
  ...
  const lowFlex: NegotiationPosition[] = [
...
test("18. globalNegotiator 싱글톤", () => {
  assertDefined(globalNegotiator, "globalNegotiator가 없음");
  assert(globalNegotiator instanceof Negotiator, "globalNegotiator는 Negotiator 인스턴스여야 함");
});
