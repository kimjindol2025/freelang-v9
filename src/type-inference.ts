...
import { ASTNode, TypeAnnotation, Literal, Variable, SExpr } from "./ast";

export { TypeAnnotation };
export interface InferenceContext {
  variableTypes: Map<string, TypeAnnotation>;
  functionTypes: Map<string, { params: TypeAnnotation[]; returnType: TypeAnnotation }>;
  constraints: Array<{ type: string; left: string; right: TypeAnnotation }>;
  typeVariables: Map<string, TypeAnnotation>; // For generics: T -> int
}
...
export class TypeInferenceEngine {
  ...
  inferType(node: any): TypeAnnotation {
    ...
    switch (kind) {
      ...
      case "if": {
        ...
        const elseType = ifNode.alternate ? this.inferType(ifNode.alternate) : ({ kind: "type", name: "any" } as TypeAnnotation);
   ...
   */
  solveConstraints(constraints: Array<{ variable: string; operator: string }>): Map<string, TypeAnnotation> {
    const solution = new Map<string, TypeAnnotation>();
   ...
   */
  private unifyTypes(type1: TypeAnnotation, type2: TypeAnnotation): TypeAnnotation {
    // Same type
    ...
    // One is 'any'
    if (type1.name === "any") return type2;
    if (type2.name === "any") return type1;

    // One is 'unknown'
    if (type1.name === "unknown") return type2;
    if (type2.name === "unknown") return type1;
