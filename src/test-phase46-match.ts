...
(label 99)`;
  const result = flInterpret(src);
  if (result !== "other") throw new Error(`got ${result}`);
});
...
(greet "ko")`;
  const result = flInterpret(src);
  if (result !== "안녕") throw new Error(`got ${result}`);
});
...
(greet "ja")`;
  const result = flInterpret(src);
  if (result !== "unknown") throw new Error(`got ${result}`);
});
...
(f 5)`;
  const result = flInterpret(src);
  if (result !== null) throw new Error(`got ${result}`);
});
...
(sign -3)`;
  const result = flInterpret(src);
  if (result !== "neg") throw new Error(`got ${result}`);
});
...
(+ (bonus 1) (bonus 2))`;
  const result = flInterpret(src);
  if (result !== 150) throw new Error(`got ${result}`);
});
...
(classify 4)`;
  const result = flInterpret(src);
  if (result !== "even") throw new Error(`got ${result}`);
});
...
(fizzbuzz 15)`;
  const result = flInterpret(src);
  if (result !== "FizzBuzz") throw new Error(`got ${result}`);
});
...
(fizzbuzz 3)`;
  const result = flInterpret(src);
  if (result !== "Fizz") throw new Error(`got ${result}`);
});
...
[FUNC f :params [$n]
  ...
  if (interpreted !== "two") throw new Error(`interpret: ${interpreted}`);
  if (compiled !== "two") throw new Error(`compile: ${compiled}`);
});
...
[FUNC f :params [$s]
  ...
  const interpreted = flInterpret(body + `\n(f "b")`);
  const compiled = makeModule(standaloneCompile1(body + `\n(export f)`)).f("b");
  if (interpreted !== 2) throw new Error(`interpret: ${interpreted}`);
  if (compiled !== 2) throw new Error(`compile: ${compiled}`);
});
...
(fact 6)`;
  const result = flInterpret(src);
  if (result !== 720) throw new Error(`got ${result}`);
});
