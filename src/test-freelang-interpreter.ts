...
function testInterpreter(code: string, description: string, validator: (result: any) => boolean) {
  try {
...
if (testInterpreter(
  "(+ 1 2)",
  "TEST 1: Literal number evaluation",
...
if (testInterpreter(
  "(concat \"hello\" \"world\")",
  "TEST 2: String literal evaluation",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].op === "concat"
)) passed++;
...
if (testInterpreter(
  "(+ $x 5)",
  "TEST 3: Variable reference",
  (ast) => ast.length > 0 && ast[0].args[0]?.kind === "variable"
)) passed++;
...
if (testInterpreter(
  ...
  "TEST 6: cond expression",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].op === "cond"
)) step2++;
...
if (testInterpreter(
  "(let [[$x 5]] (+ $x 1))",
  "TEST 7: let expression",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].op === "let"
)) step2++;
...
if (testInterpreter(
  "(define my-var 42)",
  "TEST 8: define expression",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].op === "define"
)) step2++;
...
if (testInterpreter(
  "(fn [$x] (+ $x 1))",
  "TEST 9: Lambda function definition",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].op === "fn"
)) step3++;
...
if (testInterpreter(
  "(call (fn [$x] (+ $x 1)) 5)",
  "TEST 10: Function call",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].op === "call"
)) step3++;
...
step3total++;
if (testInterpreter(
  "(pipe 5 (fn [$x] (+ $x 1)) (fn [$x] (* $x 2)))",
  "TEST 12: Function composition (pipe)",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].op === "pipe"
)) step3++;
...
if (testInterpreter(
  ...
  "TEST 13: Literal pattern matching",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].op === "match"
)) step4++;
...
if (testInterpreter(
  "(match 42 ($x (+ $x 1)))",
  "TEST 14: Variable pattern matching",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr"
)) step4++;
...
step5total++;
if (testInterpreter(
  "[MODULE math :exports [add multiply] (define add (fn [$a $b] (+ $a $b)))]",
  "TEST 19: Module definition",
  (ast) => ast.length > 0 && ast[0].kind === "block" && ast[0].type === "MODULE"
)) step5++;

// TEST 20: Import
step5total++;
if (testInterpreter(
  "(import math :only [add])",
  "TEST 20: Import expression",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].op === "import"
)) step5++;
...
combinedtotal++;
if (testInterpreter(
  "(let [[$x 5]] (match $x (5 (* $x 10)) (_ 0)))",
  "TEST 21: let + match combination",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr"
)) combined++;
...
combinedtotal++;
if (testInterpreter(
  "[FUNC process :params [[$items int]] :body (map $items (fn [$x] (* $x 2)))]",
  "TEST 22: Function definition with map",
  (ast) => ast.length > 0 && ast[0].kind === "block"
)) combined++;
