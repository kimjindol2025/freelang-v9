...
import { createFileModule } from "./stdlib-file";
import { createShellModule } from "./stdlib-shell";
import * as fs from "fs";

const W = createWorkflowModule();
...
const H = createHttpModule();
const F = createFileModule();
const S = createShellModule();
...
test("workflow_create", () => {
  ...
  if (!wf.id || !wf.name) throw new Error("Missing id/name");
  if (!X.is_uuid(wf.id)) throw new Error("Invalid UUID");
});
...
test("workflow_run all steps succeed", () => {
  ...
  if (W.workflow_get(result, "value") !== 20) throw new Error(`Expected 20, got ${W.workflow_get(result, "value")}`);
  if (result.steps_ok !== 3) throw new Error(`Expected 3 ok steps`);
});
...
test("workflow_run required step failure aborts", () => {
  ...
  if (result.steps_run !== 2) throw new Error(`Expected 2 steps run, got ${result.steps_run}`);
  if (W.workflow_get(result, "b") !== null) throw new Error("Step after failure should not run");
});
...
test("workflow_summary readable", () => {
  ...
  if (!summary.includes("success".toUpperCase())) throw new Error("Status missing from summary");
  if (!summary.includes("Steps:")) throw new Error("Steps missing");
});
...
test("task lifecycle", () => {
  ...
  if (W.task_progress(task) !== 0) throw new Error("Expected 0 progress");
  task = W.task_complete_subtask(task, "fetch", { count: 10 });
  if (Math.abs(W.task_progress(task) - 1/3) > 0.01) throw new Error("Expected 1/3 progress");
  ...
  if (W.task_progress(task) !== 1) throw new Error("Expected 100% progress");
  if (task.status !== "done") throw new Error("Expected done");
});
...
test("report create and render", () => {
  ...
  if (!rendered.includes("FreeLang v9 Status")) throw new Error("Title missing");
  if (!rendered.includes("Overview")) throw new Error("Section missing");
  if (!rendered.includes("total_phases")) throw new Error("Metrics missing");
});
...
test("Scenario 1: kimdb data analysis pipeline", () => {
  const wf = W.workflow_create("kimdb-analysis", [
    ...
    W.workflow_step("analyze", (ctx) => {
      const projects: any[] = ctx.projects;
    ...
    W.workflow_step("save_report", (ctx) => {
      ...
      report = W.report_add(report, "Top 3 Projects", ctx.top3.map((p: any) => `${p.name} (score: ${p.score})`));
  ...
  if (result.status !== "success") throw new Error(`Expected success: ${result.errors.join(", ")}`);
  if (!F.file_exists(W.workflow_get(result, "report_path"))) throw new Error("Report file not created");

  const top3 = W.workflow_get(result, "top3");
  if (!Array.isArray(top3) || top3.length !== 3) throw new Error("Expected 3 top projects");
...
test("Scenario 2: agent-driven multi-step reasoning with observability", () => {
  ...
  agent = A.agent_add_tool(agent, "score_source", (src: any) => ({
    ...src,
  ...
  const result = A.agent_loop(
    ...
    (a) => {
      ...
      if (step === "fetch") {
        ...
        const sources: any[] = A.agent_get(a, "sources");
        stepResult = sources.map((s: any) => A.agent_call_tool(a, "score_source", s));
        ...
        const ranked: any[] = A.agent_get(a, "ranked");
        const best = ranked[0];
        const summary = D.str_template(
          "Best source: {name} (type={type}, score={score}, latency={latency_ms}ms)",
          best
        );
  ...
  if (result.status !== "done") throw new Error(`Agent failed: ${A.agent_get(result, "_error")}`);
  if (result.steps !== 4) throw new Error(`Expected 4 steps, got ${result.steps}`);

  const decision = A.agent_get(result, "decision");
  if (!decision?.best) throw new Error("No decision made");
  if (T.log_count(logger, "info") < 3) throw new Error("Expected at least 3 info logs");
...
test("Scenario 3: CSV file pipeline with integrity verification", () => {
  ...
  const wf = W.workflow_create("csv-integrity-pipeline", [
    ...
    W.workflow_step("verify", (ctx) => {
      ...
      if (!X.hash_eq(ctx.integrity_hash, verifyHash)) throw new Error("Integrity check failed!");
      return { ...ctx, verified: true };
    }),
  ]);

  const result = W.workflow_run(wf, {});
  if (result.status !== "success") throw new Error(`Workflow failed: ${result.errors.join(", ")}`);
  if (!W.workflow_get(result, "verified")) throw new Error("Not verified");

  const ranked: any[] = W.workflow_get(result, "ranked");
...
test("Scenario 4: resilient HTTP fetch with full observability", () => {
  ...
  endpoints.forEach((_: string, i: number) => {
  ...
  if (T.metrics_count(metrics, "requests") !== endpoints.length) throw new Error("Wrong request count");
  if (W.task_progress(task) !== 1) throw new Error("Task not complete");
