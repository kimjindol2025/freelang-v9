...
import { FileWatcher, WatchOptions, createDebounce, runWithWatch } from "./hot-reload";
...
test("TC-6: debounce 함수 생성 (createDebounce 내부 유틸)", () => {
  const debounced = createDebounce(300);
...
test("TC-7: debounce — 연속 호출 시 마지막만 실행 (fake timer)", () => {
  ...
  const debounced = createDebounce(50);
  let count = 0;

  // 즉시 실행하는 fn (debounce 0ms 버전으로 테스트)
  const immediateDebounce = createDebounce(0);
  let last = "";
...
test("TC-9: debounce clearTimeout 정리 (이중 호출 안전)", () => {
  const debounced = createDebounce(100);
...
test("TC-11: onError 콜백 형태 검증", () => {
  ...
  assert((errorObj as Error).message === "test error", "에러 메시지 확인");
});
...
test("TC-15: runWithWatch 함수 export 확인", () => {
  assert(typeof runWithWatch === "function", "runWithWatch 는 함수여야 함");
  // 시그니처 확인: (file: string, opts?: WatchOptions) => void
  // opts는 optional이지만 JS에서 length는 선언된 파라미터 수 반환
  assert(runWithWatch.length >= 1, "최소 1개 파라미터 (file) 존재");
});
