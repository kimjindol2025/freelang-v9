...
import { CausalGraph, CausalNode, CausalEdge, CausalChain, CausalExplanation, globalCausal, whyCaused } from "./causal";
...
function evalFL(interp: Interpreter, code: string): any {
  ...
  let result: any;
  for (const node of ast) result = (interp as any).eval(node);
  return result;
}
...
test("14. confidence 전파", () => {
  ...
  assert(Math.abs(chains[0].confidence - expectedConf) < 0.01,
    `confidence가 ${expectedConf}여야 함, got ${chains[0].confidence}`);
});
...
test("22. causal-simulate 빌트인", () => {
  ...
  const inputMap = new Map<string, any>([["rain2", 1.0]]);
  const result = evalBuiltin((interp as any), "causal-simulate", [inputMap], {});
  assert(result instanceof Map, "결과가 Map이어야 함");
...
test("27. totalStrength 계산 (체인 곱)", () => {
  ...
  assert(Math.abs(chains[0].totalStrength - expected) < 0.01,
    `totalStrength가 ${expected}여야 함, got ${chains[0].totalStrength}`);
});

test("28. whyCaused 전역 함수", () => {
  // globalCausal에 이미 rain→wet-road→accident 추가됨
  const chain = whyCaused("accident", "rain");
