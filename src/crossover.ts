...
export interface CrossoverConfig {
  type: CrossoverType;
  rate: number;       // 교배율 (기본 0.7)
  blendAlpha?: number; // blend 교배 시 알파값 (기본 0.5)
}
...
export interface CrossoverResult<T> {
  ...
  child1: T;
  child2: T;
  crossoverPoint?: number;   // single-point 시
  crossoverPoints?: [number, number]; // two-point 시
  type: CrossoverType;
}

export class Crossover<T> {
  ...
  crossoverObjects(
    ...
    const child1: Record<string, unknown> = {};
    const child2: Record<string, unknown> = {};
  ...
  cross(a: T, b: T): CrossoverResult<T> {
    if (Array.isArray(a) && Array.isArray(b)) {
      const isNumeric = a.every((v: any) => typeof v === 'number');
      if (isNumeric) {
        return this.arithmetic(a as number[], b as number[]) as unknown as CrossoverResult<T>;
      }
      const result = this.singlePoint(a, b);
      return result as unknown as CrossoverResult<T>;
    }
    if (typeof a === 'string' && typeof b === 'string') {
      return this.crossoverStrings(a, b) as unknown as CrossoverResult<T>;
    }
    if (typeof a === 'object' && a !== null && typeof b === 'object' && b !== null) {
      return this.crossoverObjects(
        a as Record<string, unknown>,
        b as Record<string, unknown>
      ) as unknown as CrossoverResult<T>;
    }
    ...
    if (typeof a === 'number' && typeof b === 'number') {
      ...
      return {
        parent1: a,
        parent2: b,
        child1: (alpha * (a as unknown as number) + (1 - alpha) * (b as unknown as number)) as unknown as T,
        child2: ((1 - alpha) * (a as unknown as number) + alpha * (b as unknown as number)) as unknown as T,
        type: 'arithmetic',
      };
    }
    // fallback
...
export const globalCrossover: Crossover<unknown> = new Crossover();

export function crossoverNumbers(a: number[], b: number[]): CrossoverResult<number[]> {
  return globalCrossover.arithmetic(a, b);
}

export function crossoverStrings(a: string, b: string): CrossoverResult<string> {
  return globalCrossover.crossoverStrings(a, b);
}
