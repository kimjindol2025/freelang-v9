...
import { CognitiveArchitecture, CognitiveState, globalCognition } from "./cognitive";
...
test("2. meta 접근", () => {
  const cog = new CognitiveArchitecture();
  assert(cog.meta instanceof MetaReasoner, "meta는 MetaReasoner 인스턴스여야 함");
});

test("3. beliefs 접근", () => {
  const cog = new CognitiveArchitecture();
  assert(cog.beliefs instanceof BeliefSystem, "beliefs는 BeliefSystem 인스턴스여야 함");
});

test("4. analogies 접근", () => {
  const cog = new CognitiveArchitecture();
  assert(cog.analogies instanceof AnalogyStore, "analogies는 AnalogyStore 인스턴스여야 함");
});

test("5. hypothesis 접근", () => {
  const cog = new CognitiveArchitecture();
  assert(cog.hypothesis instanceof HypothesisTester, "hypothesis는 HypothesisTester 인스턴스여야 함");
});

test("6. critique 접근", () => {
  const cog = new CognitiveArchitecture();
  assert(cog.critique instanceof CritiqueAgent, "critique는 CritiqueAgent 인스턴스여야 함");
});

test("7. composer 접근", () => {
  const cog = new CognitiveArchitecture();
  assert(cog.composer instanceof ReasonComposer, "composer는 ReasonComposer 인스턴스여야 함");
});

test("8. debater 접근", () => {
  const cog = new CognitiveArchitecture();
  assert(cog.debater instanceof Debater, "debater는 Debater 인스턴스여야 함");
});

test("9. checkpoints 접근", () => {
  const cog = new CognitiveArchitecture();
  assert(cog.checkpoints instanceof CheckpointManager, "checkpoints는 CheckpointManager 인스턴스여야 함");
});
...
test("22. globalCognition 싱글톤", () => {
  assert(globalCognition instanceof CognitiveArchitecture, "globalCognition은 CognitiveArchitecture여야 함");
});
...
test("25. cognition-meta 내장함수", () => {
  ...
  const result = (ctx as any).lastValue;
...
test("35. globalCognition.meta는 MetaReasoner", () => {
  assert(globalCognition.meta instanceof MetaReasoner, "globalCognition.meta 타입 확인");
});
