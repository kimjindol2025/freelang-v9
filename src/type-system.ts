 ...
 * 런타임 값에서 FlType 추론
 */
export function inferType(value: any): FlType {
 ...
 */
export function isCompatible(actual: FlType, expected: FlType): boolean {
 ...
 */
export function toFlType(typeName: string): FlType {
  switch (typeName) {
    case "int":    return "int";
...
export class RuntimeTypeChecker {
  ...
  private funcTypes: Map<string, FuncTypeSignature> = new Map();
  ...
  checkCall(name: string, argValues: any[]): void {
    ...
    for (let i = 0; i < checkCount; i++) {
      ...
      if (expected === "any") continue; // any는 항상 통과
