...
const monadTC = (interp as any).getTypeClass("Monad");
const functorTC = (interp as any).getTypeClass("Functor");
...
const resultMonad = (interp as any).getTypeClassInstance("Monad", "Result");
const optionMonad = (interp as any).getTypeClassInstance("Monad", "Option");
const listMonad = (interp as any).getTypeClassInstance("Monad", "List");

const resultFunctor = (interp as any).getTypeClassInstance("Functor", "Result");
const optionFunctor = (interp as any).getTypeClassInstance("Functor", "Option");
const listFunctor = (interp as any).getTypeClassInstance("Functor", "List");
...
for (const { type, constraint, expected } of constraints) {
  const satisfies = (interp as any).satisfiesConstraint(type, constraint);
...
const resultInst = (interp as any).getTypeClassInstance("Monad", "Result");
...
const optionInst = (interp as any).getTypeClassInstance("Monad", "Option");
...
const listInst = (interp as any).getTypeClassInstance("Monad", "List");
...
const resultFunc = (interp as any).getTypeClassInstance("Functor", "Result");
if (resultFunc && resultFunc.implementations.has("fmap")) {
  ...
  console.log(
    `✅ Result fmap(×2) on ok(5) = ${JSON.stringify(mappedResult)}`
  );
}

// Test Option functor fmap
const optionFunc = (interp as any).getTypeClassInstance("Functor", "Option");
if (optionFunc && optionFunc.implementations.has("fmap")) {
  ...
  console.log(
    `✅ Option fmap(×2) on some(3) = ${JSON.stringify(mappedOption)}`
  );
}

// Test List functor fmap
const listFunc = (interp as any).getTypeClassInstance("Functor", "List");
if (listFunc && listFunc.implementations.has("fmap")) {
  ...
  console.log(
    `✅ List fmap(×2) on [1 2 3] = ${JSON.stringify(mappedList)}\n`
  );
}
