...
test("1. ReasoningTrace 생성", () => {
  const trace = new ReasoningTrace("test-root");
  assert(trace !== null);
  assert(trace.getRoot() !== null);
});

test("2. add() 노드 추가", () => {
  const trace = new ReasoningTrace("root");
...
test("7. toMarkdown() 출력", () => {
  const trace = new ReasoningTrace("분석 시작");
...
test("10. 노드 타입 6종 (thought/action/observation/decision/error/result)", () => {
  const trace = new ReasoningTrace("root");
  const types: TraceNodeType[] = ["thought", "action", "observation", "decision", "error", "result"];
...
test("15. root 노드 확인", () => {
  const trace = new ReasoningTrace("my-root");
  const root = trace.getRoot();
...
test("16. createTrace() id 반환", () => {
  const { id, trace } = createTrace("외부 생성");
  assert(typeof id === "string", "id는 문자열");
  assert(id.startsWith("trace-"), "id 형식");
  assert(trace instanceof ReasoningTrace, "trace 인스턴스");
});

test("17. getTrace() 조회", () => {
  const { id } = createTrace("조회 테스트");
  const found = getTrace(id);
  assert(found !== null, "found가 null이 아님");
  assert(found instanceof ReasoningTrace, "ReasoningTrace 인스턴스");
});

test("18. getTrace() 없는 id → null", () => {
  const result = getTrace("trace-nonexistent-99999");
...
test("27. toTree() 중첩 구조", () => {
  const trace = new ReasoningTrace("루트");
...
test("30. deleteTrace / listTraces 기능", () => {
  const { id } = createTrace("삭제 테스트");
  assert(getTrace(id) !== null, "생성 확인");
  const deleted = deleteTrace(id);
  assert(deleted === true, "삭제 성공");
  assert(getTrace(id) === null, "삭제 후 null");
});
