...
import { ReasonComposer, PipelineBuilder, globalComposer, ReasonContext, ReasonStep, ComposeResult } from "./compose-reason";
...
test("1. ReasonComposer 생성", () => {
  const composer = new ReasonComposer();
  assert(composer instanceof ReasonComposer, "ReasonComposer 인스턴스여야 함");
});
...
test("3. compose() 여러 step (파이프라인)", () => {
  ...
  const steps: ReasonStep[] = [
    ...
    { name: "mul3", fn: (x: number) => x * 3 },
    { name: "sub2", fn: (x: number) => x - 2 },
  ];
...
test("4. 이전 step 출력이 다음 step input", () => {
  ...
  const received: any[] = [];
  const steps: ReasonStep[] = [
    { name: "s1", fn: (x: any) => { received.push(x); return x + "A"; } },
    { name: "s2", fn: (x: any) => { received.push(x); return x + "B"; } },
  ];
...
test("5. history 기록", () => {
  const composer = new ReasonComposer();
  const steps: ReasonStep[] = [
    { name: "step-a", fn: (x: number) => x + 10 },
    { name: "step-b", fn: (x: number) => x * 2 },
  ];
  const result = composer.compose(steps, 5);
...
test("7. condition으로 step 건너뜀", () => {
  const composer = new ReasonComposer();
  const steps: ReasonStep[] = [
    { name: "always", fn: (x: number) => x + 1 },
    { name: "skip-if-big", fn: (x: number) => x * 100, condition: (x: number) => x < 3 },
    { name: "add5", fn: (x: number) => x + 5 },
  ];
  ...
  const steps: ReasonStep[] = [
    {
      ...
      onError: (e: Error, input: any) => `fallback:${input}`
    },
  ];
...
test("13. PipelineBuilder 생성", () => {
  ...
  assert(builder instanceof PipelineBuilder, "should be PipelineBuilder");
});

test("14. pipeline().step().step().run()", () => {
  const composer = new ReasonComposer();
  const result = composer.pipeline()
    .step("double", (x: number) => x * 2)
    .step("add10", (x: number) => x + 10)
    .run(5);
  // 5*2+10 = 20
...
test("17. context.step 증가", () => {
  ...
  const steps: ReasonStep[] = [
    { name: "s0", fn: (x: any, ctx: ReasonContext) => { stepNums.push(ctx.step); return x; } },
    { name: "s1", fn: (x: any, ctx: ReasonContext) => { stepNums.push(ctx.step); return x; } },
    { name: "s2", fn: (x: any, ctx: ReasonContext) => { stepNums.push(ctx.step); return x; } },
  ];
...
test("19. globalComposer 싱글톤", () => {
  assert(globalComposer instanceof ReasonComposer, "globalComposer should be ReasonComposer");
  const r = globalComposer.compose([{ name: "id", fn: (x: any) => x }], 99);
...
test("20. compose-reason 내장함수", () => {
  ...
  const steps: ReasonStep[] = [
    { name: "inc", fn: (x: number) => x + 1 },
    { name: "dbl", fn: (x: number) => x * 2 },
  ];
  const result = globalComposer.compose(steps, 3);
  // (3+1)*2 = 8
...
test("21. compose-history 내장함수", () => {
  const steps: ReasonStep[] = [
    { name: "step-alpha", fn: (x: any) => x },
    { name: "step-beta", fn: (x: any) => x },
  ];
  const result = globalComposer.compose(steps, "v");
...
test("22. compose-steps 내장함수", () => {
  ...
  const result = globalComposer.compose(steps, 0);
...
test("24. step 이름이 history에 기록", () => {
  const composer = new ReasonComposer();
  const steps: ReasonStep[] = [
    { name: "COT-step", fn: (x: any) => x },
    { name: "REFLECT-step", fn: (x: any) => x },
    { name: "CRITIQUE-step", fn: (x: any) => x },
  ];
  const result = composer.compose(steps, "input");
  const names = result.history.map(h => h.name);
...
test("25. 통합: 3단계 파이프라인 (META→REASON→REFLECT)", () => {
  ...
  const steps: ReasonStep[] = [
    {
      name: "META-REASON",
      fn: (input: any, ctx: ReasonContext) => {
    ...
    },
    {
      ...
      fn: (input: any, ctx: ReasonContext) => {
        ...
        if (strategy === "arithmetic") {
          reasonOutput = `computed:${(input as number) * 2 + 1}`;
        } else {
  ...
  assert((result.output as any).score === 0.9, "final output should include score");
...
test("26. history.input / history.output 올바른 값", () => {
  const composer = new ReasonComposer();
  const steps: ReasonStep[] = [
    { name: "transform", fn: (x: number) => x + 100 },
  ];
  const result = composer.compose(steps, 42);
...
test("27. onError 후 다음 step 계속 실행", () => {
  ...
  const steps: ReasonStep[] = [
    ...
    { name: "after-error", fn: (x: any) => `${x}-done` },
  ];
...
test("28. PipelineBuilder condition 옵션", () => {
  const composer = new ReasonComposer();
  const result = composer.pipeline()
    .step("always-run", (x: number) => x + 1)
    .step("skip-this", (x: number) => x * 999, { condition: () => false })
    .step("final", (x: number) => x + 5)
    .run(10);
  ...
  const steps: ReasonStep[] = [
    {
      ...
      onError: (_e: Error, input: any) => input + 1
    },
    {
      name: "fail2",
      fn: () => { throw new Error("e2"); },
      onError: (_e: Error, input: any) => input * 2
    },
  ];
  const result = composer.compose(steps, 5);
...
test("30. 객체 타입 데이터 파이프라인", () => {
  const composer = new ReasonComposer();
  const steps: ReasonStep[] = [
    { name: "add-field", fn: (obj: any) => ({ ...obj, score: 100 }) },
    { name: "double-score", fn: (obj: any) => ({ ...obj, score: obj.score * 2 }) },
    { name: "add-label", fn: (obj: any) => ({ ...obj, label: `score-${obj.score}` }) },
  ];
  const result = composer.compose(steps, { name: "test" });
  assert((result.output as any).name === "test", "name should be preserved");
  assert((result.output as any).score === 200, `score should be 200, got ${(result.output as any).score}`);
  assert((result.output as any).label === "score-200", `label should be score-200`);
});
