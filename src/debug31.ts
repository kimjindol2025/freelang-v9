...
let count = 0;
while ((m = re.exec(lexer2JS)) !== null && count < 3) {
  console.log(`\n--- 'by' at ${m.index} ---`);
  console.log(JSON.stringify(lexer2JS.substring(m.index - 60, m.index + 60)));
  count++;
}
if (count === 0) console.log("No 'by' found");

// Also try to eval and catch error location
try {
  new Function(lexer2JS);
} catch(e: any) {
  console.log("Syntax error:", e.message);
  // show lines around error
  const lines = lexer2JS.split("\n");
  const lineMatch = e.message.match(/line (\d+)/);
  if (lineMatch) {
    const ln = parseInt(lineMatch[1]);
    console.log("Lines", ln-2, "to", ln+2, ":");
