...
function compileFL(src: string): string {
...
test("assoc: 맵에 키 추가", () => {
  ...
  if (typeof r !== "object" || r === null || r.b !== 2) throw new Error(`got ${JSON.stringify(r)}`);
});

test("get: 맵에서 키 조회", () => {
  const r = run(`(get {:type "NUMBER" :value "42"} "type")`);
  if (r !== "NUMBER") throw new Error(`got ${r}`);
});

test("dissoc: 맵에서 키 제거", () => {
  const r = run(`(dissoc {:a 1 :b 2} "a")`);
  if (typeof r !== "object" || r === null || "a" in r) throw new Error(`got ${JSON.stringify(r)}`);
  if (r.b !== 2) throw new Error(`b missing in ${JSON.stringify(r)}`);
});
...
test("lex FL 파일 로드 성공", () => {
  ...
  const interp = new Interpreter();
  interp.interpret(parse(lex(lexerSrc)));
  if (!interp.context.functions.has("lex")) throw new Error("lex function not registered");
});

function runWithCompiler(code: string): any {
  const interp = new Interpreter();
  // 컴파일러 구성 로드
...
test("lex: 숫자 토큰 타입", () => {
  ...
  `);
  if (r !== "NUMBER") throw new Error(`expected NUMBER, got ${r}`);
});
...
test("lex: 변수 토큰 타입", () => {
  ...
  `);
  if (r !== "VARIABLE") throw new Error(`expected VARIABLE, got ${r}`);
});
...
test("lex: KEYWORD 토큰", () => {
  ...
  `);
  if (r !== "KEYWORD") throw new Error(`expected KEYWORD, got ${r}`);
});
...
test("lex: BOOL true", () => {
  ...
  `);
  if (r !== "BOOL") throw new Error(`expected BOOL, got ${r}`);
});
...
test("lex: EOF 토큰 포함", () => {
  ...
  `);
  if (r !== "EOF") throw new Error(`expected EOF, got ${r}`);
});
...
test("parse: 숫자 리터럴 AST", () => {
  ...
  `);
  if (r !== "literal") throw new Error(`expected literal, got ${r}`);
});
...
test("parse: FUNC 블록 kind", () => {
  ...
  `);
  if (r !== "block") throw new Error(`expected block, got ${r}`);
});
...
test("parse: FUNC 블록 타입", () => {
  ...
  `);
  if (r !== "FUNC") throw new Error(`expected FUNC, got ${r}`);
});
...
test("parse: S-expr kind", () => {
  ...
  `);
  if (r !== "sexpr") throw new Error(`expected sexpr, got ${r}`);
});
...
test("parse: 맵 리터럴", () => {
  ...
  `);
  if (r !== "map-literal") throw new Error(`expected map-literal, got ${r}`);
});
...
test("codegen: 숫자 리터럴 → JS", () => {
  ...
  if (!r.includes("42")) throw new Error(`JS does not contain 42: ${r}`);
});
...
test("codegen: FUNC 블록 → JS function", () => {
  ...
  `);
  if (typeof r !== "string") throw new Error(`expected string, got ${typeof r}`);
  if (!r.includes("function add")) throw new Error(`JS missing function add: ${r.substring(0, 200)}`);
  if (!r.includes("a + b") && !r.includes("a+b")) throw new Error(`JS missing a+b: ${r.substring(0, 200)}`);
});
...
(export add)
  ...
  // 생성된 JS 실행
  const sandbox: Record<string, any> = { module: { exports: {} }, exports: {} };
  vm.createContext(sandbox);
  vm.runInContext(jsCode, sandbox);
  const addFn = sandbox.module.exports.add || sandbox.add;
  if (typeof addFn !== "function") throw new Error(`add not exported: ${jsCode.substring(0, 300)}`);
...
(export max2)
  ...
  if (typeof max2 !== "function") throw new Error(`max2 not exported: ${jsCode.substring(0, 300)}`);
  if (max2(3, 7) !== 7) throw new Error(`expected 7, got ${max2(3, 7)}`);
  if (max2(9, 5) !== 9) throw new Error(`expected 9, got ${max2(9, 5)}`);
});
...
(export greet)
  ...
  if (typeof greet !== "function") throw new Error(`greet not exported: ${jsCode.substring(0, 300)}`);
  const r = greet("World");
...
(export fact)
  ...
  if (typeof fact !== "function") throw new Error(`fact not exported: ${jsCode.substring(0, 300)}`);
  if (fact(5) !== 120) throw new Error(`expected 120, got ${fact(5)}`);
});
...
(export join3)
  ...
  if (typeof join3 !== "function") throw new Error(`join3 not exported`);
  if (join3("a", "b", "c") !== "abc") throw new Error(`expected abc, got ${join3("a","b","c")}`);
});
