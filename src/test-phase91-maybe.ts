...
test("TC-1: (maybe 0.9 \"hello\") → confidence=0.9, value=\"hello\"", () => {
  ...
  if (!res || res._tag !== 'Maybe') throw new Error(`_tag이 Maybe가 아님: ${JSON.stringify(res)}`);
  if (res.confidence !== 0.9) throw new Error(`confidence=${res.confidence}, 기대=0.9`);
  if (res.value !== 'hello') throw new Error(`value=${res.value}, 기대=hello`);
});

test("TC-2: (uncertain-none) → _tag=\"None\"", () => {
  const res = run(`(uncertain-none)`);
  if (!res || res._tag !== 'None') throw new Error(`_tag이 None이 아님: ${JSON.stringify(res)}`);
});

test("TC-3: (maybe 0.5 42 \"이유\") → reason=\"이유\"", () => {
  const res = run(`(maybe 0.5 42 "이유")`);
  if (!res || res._tag !== 'Maybe') throw new Error(`_tag이 Maybe가 아님`);
  if (res.value !== 42) throw new Error(`value=${res.value}, 기대=42`);
  if (res.reason !== '이유') throw new Error(`reason=${res.reason}, 기대=이유`);
});

test("TC-4: (is-maybe? (maybe 0.8 1)) → true", () => {
  const res = run(`(is-maybe? (maybe 0.8 1))`);
  if (res !== true) throw new Error(`got ${res}, 기대=true`);
});

test("TC-5: (is-maybe? (uncertain-none)) → false", () => {
  const res = run(`(is-maybe? (uncertain-none))`);
  if (res !== false) throw new Error(`got ${res}, 기대=false`);
});
...
test("TC-7: confidence 범위 초과 (1.5) → 에러 발생", () => {
  ...
  if (!threw) throw new Error("에러가 발생해야 하는데 발생하지 않음");
});

test("TC-8: (maybe 1.0 \"확실\") → confidence=1.0", () => {
  const res = run(`(maybe 1.0 "확실")`);
  if (!res || res._tag !== 'Maybe') throw new Error(`_tag이 Maybe가 아님`);
  if (res.confidence !== 1.0) throw new Error(`confidence=${res.confidence}, 기대=1.0`);
  if (res.value !== '확실') throw new Error(`value=${res.value}, 기대=확실`);
});
...
test("TC-12: (most-likely (list ...)) → 가장 높은 confidence 선택 (\"b\")", () => {
  ...
  if (!res || res._tag !== 'Maybe') throw new Error(`_tag이 Maybe가 아님: ${JSON.stringify(res)}`);
  if (res.value !== 'b') throw new Error(`value=${res.value}, 기대=b`);
  if (res.confidence !== 0.8) throw new Error(`confidence=${res.confidence}, 기대=0.8`);
});

test("TC-13: (most-likely (list)) → None (빈 배열)", () => {
  const res = run(`(most-likely (list))`);
  if (!res || res._tag !== 'None') throw new Error(`빈 배열에서 None이 나와야 함: ${JSON.stringify(res)}`);
});

test("TC-14: (when-confident (maybe 0.9 10) 0.7 fn) → 20", () => {
  const res = run(`(when-confident (maybe 0.9 10) 0.7 (fn [$x] (* $x 2)))`);
  if (res !== 20) throw new Error(`got ${res}, 기대=20`);
});
...
test("TC-16: combine(0.8, 0.9, fn +) → maybe(0.72, 5) (확률 곱)", () => {
  ...
  if (!res || res._tag !== 'Maybe') throw new Error(`_tag이 Maybe가 아님: ${JSON.stringify(res)}`);
  if (res.value !== 5) throw new Error(`value=${res.value}, 기대=5`);
  // 부동소수점 허용: 0.72 ±0.001
  if (Math.abs(res.confidence - 0.72) > 0.001) throw new Error(`confidence=${res.confidence}, 기대=0.72`);
});

test("TC-17: combine with uncertain-none → None", () => {
  const res = run(`(combine (maybe 0.8 2) (uncertain-none) +)`);
  if (!res || res._tag !== 'None') throw new Error(`None이 나와야 함: ${JSON.stringify(res)}`);
});

test("TC-18: confidence 출력 확인 (maybe-confidence)", () => {
  const res = run(`(maybe-confidence (maybe 0.75 "test"))`);
  if (res !== 0.75) throw new Error(`got ${res}, 기대=0.75`);
});

test("TC-19: reason 필드 보존 (maybe-reason)", () => {
  const res = run(`(maybe-reason (maybe 0.6 "Seoul" "검색 기반"))`);
  if (res !== '검색 기반') throw new Error(`got ${res}, 기대=검색 기반`);
});
...
test("TC-22: 여러 후보 중 best 선택", () => {
  const res = run(`
    ...
    (maybe-value $best)
  `);
  if (res !== '서울') throw new Error(`got ${res}, 기대=서울`);
});
...
test("TC-23: threshold로 신뢰 구간 필터링", () => {
  ...
  `);
  if (res !== null) throw new Error(`got ${res}, 기대=null (threshold 미달)`);
});
...
test("TC-24: uncertain-none 처리 fallback 패턴", () => {
  ...
  `);
  if (res !== 'fallback') throw new Error(`got ${res}, 기대=fallback`);
});
...
test("TC-25: combine 체인 — 확률 누적 감소 확인", () => {
  ...
  `);
  if (typeof res !== 'number') throw new Error(`got ${res}, 숫자여야 함`);
  // 0.9 * 0.8 = 0.72 ±0.001
  if (Math.abs(res - 0.72) > 0.001) throw new Error(`got ${res}, 기대=0.72`);
});
...
test("TC-26: maybe-value로 값 추출", () => {
  ...
  if (res !== 'extracted') throw new Error(`got ${res}`);
});
...
test("TC-30: maybe confidence=0.0 → confident threshold=0 OK", () => {
  ...
  if (res !== 'zero') throw new Error(`got ${res}, 기대=zero`);
});
...
test("Phase 56 regression 14/14 PASS", () => {
  const result = execSync(
    ...
    { encoding: "utf-8", cwd: "/home/kimjin/kim/Desktop/kim/01_Active_Projects/freelang-v9" }
  );
