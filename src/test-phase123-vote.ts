...
import { VotingSystem, globalVoting, Ballot, VoteResult, VotingMethod } from "./vote";
...
test("11. 단일 후보", () => {
  const vs = new VotingSystem();
  const ballots: Ballot[] = [{ voterId: "v1", choices: ["X"] }];
...
test("12. 단일 투표자", () => {
  const vs = new VotingSystem();
  const ballots: Ballot[] = [{ voterId: "solo", choices: ["B"] }];
...
test("14. globalVoting 싱글톤", () => {
  ...
  assert(globalVoting instanceof VotingSystem, "globalVoting은 VotingSystem 인스턴스여야 함");
});
...
test("25. 통합: 5명 투표 → winner", () => {
  ...
  const ballots: Ballot[] = agents.map((a, i) => ({
    voterId: a,
    choices: [picks[i]],
  }));
