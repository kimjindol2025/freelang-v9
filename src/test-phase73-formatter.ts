...
import { formatFL, FLFormatter } from "./formatter";
...
test("(define x 42) 정상 포맷", () => {
  const out = formatFL("(define x 42)");
  assert(out.trim() === "(define x 42)", `예상: "(define x 42)", 실제: "${out.trim()}"`);
});

test("(+ 1 2) 포맷", () => {
  const out = formatFL("(+ 1 2)");
  assert(out.trim() === "(+ 1 2)", `실제: "${out.trim()}"`);
});

test("문자열 리터럴 보존", () => {
  const out = formatFL('(println "Hello, World!")');
...
test("변수 $x는 SExpr 안에서 포맷", () => {
  ...
  const out = formatFL("(define x 1) (+ $x 0)");
...
for (const sample of idempotentSamples) {
  test(`멱등성: ${sample.slice(0, 40)}`, () => {
    const once = formatFL(sample);
    const twice = formatFL(once);
    assert(
      once === twice,
...
test("fn: 긴 body는 여러 줄", () => {
  ...
  const out = formatFL(src);
  // 멱등성 확인
  assert(formatFL(out) === out, "fn 멱등성 실패");
});
...
test("긴 FUNC 블록 — 여러 줄 분리", () => {
  ...
  // 멱등성
  assert(formatFL(out) === out, "긴 FUNC 멱등성 실패");
});
...
test("이미 포맷된 코드 → 변경 없음", () => {
  ...
  const reFormatted = formatFL(formatted);
...
test("미포맷 코드 → 변경 감지", () => {
  ...
  const formatted = formatFL(uglySrc);
...
function runFL(src: string): any {
  const interp = new Interpreter();
  interp.interpret(parse(lex(src)));
  return (interp as any).context.lastValue;
}

const phase56Cases: Array<{ name: string; src: string; expected: any }> = [
...
for (const tc of phase56Cases) {
  test(`regression: ${tc.name}`, () => {
    ...
    // 포맷 후 실행
    const formatted = formatFL(tc.src);
    ...
    if (Array.isArray(tc.expected)) {
      ...
      assert(
        origVal === tc.expected,
        `원본 결과 불일치: ${JSON.stringify(origVal)} !== ${JSON.stringify(tc.expected)}`
      );
      assert(
        fmtResult === tc.expected,
        `포맷 후 결과 불일치: ${JSON.stringify(fmtResult)} !== ${JSON.stringify(tc.expected)}`
      );
    }

    // 멱등성 확인
    const twice = formatFL(formatted);
