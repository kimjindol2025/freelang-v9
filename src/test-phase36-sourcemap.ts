...
test("FUNC 앞에 @fl 주석", () => {
  ...
  if (!js.includes("// @fl:")) throw new Error("no @fl comment");
  const match = js.match(/\/\/ @fl:(\d+)/);
  if (!match) throw new Error("no @fl:N comment");
...
test("멀티라인: 각 함수의 FL 소스 라인 추적", () => {
  ...
  if (comments.length < 3) throw new Error(`only ${comments.length} @fl comments`);
  // add: line 1, mul: line 2, export: line 3
  if (comments[0] !== 1) throw new Error(`add line=${comments[0]}, want 1`);
  if (comments[1] !== 2) throw new Error(`mul line=${comments[1]}, want 2`);
  if (comments[2] !== 3) throw new Error(`export line=${comments[2]}, want 3`);
...
test("들여쓰기 소스의 라인 추적", () => {
  ...
  const js = standaloneCompile1(src);
  const comments = [...js.matchAll(/\/\/ @fl:(\d+)/g)].map(m => parseInt(m[1]));
  if (comments[0] !== 3) throw new Error(`greet line=${comments[0]}, want 3`);
  if (comments[1] !== 6) throw new Error(`export line=${comments[1]}, want 6`);
  console.log(`    → @fl lines: ${comments.join(', ')}`);
});
...
test("loop/recur에서 @fl 주석", () => {
  ...
  const js = standaloneCompile1(src);
  const m = makeModule(js);
  if (m["sum-n"](100) !== 5050) throw new Error(`got ${m["sum-n"](100)}`);
  const comments = [...js.matchAll(/\/\/ @fl:(\d+)/g)].map(m => parseInt(m[1]));
  console.log(`    → @fl lines: ${comments.join(', ')}`);
});
...
test("Gen1 lexer.fl 컴파일 → @fl 주석 포함", () => {
  ...
  if (comments.length < 5) throw new Error(`only ${comments.length} @fl comments`);
...
test("Gen1 codegen.fl 컴파일 → @fl 주석 포함", () => {
  ...
  const comments = [...js.matchAll(/\/\/ @fl:(\d+)/g)];
  if (comments.length < 20) throw new Error(`only ${comments.length} @fl comments`);
  console.log(`    → ${comments.length}개 @fl 주석`);
});
