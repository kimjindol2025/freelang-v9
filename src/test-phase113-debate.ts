...
import { Debater, globalDebater, Argument, DebateResult } from "./debate";
...
// 공통 찬성/반대 에이전트 헬퍼
function makeProAgent(strength: number) {
  return (round: number, conArgs: Argument[]): Argument => ({
    side: 'pro',
...
function makeConAgent(strength: number) {
  return (round: number, proArgs: Argument[]): Argument => ({
    side: 'con',
...
test("13. pro fn에 이전 con 논증 전달됨", () => {
  const d = new Debater();
  const receivedConArgs: Argument[][] = [];
  const result = d.debate({
    proposition: "논증 전달",
    pro: (round, conArgs) => {
...
test("14. con fn에 이전 pro 논증 전달됨", () => {
  const d = new Debater();
  const receivedProArgs: Argument[][] = [];
  const result = d.debate({
    proposition: "논증 전달",
    pro: makeProAgent(0.6),
    con: (round, proArgs) => {
...
test("21. 각 Argument의 strength 0~1 범위", () => {
  ...
  const allValid = result.rounds.every(r =>
    r.proArgument.strength >= 0 && r.proArgument.strength <= 1 &&
