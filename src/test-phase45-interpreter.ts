...
test("산술 연산 (+)", () => {
  ...
  const result = flInterpret(src);
  if (result !== 7) throw new Error(`got ${result}`);
});
...
test("if false 분기", () => {
  ...
  const result = flInterpret(src);
  if (result !== 2) throw new Error(`got ${result}`);
});

test("if 중첩", () => {
  const src = `[FUNC f :params [$n] :body ((if (< $n 0) "neg" (if (= $n 0) "zero" "pos")))] (f 5)`;
  const result = flInterpret(src);
  if (result !== "pos") throw new Error(`got ${result}`);
});
...
(countdown 100 0)`;
  const result = flInterpret(src);
  if (result !== 100) throw new Error(`got ${result}`);
...
test("interpreter.fl을 Gen1 컴파일러로 재컴파일 (Gen2 interpreter)", () => {
  ...
  if (typeof mod2.interpret !== "function") throw new Error("interpret not exported");
