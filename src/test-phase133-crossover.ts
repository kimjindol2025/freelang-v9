...
import { Crossover, globalCrossover, crossoverNumbers, crossoverStrings } from "./crossover";
...
test("21. crossoverPoint 범위 유효", () => {
  ...
  assert(res.crossoverPoint! >= 0 && res.crossoverPoint! <= a.length,
    `crossoverPoint 범위 벗어남: ${res.crossoverPoint}`);
});
...
test("23. uniform: 두 부모 요소만 사용", () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    for (const v of res.child1) {
      if (!allVals.has(v)) { valid = false; break; }
    }
  }
