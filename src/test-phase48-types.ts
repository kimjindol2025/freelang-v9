...
test("typechecker.fl → Gen1 컴파일", () => {
  ...
  if (typeof tcMod1.typecheck !== "function") throw new Error("typecheck not exported");
  console.log(`    → typechecker Gen1 컴파일 성공 (${js.length} chars)`);
});

function flTypecheck(flSrc: string): { errors: any[]; warnings: any[] } {
  const ast = convertTStoFL(parse(lex(flSrc)));
  return tcMod1.typecheck(ast) as { errors: any[]; warnings: any[] };
}
...
(add 1 2)`;
  ...
  if (r.errors.length !== 0) throw new Error(`errors: ${JSON.stringify(r.errors)}`);
});
...
(add 1 2 3)`;
  const r = flTypecheck(src);
  if (r.errors.length === 0) throw new Error("에러가 없음");
  if (!r.errors[0].msg.includes("아리티")) throw new Error(`expected 아리티 error: ${r.errors[0].msg}`);
...
(add 1 2)`;
  const r = flTypecheck(src);
  if (r.errors.length !== 0) throw new Error(`unexpected: ${JSON.stringify(r.errors)}`);
});
...
test("미정의 변수 참조 → 에러", () => {
  ...
  const r = flTypecheck(src);
  if (r.errors.length === 0) throw new Error("에러가 없음");
  if (!r.errors[0].msg.includes("미정의")) throw new Error(`expected 미정의: ${r.errors[0].msg}`);
  console.log(`    → 감지됨: ${r.errors[0].msg}`);
});
...
(f 5)`;
  const r = flTypecheck(src);
  const varErrors = r.errors.filter((e: any) => e.msg.includes("미정의"));
  if (varErrors.length !== 0) throw new Error(`unexpected: ${JSON.stringify(varErrors)}`);
});
...
(f "hello")`;
  ...
  if (r.errors.length !== 0) {
    const arityErrs = r.errors.filter((e: any) => e.msg.includes("아리티"));
    if (arityErrs.length !== 0) throw new Error(`unexpected arity error: ${arityErrs[0].msg}`);
  }
...
test("typecheck 반환 { errors, warnings }", () => {
  ...
  if (!Array.isArray(r.errors)) throw new Error("errors가 배열이 아님");
  if (!Array.isArray(r.warnings)) throw new Error("warnings가 배열이 아님");
});
...
[FUNC add :params [$a $b] :body ((+ $a $b))]`;
  const r = flTypecheck(src);
  if (r.errors.length === 0) throw new Error("에러 없음");
  const e = r.errors[0];
  if (!e.level || !e.msg) throw new Error(`구조 이상: ${JSON.stringify(e)}`);
  if (e.level !== "error") throw new Error(`level이 error가 아님: ${e.level}`);
});
...
test("Gen1 FL 툴체인 로드", () => {
  ...
  const lAst = convertTStoFL(parse(lex(lexerSrc)));
  (interp.context as any).variables.set("$__fl_ast__", lAst);
  interp.interpret(parse(lex("(gen-js $__fl_ast__)")));
  lexerMod1 = makeModule(interp.context.lastValue as string);

  const pAst = convertTStoFL(parse(lex(parserSrc)));
  (interp.context as any).variables.set("$__fl_ast__", pAst);
  interp.interpret(parse(lex("(gen-js $__fl_ast__)")));
  parserMod1 = makeModule(interp.context.lastValue as string);

  const cAst = convertTStoFL(parse(lex(codegenSrc)));
  (interp.context as any).variables.set("$__fl_ast__", cAst);
  interp.interpret(parse(lex("(gen-js $__fl_ast__)")));
  codegenMod1 = makeModule(interp.context.lastValue as string);
});
...
test("typechecker.fl → Gen2 (Gen1으로 컴파일)", () => {
  ...
  if (typeof tcMod2.typecheck !== "function") throw new Error("typecheck not exported");
