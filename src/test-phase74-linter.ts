...
import { FLLinter } from "./linter";
import { ALL_RULES, undefinedVars, unusedBindings, emptyBody, arityCheck, unreachableMatch, deadCode, shadowedVars } from "./lint-rules";
...
// 특정 규칙만 포함한 Linter
function makeLinterWith(...rules: typeof ALL_RULES): FLLinter {
  const linter = new FLLinter();
...
test("깨끗한 FUNC 정의 — empty-body 없음", () => {
  const linter = makeLinterWith(emptyBody);
  const src = `
...
test("깨끗한 코드 — arity-check 에러 없음", () => {
  const linter = makeLinterWith(arityCheck);
  const src = `
...
test("깨끗한 코드 — unreachable-match 에러 없음", () => {
  const linter = makeLinterWith(unreachableMatch);
  const src = `
...
test("미정의 변수 $z 참조", () => {
  const linter = makeLinterWith(undefinedVars);
  const src = `
...
test("unused let 바인딩 감지", () => {
  const linter = makeLinterWith(unusedBindings);
  const src = `
...
test("wildcardPattern 이후 케이스 감지", () => {
  ...
  try {
    ...
    if (diags.some(d => d.rule === "parse-error")) return; // 파서가 처리 못하면 OK
  } catch {
    // 파서 예외도 OK
  }
});
...
test("warn만 필터링", () => {
  const linter = makeLinterWith(emptyBody, arityCheck);
  const src = `
...
function runPhase56(src: string): any {
  const interp = new Interpreter();
  interp.interpret(parse(lex(src)));
  return (interp as any).context.lastValue;
}

function runPhase56Multi(src: string): Interpreter {
  const interp = new Interpreter();
  interp.interpret(parse(lex(src)));
  return interp;
}
...
function applyFn(interp: Interpreter, fnVal: any, args: any[]): any {
  // fn 값을 직접 호출하기 위해 임시 FUNC 우회
  // interp의 evaluator를 직접 사용
  const evalFn = (interp as any).evalFn || (interp as any).callFunction;
  if (evalFn) return evalFn.call(interp, fnVal, args, (interp as any).context);
  return undefined;
}
...
test("[Phase56-1] 함수 내 define이 전역 $x를 오염 안 함", () => {
  ...
  if (x !== 10) throw new Error(`전역 $x가 ${x}로 변경됨`);
});
...
test("[Phase56-7] let 스코프 격리", () => {
  ...
  if (local !== undefined && local !== null) throw new Error(`$local이 외부에 보임: ${local}`);
});
  ...
  const res = runPhase56(`
    (let [[$x 1]]
      (let [[$y 2]]
        (+ $x $y)))
  `);
  if (res !== 3) throw new Error(`got ${res}`);
});
...
test("[Phase56-9] set! — 클로저 상태 변경", () => {
  ...
  if (c !== 3) throw new Error(`counter는 3이어야 함, got ${c}`);
});
...
test("[Phase56-10] set! — 전역 변수만 변경", () => {
  ...
  `);
  const x = getVar(interp, "x");
  if (x !== 10) throw new Error(`x는 10이어야 함, got ${x}`);
});
...
test("[Phase56-12] 서로 다른 함수 간 변수 격리", () => {
  ...
  if (shared !== undefined && shared !== null) throw new Error(`$shared가 전역에 보임: ${shared}`);
});
...
test("[Phase56-13] 재귀 + 클로저 조합", () => {
  ...
  if (c !== 3) throw new Error(`cnt는 3이어야 함, got ${c}`);
});
...
test("[Phase56-14] 깊은 중첩 클로저", () => {
  const res = runPhase56(`
    ...
    (g 30)
  `);
  if (res !== 60) throw new Error(`got ${res}`);
});
...
test("dead-code: do 블록 순수 표현식 감지", () => {
  const linter = makeLinterWith(deadCode);
  const src = `
    [FUNC test :params [$x]
      :body (do
        42
...
test("shadowed-vars: let이 파라미터 덮어쓰기 감지", () => {
  const linter = makeLinterWith(shadowedVars);
  const src = `
...
test("ALL_RULES 배열에 7개 규칙 포함", () => {
  assertEqual(ALL_RULES.length, 7, "7개 규칙 있어야 함");
