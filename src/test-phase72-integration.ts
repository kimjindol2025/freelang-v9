...
function getVar(interp: Interpreter, name: string): any {
  ...
  const val = (interp as any).context.variables.get(withDollar);
  if (val !== undefined) return val;
  // fallback: 함수 레지스트리에서도 확인
  return (interp as any).context.functions.get(name)?.capturedValue
    ?? (interp as any).context.variables.get(name);
}
...
test("TC-3b: when — 조건이 false일 때 nil", () => {
  ...
  assert(
    result === null || result === undefined || result === "nil",
    `when false → nil/null (got ${JSON.stringify(result)})`
  );
});
...
test("TC-6d: [Phase 61] TCO — callUserFunctionTCO 50만 재귀", () => {
  ...
  const result = callUserFunctionTCO(interp as any, "count-down", [500000]);
...
test("TC-6e: [Phase 63] 표준 매크로 when — 등록 확인", () => {
  // 매크로가 등록되어 있는지 확인
  const interp = new Interpreter();
  const expander = (interp as any).context?.macroExpander;
