...
test("defprotocol 정의가 레지스트리에 등록됨", () => {
  ...
  if (proto.methods.length !== 2) throw new Error(`Expected 2 methods, got ${proto.methods.length}`);
  if (proto.methods[0].name !== "serialize") throw new Error(`Expected 'serialize', got '${proto.methods[0].name}'`);
  if (proto.methods[1].name !== "deserialize") throw new Error(`Expected 'deserialize', got '${proto.methods[1].name}'`);
  interp.destroy();
});
...
test("deserialize Point → 기본값 객체 반환", () => {
  ...
  if (typeof result !== "object" || result === null) throw new Error(`Expected object, got ${typeof result}`);
  if (result.__type !== "Point") throw new Error(`Expected __type "Point", got "${result.__type}"`);
  if (result.x !== 0.0) throw new Error(`Expected x=0.0, got ${result.x}`);
  interp.destroy();
});
...
test("Point와 Circle 모두 Serializable 구현", () => {
  ...
  if (rp !== "Point(1,2)") throw new Error(`Point serialize: expected "Point(1,2)", got "${rp}"`);
  if (rc !== "Circle(r=5)") throw new Error(`Circle serialize: expected "Circle(r=5)", got "${rc}"`);
  interp.destroy();
});
...
test("dispatch은 __type 기반으로 올바른 구현 선택", () => {
  ...
  if (d1 !== "Dog named Buddy") throw new Error(`Dog: expected "Dog named Buddy", got "${d1}"`);
  if (d2 !== "Cat named Whiskers") throw new Error(`Cat: expected "Cat named Whiskers", got "${d2}"`);
  interp.destroy();
});
...
test("__type이 없는 객체 → 에러", () => {
  ...
  if (!threw) throw new Error("Expected error for type without __type");
});
...
test("__type이 구현 없는 타입 → 에러", () => {
  ...
  if (!threw) throw new Error("Expected error for unimplemented type");
});
...
test("3-메서드 프로토콜 구현 및 호출", () => {
  ...
  if (a !== 12) throw new Error(`area: expected 12, got ${a}`);
  if (p !== 14) throw new Error(`perimeter: expected 14, got ${p}`);
  if (l !== "Rectangle") throw new Error(`label: expected "Rectangle", got "${l}"`);
  interp.destroy();
});
...
test("같은 프로토콜을 여러 Shape 타입에 적용", () => {
  ...
  if (ar !== 20) throw new Error(`Rect area: expected 20, got ${ar}`);
  // 3.14 * 4 = 12.56
  if (Math.abs(ac - 12.56) > 0.01) throw new Error(`Circ area: expected ~12.56, got ${ac}`);
  interp.destroy();
});
...
test("defstruct 생성자 __type 자동 태깅", () => {
  ...
  if (!v || v.__type !== "Vector3") throw new Error(`Expected __type "Vector3", got "${v?.__type}"`);
  if (v.x !== 1.0) throw new Error(`Expected x=1.0, got ${v.x}`);
  if (v.z !== 3.0) throw new Error(`Expected z=3.0, got ${v.z}`);
  interp.destroy();
});
...
test("defstruct 술어 함수 작동", () => {
  ...
  if (ic !== true) throw new Error(`Circle? should return true, got ${ic}`);
  if (in_ !== false) throw new Error(`Circle? should return false for non-Circle, got ${in_}`);
  interp.destroy();
});
...
test("defstruct + 다형성 2개 타입", () => {
  ...
  if (n1 !== "Alice") throw new Error(`Person name: expected "Alice", got "${n1}"`);
  if (n2 !== "ACME Corp") throw new Error(`Company name: expected "ACME Corp", got "${n2}"`);
  interp.destroy();
});
...
test("기본 산술", () => {
  ...
  if (r !== 17) throw new Error(`Expected 17, got ${r}`);
});
...
test("if 분기 — false", () => {
  ...
  if (r !== 99) throw new Error(`Expected 99, got ${r}`);
});
...
test("and/or 단락 평가", () => {
  ...
  if (r1 !== true) throw new Error(`and true true: expected true, got ${r1}`);
  if (r2 !== false) throw new Error(`and true false: expected false, got ${r2}`);
  if (r3 !== true) throw new Error(`or false true: expected true, got ${r3}`);
});
...
test("cond 다분기", () => {
  const r = run(`
    ...
    (cond
      ...
      [(> $x 0) "positive"])
  `);
  if (r !== "positive") throw new Error(`Expected "positive", got "${r}"`);
});

test("string 연산", () => {
  const r = run(`(str "hello" " " "world")`);
  if (r !== "hello world") throw new Error(`Expected "hello world", got "${r}"`);
});

test("맵 리터럴 + get", () => {
  const r = run(`(get {:name "Alice" :age 30} :name)`);
  if (r !== "Alice") throw new Error(`Expected "Alice", got "${r}"`);
});
