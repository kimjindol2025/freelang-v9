...
const ALL_STRATEGIES: ReasoningStrategy[] = ['COT', 'TOT', 'HYPOTHESIS', 'DEBATE', 'REFLECT', 'DIRECT'];
...
test("1. MetaReasoner 생성", () => {
  ...
  return mr instanceof MetaReasoner;
});
...
test("5. scores 내림차순 정렬", () => {
  ...
  for (let i = 0; i < result.scores.length - 1; i++) {
    if (result.scores[i].score < result.scores[i + 1].score) return false;
  }
  return true;
});
...
test("13. addStrategy() 추가", () => {
  const mr = new MetaReasoner(['COT', 'TOT']);
...
test("14. 커스텀 strategies 생성자", () => {
  const mr = new MetaReasoner(['COT', 'REFLECT']);
  const result = mr.analyze("step by step solution");
...
test("15. globalMetaReasoner 싱글톤", () => {
  return globalMetaReasoner instanceof MetaReasoner;
});

// 16. meta-reason 내장함수 (Interpreter 통해)
test("16. meta-reason 내장함수", () => {
  const interp = new Interpreter();
  interp.interpret(parse(lex('(meta-reason "step by step how to solve")')));
  const result = (interp as any).context.lastValue;
  return typeof result === "string" && ALL_STRATEGIES.includes(result as ReasoningStrategy);
});

// 17. meta-reason-scores 내장함수
test("17. meta-reason-scores 내장함수", () => {
  const interp = new Interpreter();
  interp.interpret(parse(lex('(meta-reason-scores "compare options for this")')));
  const result = (interp as any).context.lastValue;
...
test("18. meta-reason-rationale 내장함수", () => {
  const interp = new Interpreter();
  interp.interpret(parse(lex('(meta-reason-rationale "should we do this or that")')));
  const result = (interp as any).context.lastValue;
...
test("30. meta-reason-scores 결과 구조 검증", () => {
  const interp = new Interpreter();
  interp.interpret(parse(lex('(meta-reason-scores "step by step how")')));
  const result = (interp as any).context.lastValue as any;
