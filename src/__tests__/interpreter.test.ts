...
describe('Interpreter - 기본 실행', () => {
  ...
  test('변수 할당 및 접근', () => {
    const tokens = lexer.tokenize('var x = 42; x');
  ...
  test('기본 산술 연산', () => {
    const tokens = lexer.tokenize('var result = 10 + 5');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('배열 생성 및 접근', () => {
    const tokens = lexer.tokenize('var arr = [1, 2, 3]; arr[0]');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('객체 생성 및 속성 접근', () => {
    const tokens = lexer.tokenize('var obj = { x: 10 }; obj.x');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('조건부 실행', () => {
    const tokens = lexer.tokenize('var x = 0; if (5 > 3) { x = 1 }');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('반복문 실행', () => {
    const tokens = lexer.tokenize('var sum = 0; while (sum < 100) { sum = sum + 1 }');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('재귀 함수', () => {
    const tokens = lexer.tokenize('fn fact(n) { if (n <= 1) { 1 } else { n * fact(n-1) } }');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('클로저 동작', () => {
    const tokens = lexer.tokenize('fn outer(x) { fn inner(y) { x + y } }');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('스코프 격리', () => {
    const tokens = lexer.tokenize('var x = 5; { var x = 10 }; x');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
...
describe('Interpreter - 비동기 실행', () => {
  ...
  test('Promise 생성', async () => {
    const tokens = lexer.tokenize('var p = Promise.resolve(42)');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('async/await 실행', async () => {
    const tokens = lexer.tokenize('async fn test() { await Promise.resolve(1) }');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('동시 실행 (Promise.all)', async () => {
    const tokens = lexer.tokenize('var results = Promise.all([1, 2, 3])');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('에러 처리 (try-catch)', async () => {
    const tokens = lexer.tokenize('try { throw "error" } catch(e) { var caught = true }');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('타임아웃 처리', async () => {
    const tokens = lexer.tokenize('var timeout = new Promise((r) => setTimeout(r, 100))');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
...
describe('Interpreter - 타입 시스템', () => {
  ...
  test('타입 강제 - 정수', () => {
    const tokens = lexer.tokenize('var x: int = 42');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('타입 강제 - 문자열', () => {
    const tokens = lexer.tokenize('var s: string = "hello"');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('제네릭 함수 실행', () => {
    const tokens = lexer.tokenize('fn<T> id(x: T): T { x }');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('포인터 역참조', () => {
    const tokens = lexer.tokenize('var x = 10; var ptr = &x; *ptr');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
...
describe('Interpreter - 에러 처리', () => {
  ...
  test('정의되지 않은 변수 접근', () => {
    const tokens = lexer.tokenize('undefined_var');
    const ast = parser.parse(tokens);
    expect(() => interpreter.interpret(ast)).toBeDefined();
  });

  test('타입 불일치 감지', () => {
    const tokens = lexer.tokenize('var x: int = "string"');
    const ast = parser.parse(tokens);
    expect(() => interpreter.interpret(ast)).toBeDefined();
  });

  test('스택 오버플로우 방지', () => {
    const tokens = lexer.tokenize('fn bad() { bad() }');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
  ...
  test('메모리 누수 방지', () => {
    const tokens = lexer.tokenize('var arr = []; for i in range(0, 1000) { arr.push(i) }');
    const ast = parser.parse(tokens);
    const context = interpreter.interpret(ast);
