...
function benchmarkCode(
  ...
  funcName: string,
  iterations: number = 1
): { result: any; avgTime: number; totalTime: number } {
  ...
  for (let i = 0; i < iterations; i++) {
    result = (interp as any).eval(testFunc.body);
  }
...
`;
  ...
  const { result, avgTime, totalTime } = benchmarkCode(
    deepCallCode,
    "deep-call-test",
    iterations
  );
...
try {
  ...
  console.log(
    `✅ Variable lookup 300000 times: ${lookupTime.toFixed(2)}ms`
  );
