...
export interface PackageManifest {
  ...
  version: string;
  description?: string;
  flDeps: Record<string, string>;  // FL 패키지 의존성
  exports: string[];               // export할 함수/심볼
}

// ---------------------------------------------------------------------------
// Semver 간단 구현 (major.minor.patch 비교용)
// ---------------------------------------------------------------------------
 ...
 *   [PKG my-pkg :version "1.0.0" :desc "설명" :deps {other-pkg "^1.0"} :exports [fn1 fn2]]
 */
export function parseManifest(src: string): PackageManifest {
  // [PKG ...] 블록 추출 (중첩 대괄호 고려)
  ...
  if (end === -1) throw new Error('parseManifest: [PKG 블록이 닫히지 않았습니다');
  ...
  if (tokens[0] !== 'PKG') throw new Error('parseManifest: 첫 토큰이 PKG 아님');

  const name = tokens[1];
  if (!name || name.startsWith(':')) throw new Error('parseManifest: 패키지 이름 누락');

  let version = '0.0.0';
  let description: string | undefined;
  let flDeps: Record<string, string> = {};
  let exports: string[] = [];
...
function tokenizeBlock(src: string): string[] {
  const tokens: string[] = [];
  let i = 0;
...
function parseDepsBlock(raw: string): Record<string, string> {
...
export function resolveTree(
  ...
  for (const [depName, depVersion] of Object.entries(manifest.flDeps)) {
    ...
    if (!depManifest) {
      throw new Error(
        `resolveTree: 의존성 "${depName}@${depVersion}"를 찾을 수 없습니다 (요청: "${root}")`
      );
    }
