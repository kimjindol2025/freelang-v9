...
import { MemorySystem, globalMemory, MemoryScope } from './memory-system';
...
test('20. globalMemory 싱글톤 공유', () => {
  globalMemory.clear();
  globalMemory.remember('singleton-test', 'global-value');
...
test('21. mem-remember 내장함수', () => {
  ...
  const v = globalMemory.recall('test-key');
...
test('22. mem-recall 내장함수', () => {
  globalMemory.clear();
  globalMemory.remember('interp-test', 'hello');
  const interp = makeInterp();
...
test('25. mem-stats 내장함수', () => {
  globalMemory.clear();
  globalMemory.remember('a', 1);
  globalMemory.remember('b', 2);
  const interp = makeInterp();
