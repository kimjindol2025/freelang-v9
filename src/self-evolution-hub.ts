...
import { EvolutionEngine, evolveNumbers as _evolveNumbers, evolveStrings as _evolveStrings } from "./evolve";
import { Mutator } from "./mutate";
import { Crossover } from "./crossover";
import { FitnessEvaluator } from "./fitness";
import { GenerationLoop } from "./generation";
import { Pruner } from "./prune";
import { SelfRefactorer } from "./refactor-self";
import { SelfBenchmark } from "./benchmark-self";
import { SelfVersioning } from "./version-self";
import { randomUUID } from "crypto";

// ── 타입 정의 ─────────────────────────────────────────────────────────────────

export interface EvolutionCycleConfig {
  target: unknown;                  // 목표 솔루션
  populationSize: number;           // 개체군 크기
  generations: number;              // 세대 수
  mutationRate: number;             // 변이율
  eliteRatio: number;               // 엘리트 비율
  pruneThreshold: number;           // 가지치기 임계값
  enableVersioning: boolean;        // 버전 관리 사용 여부
  enableBenchmark: boolean;         // 벤치마크 측정 여부
  enableRefactor: boolean;          // 리팩토링 사용 여부
}

export const DEFAULT_CYCLE_CONFIG: EvolutionCycleConfig = {
  target: null,
  populationSize: 20,
  generations: 30,
  mutationRate: 0.1,
...
export interface EvolutionCycleResult {
  best: unknown;                    // 최고 솔루션
  bestFitness: number;              // 최고 적합도
  generations: number;              // 실행 세대 수
  improvements: number;             // 총 개선 횟수
  prunedCount: number;              // 가지치기된 수
  benchmarkMs?: number;             // 실행 시간 (enableBenchmark)
  versionId?: string;               // 저장된 버전 ID (enableVersioning)
  report: string;                   // 텍스트 리포트
}
...
export interface SelfEvolutionReport {
  ...
  fitnessProgress: number[];        // 세대별 최고 적합도
  refactorSuggestions: number;      // 리팩토링 제안 수
  versions: number;                 // 생성된 버전 수
  summary: string;
}

// ── SelfEvolutionHub ──────────────────────────────────────────────────────────

export class SelfEvolutionHub {
  private fitnessEval: FitnessEvaluator;
  private pruner: Pruner<unknown>;
  private refactorer: SelfRefactorer;
  private benchmark: SelfBenchmark;
  private versioning: SelfVersioning<unknown>;

  // 내부 통계
  private _cycleCount: number = 0;
  private _totalGenerations: number = 0;
  private _refactorSuggestions: number = 0;
  private _versionCount: number = 0;
  private _fitnessHistory: number[] = [];

  constructor() {
    this.fitnessEval = new FitnessEvaluator({ normalize: true, maximize: true });
    this.pruner = new Pruner<unknown>();
    this.refactorer = new SelfRefactorer();
    this.benchmark = new SelfBenchmark("self-evolution-hub");
    this.versioning = new SelfVersioning<unknown>(200);
  }
  ...
  runCycle(
    population: unknown[],
    fitnessFunc: (item: unknown) => number,
    mutateFunc: (item: unknown) => unknown,
    crossoverFunc: (a: unknown, b: unknown) => unknown,
    config?: Partial<EvolutionCycleConfig>
  ): EvolutionCycleResult {
    const cfg: EvolutionCycleConfig = { ...DEFAULT_CYCLE_CONFIG, ...config };
    ...
    let best: unknown = pop[0];
    let improvements = 0;
    let prunedCount = 0;
    const fitnessProgress: number[] = [];
    let gens = 0;
  ...
  evolveNumbers(
    ...
    const cfg: EvolutionCycleConfig = { ...DEFAULT_CYCLE_CONFIG, target, ...config };
    const popSize = cfg.populationSize;
    const len = target.length;

    // 초기 population 생성 (랜덤 배열)
    const initPop: number[][] = Array.from({ length: popSize }, () =>
      Array.from({ length: len }, () => Math.random() * 10)
    );

    // 적합도: 목표 배열에 가까울수록 높음
    const fitnessFunc = (item: unknown): number => {
    ...
    // 변이: 랜덤 인덱스 값 조정
    const mutateFunc = (item: unknown): unknown => {
      const arr = [...(item as number[])];
    ...
    // 교배: 단순 단일점 교배
    const crossoverFunc = (a: unknown, b: unknown): unknown => {
  ...
  evolveString(
    ...
    // 초기 population
    const initPop: string[] = Array.from({ length: popSize }, () =>
  ...
  selfImprove(baseConfig: Partial<EvolutionCycleConfig>): {
    ...
    const base: EvolutionCycleConfig = { ...DEFAULT_CYCLE_CONFIG, ...baseConfig };

    // 여러 설정 후보로 테스트
    const candidates: EvolutionCycleConfig[] = [
      { ...base },
  ...
  private _emptyResult(cfg: EvolutionCycleConfig): EvolutionCycleResult {
    return {
      best: null,
      bestFitness: 0,
      generations: 0,
  ...
  private _buildReport(info: {
    ...
    if (info.benchmarkMs !== undefined) parts.push(`실행시간=${info.benchmarkMs}ms`);
  ...
  get totalGenerations(): number { return this._totalGenerations; }
  get refactorSuggestions(): number { return this._refactorSuggestions; }
  get versionCount(): number { return this._versionCount; }
  get fitnessHistory(): number[] { return [...this._fitnessHistory]; }
}

// ── 싱글톤 ────────────────────────────────────────────────────────────────────

export const globalSelfEvolution = new SelfEvolutionHub();
