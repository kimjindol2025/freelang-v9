...
test("TC-1: FLAgent 생성", () => {
  const agent = new FLAgent({ goal: "테스트 목표", maxSteps: 5 });
  assert(agent instanceof FLAgent, "FLAgent 인스턴스");
});

test("TC-2: getState 초기값 (step=0, done=false)", () => {
  const agent = new FLAgent({ goal: "초기화 테스트", maxSteps: 8 });
...
test("TC-3: step() 추가 + 실행", () => {
  const agent = new FLAgent({ goal: "step 테스트", maxSteps: 3 });
...
test("TC-4: 단계 후 step 카운트 증가", () => {
  const agent = new FLAgent({ goal: "카운트 테스트", maxSteps: 5 });
  agent
    .step("step1", (_s) => "a")
    .step("step2", (_s) => "b")
    .step("step3", (_s) => "c");
...
test("TC-5: setResult → done=true", () => {
  const agent = new FLAgent({ goal: "결과 테스트", maxSteps: 10 });
...
test("TC-6: AgentState 구조 확인", () => {
  const agent = new FLAgent({ goal: "구조 확인", maxSteps: 5 });
  const state = agent.getState();
...
test("TC-7: run() — 1~5 합 계산 (15)", () => {
  ...
  const state = agent.run((s) => {
    const n = getMemory(s, "n", 1);
    const sum = getMemory(s, "sum", 0);
    if (n > 5) {
      agentDoneSignal(s, sum);
      return null;
    }
    setMemory(s, "sum", sum + n);
    setMemory(s, "n", n + 1);
    return {
      step: s.step,
      thought: `n=${n} 더하기`,
      action: "add",
      observation: getMemory(s, "sum", 0),
    };
  });
...
test("TC-8: maxSteps 초과 시 강제 종료", () => {
  const agent = new FLAgent({ goal: "무한 루프 방지", maxSteps: 3 });
...
test("TC-10: history 배열 누적", () => {
  const agent = new FLAgent({ goal: "history 테스트", maxSteps: 5 });
  agent
    .step("step-a", (_s) => "A")
    .step("step-b", (_s) => "B")
    .step("step-c", (_s) => "C");
...
test("TC-11: AgentAction 구조 (step, thought, action, observation)", () => {
  const agent = new FLAgent({ goal: "action 구조", maxSteps: 5 });
  const state = agent.run((_s) => {
    agentDoneSignal(_s, "done");
    return {
      step: 0,
      thought: "첫 번째 생각",
      action: "test-action",
  ...
  // 직접 액션을 history에 확인
  const agent2 = new FLAgent({ goal: "action 구조 확인", maxSteps: 5 });
  let callCount = 0;
...
test("TC-12: memory 읽기/쓰기", () => {
  const agent = new FLAgent({ goal: "memory 테스트", maxSteps: 5 });
  const state = agent.getState();
  assertEquals(getMemory(state, "x", 0), 0, "기본값 0");
  setMemory(state, "x", 100);
  assertEquals(getMemory(state, "x", 0), 100, "x=100 저장됨");
  setMemory(state, "name", "FreeLang");
  assertEquals(getMemory(state, "name", ""), "FreeLang", "name 저장됨");
});
...
test("TC-14: 에이전트가 목표 달성 후 result 반환", () => {
  const agent = new FLAgent({ goal: "목표 달성", maxSteps: 10 });
  let counter = 0;
  const state = agent.run((s) => {
    counter++;
    if (counter >= 3) {
      agentDoneSignal(s, "목표달성!");
      return null;
    }
...
test("TC-16: `(agent-result $agent)` → 최종값", () => {
  // TS에서 직접 테스트
  const agent = agentNew({ goal: "결과 확인", maxSteps: 5 });
  agent.setResult(42);
  assertEquals(agentResult(agent), 42, `result=${agentResult(agent)}`);
});

test("TC-17: `(agent-history $agent)` → 배열", () => {
  const agent = agentNew({ goal: "history 확인", maxSteps: 5 });
  agent.step("초기화", (_s) => "init");
  agentRun(agent);
  const hist = agentHistory(agent);
...
test("TC-19: maxSteps=1 → 1단계만", () => {
  const agent = new FLAgent({ goal: "1단계", maxSteps: 1 });
  const state = agent.run((_s) => ({
    step: _s.step,
    thought: "단일 단계",
    action: "single",
...
test("TC-21: toMarkdown → '## Step' 포함", () => {
  const agent = new FLAgent({ goal: "마크다운 테스트", maxSteps: 5 });
...
test("TC-22: 빈 history", () => {
  const agent = new FLAgent({ goal: "빈 에이전트", maxSteps: 5 });
...
test("TC-23: 중첩 에이전트 (agent 내부에서 새 agent 실행)", () => {
  const outerAgent = new FLAgent({ goal: "외부 에이전트", maxSteps: 5 });
  let innerResult: any = null;
  outerAgent.step("내부 에이전트 실행", (_s) => {
    const innerAgent = new FLAgent({ goal: "내부 에이전트", maxSteps: 3 });
...
test("TC-24: Phase 56 regression 14/14", () => {
  ...
  const out = execSync(
    ...
    { cwd: "/home/kimjin/kim/Desktop/kim/01_Active_Projects/freelang-v9" }
  ).toString();
  const m = out.match(/(\d+) passed, (\d+) failed/);
  if (!m) throw new Error("출력 파싱 실패: " + out.slice(-200));
