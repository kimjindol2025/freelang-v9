...
export function createCollectionModule() {
  return {
    ...
    "arr_take": (arr: any[], n: number): any[] => arr.slice(0, n),

    // arr_drop arr n -> [any]  (all but first n elements)
    "arr_drop": (arr: any[], n: number): any[] => arr.slice(n),

    // arr_sum arr -> number
    "arr_sum": (arr: number[]): number => arr.reduce((a, b) => a + b, 0),
    ...
    // arr_min arr -> number
    "arr_min": (arr: number[]): number => Math.min(...arr),

    // arr_max arr -> number
    "arr_max": (arr: number[]): number => Math.max(...arr),
