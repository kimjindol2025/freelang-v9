...
function testParser(code: string, description: string, validator: (ast: any) => boolean) {
  try {
    const tokens = lex(code);
    const ast = tsParse(tokens);
...
if (testParser(
  "(+ 1 2)",
  "TEST 1: Number literals in S-expr",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].args[0].kind === "literal" && ast[0].args[0].type === "number"
)) passed++;
...
if (testParser(
  ...
  (ast) => ast.length > 0 && ast[0].args[0].kind === "literal" && ast[0].args[0].type === "string"
)) passed++;
...
if (testParser(
  "(+ $x $y)",
  "TEST 3: Variables ($x, $y)",
  (ast) => ast.length > 0 && ast[0].args[1].kind === "variable"
)) passed++;
...
if (testParser(
  ...
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].args[0].kind === "literal" && ast[0].args[0].type === "symbol"
)) passed++;
...
if (testParser(
  "(+ 1 2)",
  "TEST 5: Simple S-expression (+ 1 2)",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr" && ast[0].op === "+"
)) step2++;
...
if (testParser(
  "(+ 1 (* 2 3))",
  "TEST 6: Nested S-expression (+ 1 (* 2 3))",
  (ast) => ast.length > 0 && ast[0].args[1].kind === "sexpr"
)) step2++;
...
if (testParser(
  "(fn [x] (+ x 1))",
  "TEST 7: Multiple nested expressions",
  (ast) => ast.length > 0 && ast[0].kind === "sexpr"
)) step2++;
...
if (testParser(
  "(fn [$x] (+ $x 1))",
  "TEST 8: Function definition (fn [$x] ...)",
  (ast) => ast.length > 0 && ast[0].op === "fn"
)) step2++;
...
if (testParser(
  "[1 2 3]",
  "TEST 9: Array literal [1 2 3]",
  (ast) => ast.length > 0 && ast[0].kind === "block" && ast[0].type === "Array"
)) step3++;
...
if (testParser(
  "[$x $y $z]",
  "TEST 10: Array with variables [$x $y $z]",
  (ast) => ast.length > 0 && (ast[0].fields?.get("items") as any[])?.length === 3
)) step3++;
...
if (testParser(
  ...
  "TEST 11: FUNC block definition",
  (ast) => ast.length > 0 && ast[0].kind === "block" && ast[0].type === "FUNC"
)) step3++;

// TEST 12: Nested blocks
step3total++;
if (testParser(
  "[FUNC test :params [[$x int]] :body (+ $x 1)]",
  "TEST 12: Nested array in block",
  (ast) => ast.length > 0 && ast[0].type === "FUNC"
)) step3++;
...
if (testParser(
  "(let [[$x 1]] (+ $x 2))",
  "TEST 14: let form (let [[name value]] body)",
  (ast) => ast.length > 0 && ast[0].op === "let"
)) step4++;
...
if (testParser(
  "(if (> $x 0) (+ $x 1) (- $x 1))",
  "TEST 15: if form (if condition then else)",
  (ast) => ast.length > 0 && ast[0].op === "if"
)) step4++;
...
if (testParser(
  ...
  "TEST 16: cond form (cond [cond body] ...)",
  (ast) => ast.length > 0 && ast[0].op === "cond"
)) step4++;
...
combinedtotal++;
if (testParser(
  "[FUNC test :params [$x] :body (+ $x 1)] (test 5)",
  "TEST 18: Multiple top-level forms",
  (ast) => ast.length === 2 && ast[0].kind === "block" && ast[1].kind === "sexpr"
)) combined++;
