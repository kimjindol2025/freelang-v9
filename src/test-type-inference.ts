...
import { TypeInferenceEngine, TypeAnnotation } from "./type-inference";

console.log("🔍 Phase 6: Type Inference Tests\n");

function typeToString(type: TypeAnnotation): string {
  if (type.union) {
    return type.union.map((t: TypeAnnotation) => typeToString(t)).join(" | ");
  }
...
try {
  const engine1 = new TypeInferenceEngine();
  ...
  const pass1 =
    intType.name === "int" && strType.name === "string" && boolType.name === "bool";
...
try {
  const engine2 = new TypeInferenceEngine();
...
try {
  const engine3 = new TypeInferenceEngine();
...
try {
  const engine4 = new TypeInferenceEngine();

  // Function that returns int
  const doubleFunc = {
    kind: "function-block",
    name: "double",
    params: [{ name: "$x", type: { kind: "type" as const, name: "int" } }],
    body: {
      kind: "sexpr",
      op: "*",
  ...
  const greetFunc = {
    kind: "function-block",
    name: "greet",
    params: [{ name: "$name", type: { kind: "type" as const, name: "string" } }],
    body: {
      kind: "sexpr",
      op: "concat",
...
try {
  const engine5 = new TypeInferenceEngine();
...
try {
  const engine6 = new TypeInferenceEngine();

  // identity<T>(x: T) function
  const identityFunc = {
    generics: ["T"],
    params: [{ name: "$x", type: { kind: "type" as const, name: "T", isTypeVariable: true } }],
...
try {
  const engine7 = new TypeInferenceEngine();
...
try {
  const engine8 = new TypeInferenceEngine();
  ...
  const pass8 =
    xType.name === "int" && nameType.name === "string" && itemsType.name === "array<int>";
...
try {
  const engine9 = new TypeInferenceEngine();
  ...
  const pass9 =
    addOneFunc?.returnType.name === "int" && makePairFunc?.returnType.name === "pair<int,string>";
...
try {
  const engine10 = new TypeInferenceEngine();
