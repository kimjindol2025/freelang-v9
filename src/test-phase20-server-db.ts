...
import { createServerModule } from "./stdlib-server";
...
const mockCallFn = (name: string, args: any[]) => {
  if (name === "handle_hello") return { msg: "hello" };
  if (name === "handle_text") return "plain text";
  if (name === "handle_status") return { __fl_response: true, type: "json", status: 201, body: { created: true } };
  throw new Error(`Unknown handler: ${name}`);
};

const S = createServerModule(mockCallFn);
...
try { S["server_get"]("/test", "handle_hello"); routeRegistered = true; } catch {}
...
const proj = D["db_project"]("freelang-v9") as any;
check("db_project: 오류 없이 반환 (null or data)", proj === null || typeof proj === "object");

const all = D["db_all"]("projects") as any[];
...
try {
  ...
  // 카운트
  const cnt = D["db_count"](tmpDb, "items") as number;
  check("db_count: 2행", cnt === 2);

  // 조회
  const rows = D["db_query"](tmpDb, "SELECT * FROM items ORDER BY val") as any[];
  ...
  // ? 바인딩
  const filtered = D["db_query"](tmpDb, "SELECT * FROM items WHERE name=?", ["bar"]) as any[];
  check("db_query: ? 바인딩", filtered.length === 1 && filtered[0]?.name === "bar");

  // 업데이트
  D["db_update"](tmpDb, "items", { val: 100 }, "name='foo'");
  const updated = D["db_query"](tmpDb, "SELECT val FROM items WHERE name='foo'") as any[];
  check("db_update: val=100", Number(updated[0]?.val) === 100);

  // 테이블 목록
  const tables = D["db_tables"](tmpDb) as string[];
  check("db_tables: items 포함", tables.includes("items"));

  // 행 삭제
  D["db_delete_row"](tmpDb, "items", "name='foo'");
  const cntAfter = D["db_count"](tmpDb, "items") as number;
