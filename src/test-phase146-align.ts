...
function assertLte(a: number, b: number, msg?: string): void {
  if (a > b) throw new Error(msg ?? `Expected ${a} <= ${b}`);
}

function assertBetween(a: number, lo: number, hi: number, msg?: string): void {
...
function makeSystem(): AlignmentSystem {
  const sys = new AlignmentSystem();
...
function makeGoodAction(): Action {
  return {
    id: "action-good",
...
test("19. align-score 빌트인", () => {
  ...
  globalAlignment.addGoal({ id: "g-builtin", description: "빌트인 목표", priority: 7, measurable: true });
  globalAlignment.addValue({ id: "v-builtin", name: "신뢰", description: "신뢰 구축", weight: 0.8 });
...
test("21. align-conflicts 빌트인", () => {
  ...
  globalAlignment.addGoal({ id: "gc1", description: "빠른 성장", priority: 10, measurable: true });
  globalAlignment.addGoal({ id: "gc2", description: "안정적 유지", priority: 2, measurable: false });
