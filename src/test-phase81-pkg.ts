...
function assertThrows(fn: () => void, containing?: string): void {
  try {
    fn();
...
test('TC-1: PackageRegistry 생성', () => {
  ...
  assert(registry instanceof PackageRegistry, 'instanceof PackageRegistry');
...
test('TC-10: resolveTree — 단일 패키지', () => {
  ...
  const tree = resolveTree('standalone', registry);
...
test('TC-11: resolveTree — 의존성 1단계', () => {
  ...
  const tree = resolveTree('app', registry);
...
test('TC-12: resolveTree — 의존성 없음', () => {
  ...
  const tree = resolveTree('empty-deps', registry);
  assertEqual(tree.length, 1);
...
test('TC-13: resolveTree — 없는 dep → 에러', () => {
  ...
  assertThrows(() => resolveTree('broken-app', registry), '찾을 수 없습니다');
});
...
test('TC-15: semver 비교', () => {
  assert(compareSemVer('2.0.0', '1.0.0') > 0, '2.0.0 > 1.0.0');
  assert(compareSemVer('1.0.0', '2.0.0') < 0, '1.0.0 < 2.0.0');
  assert(compareSemVer('1.0.0', '1.0.0') === 0, '1.0.0 == 1.0.0');
  assert(compareSemVer('1.1.0', '1.0.9') > 0, '1.1.0 > 1.0.9');
...
test('TC-18: 순환 의존성 방지', () => {
  ...
  assertThrows(() => resolveTree('cycle-a', registry), '순환 의존성');
});
...
test('TC-19: satisfiesVersion — ^ 범위', () => {
  assert(satisfiesVersion('1.2.3', '^1.0.0'), '1.2.3 satisfies ^1.0.0');
  assert(satisfiesVersion('1.9.9', '^1.0.0'), '1.9.9 satisfies ^1.0.0');
  assert(!satisfiesVersion('2.0.0', '^1.0.0'), '2.0.0 does not satisfy ^1.0.0');
  assert(!satisfiesVersion('0.9.9', '^1.0.0'), '0.9.9 does not satisfy ^1.0.0');
});
...
test('TC-23: resolveTree — 2단계 의존성', () => {
  ...
  const tree = resolveTree('level-2', registry);
...
test('TC-24: parseManifest — PKG 블록 없음 → 에러', () => {
  assertThrows(() => parseManifest('no pkg block here'), '[PKG');
});
