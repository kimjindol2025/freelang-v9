...
import {
  FreeLangV9,
  freelangV9,
  FREELANG_V9_MANIFEST,
  FreeLangV9Config,
  FreeLangV9Status,
  FreeLangV9Response,
} from "./freelang-v9-complete";

import { evalPhase150Complete } from "./eval-phase150";
...
test("1. FreeLangV9 기본 생성", () => {
  const fl = new FreeLangV9();
  return fl instanceof FreeLangV9;
});
...
test("3. FreeLangV9 최소 설정 생성", () => {
  const fl = new FreeLangV9({ enableEthics: false, enableWisdom: false });
  return fl instanceof FreeLangV9;
});
...
test("10. process — ethicsCheck 포함", () => {
  const fl = new FreeLangV9({ enableEthics: true });
...
test("11. process — aligned boolean 포함", () => {
  const fl = new FreeLangV9({ enableAlignment: true });
  const r = fl.process("정렬 테스트");
  return typeof r.aligned === "boolean";
});

test("12. process — wisdom 문자열 포함", () => {
  const fl = new FreeLangV9({ enableWisdom: true });
...
test("28. selfDiagnose — healthy=true (기본 설정)", () => {
  const fl = new FreeLangV9();  // 모두 활성화 기본값
  const d = fl.selfDiagnose();
...
test("38. FREELANG_V9_MANIFEST 존재", () => {
  return FREELANG_V9_MANIFEST !== null && FREELANG_V9_MANIFEST !== undefined;
});

test("39. MANIFEST.phases = 150", () => {
  return FREELANG_V9_MANIFEST.phases === 150;
});

test("40. MANIFEST.tiers = 10", () => {
  return FREELANG_V9_MANIFEST.tiers === 10;
});

test("41. MANIFEST.features 50개 이상", () => {
  return Array.isArray(FREELANG_V9_MANIFEST.features) &&
    FREELANG_V9_MANIFEST.features.length >= 50;
});

test("42. MANIFEST.version '9.0.0'", () => {
  return FREELANG_V9_MANIFEST.version === "9.0.0";
});

test("43. MANIFEST.description 존재", () => {
  return typeof FREELANG_V9_MANIFEST.description === "string" &&
    FREELANG_V9_MANIFEST.description.length > 0;
});

test("44. MANIFEST.philosophy 배열", () => {
  return Array.isArray(FREELANG_V9_MANIFEST.philosophy) &&
    FREELANG_V9_MANIFEST.philosophy.length > 0;
});

test("45. MANIFEST.tiers_detail 10개", () => {
  return Object.keys(FREELANG_V9_MANIFEST.tiers_detail).length === 10;
});
...
test("46. fl-v9-process 빌트인", () => {
  const r = evalPhase150Complete("fl-v9-process", ["AI 테스트"]);
  return r instanceof Map && r.has("input") && r.has("executionMs");
});

test("47. fl-v9-status 빌트인", () => {
  const r = evalPhase150Complete("fl-v9-status", []);
  return r instanceof Map && r.get("version") === "9.0.0";
});

test("48. fl-v9-think 빌트인", () => {
  const r = evalPhase150Complete("fl-v9-think", ["AI 결정", ["안전"]]);
  return r instanceof Map && r.has("reasoning");
});

test("49. fl-v9-diagnose 빌트인", () => {
  const r = evalPhase150Complete("fl-v9-diagnose", []);
  return r instanceof Map && r.has("healthy") && r.has("score");
});

test("50. fl-v9-features 빌트인", () => {
  const r = evalPhase150Complete("fl-v9-features", []);
  return Array.isArray(r) && r.length > 0;
});

test("51. fl-v9-version 빌트인", () => {
  const r = evalPhase150Complete("fl-v9-version", []);
  return r === "9.0.0";
});

test("52. fl-v9-enable 빌트인", () => {
  const r = evalPhase150Complete("fl-v9-enable", ["ethics-check"]);
  return r === null;
});

test("53. fl-v9-disable 빌트인", () => {
  const r = evalPhase150Complete("fl-v9-disable", ["wisdom"]);
  return r === null;
});

test("54. fl-v9-manifest 빌트인", () => {
  const r = evalPhase150Complete("fl-v9-manifest", []);
...
test("55. fl-v9-manifest features 50개 이상", () => {
  const r = evalPhase150Complete("fl-v9-manifest", []);
  const features = (r as Map<string, any>).get("features");
...
test("56. freelangV9 전역 인스턴스 존재", () => {
  return freelangV9 instanceof FreeLangV9;
});

test("57. 전역 인스턴스 version '9.0.0'", () => {
  return freelangV9.getVersion() === "9.0.0";
});

test("58. fl-v9-status tiers 10개 (빌트인)", () => {
  const s = evalPhase150Complete("fl-v9-status", []) as Map<string, any>;
...
test("60. Tier 1~10 통합 확인 — MANIFEST features 포함 검증", () => {
  ...
  const manifest = FREELANG_V9_MANIFEST.features;
...
test("61. Tier 5 features 포함 검증", () => {
  ...
  return tier5.every(f => FREELANG_V9_MANIFEST.features.includes(f));
});

test("62. Tier 8 features 포함 검증", () => {
  const tier8 = ["consensus", "delegate", "vote", "swarm", "orchestrate"];
  return tier8.every(f => FREELANG_V9_MANIFEST.features.includes(f));
});

test("63. Tier 9 features 포함 검증", () => {
  const tier9 = ["evolve", "mutate", "crossover", "fitness", "generation"];
  return tier9.every(f => FREELANG_V9_MANIFEST.features.includes(f));
});
...
test("66. unknown 빌트인 → null", () => {
  const r = evalPhase150Complete("unknown-op-xyz", []);
  return r === null;
});

test("67. fl-v9-think executionMs 타입 체크", () => {
  const r = evalPhase150Complete("fl-v9-think", ["문제"]) as Map<string, any>;
  return typeof r.get("executionMs") === "number";
});

test("68. fl-v9-diagnose healthy 타입", () => {
  const r = evalPhase150Complete("fl-v9-diagnose", []) as Map<string, any>;
