...
import { Swarm, Particle, SwarmResult, globalSwarm } from "./swarm";
...
function assertApprox(a: number, b: number, delta: number, msg?: string) {
  if (Math.abs(a - b) > delta) throw new Error(msg ?? `Expected ${a} ≈ ${b} (delta=${delta})`);
}
...
test("3. bestPosition 범위 내", () => {
  ...
  assert(result.bestPosition >= 0 && result.bestPosition <= 5,
    `bestPosition=${result.bestPosition} 범위 초과`);
});
...
test("5. iterations 기록", () => {
  ...
  assert(result.iterations >= 1 && result.iterations <= 20,
    `iterations=${result.iterations} 범위 초과`);
});
...
test("10. bounds 적용 [-5, 5]", () => {
  ...
  assert(result.bestPosition >= -5 && result.bestPosition <= 5,
    `bestPosition=${result.bestPosition} 범위 초과`);
});
...
test("16. globalSwarm 싱글톤", () => {
  ...
  assert(globalSwarm instanceof Swarm, "globalSwarm이 Swarm 인스턴스가 아님");
  const result = globalSwarm.optimize({ objective: (x) => x });
...
test("23. global best = particles 중 최대 bestScore", () => {
  ...
  assert(result.bestScore >= maxParticleScore - 1e-9,
    `globalBest=${result.bestScore} < particle max=${maxParticleScore}`);
});
...
try {
  const regOutput = execSync(
    ...
    { cwd: "/home/kimjin/kim/Desktop/kim/01_Active_Projects/freelang-v9", encoding: "utf8", timeout: 30000 }
  );
