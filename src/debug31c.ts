...
// Try to find the invalid token
try {
  new Function(lexer2JS);
} catch(e: any) {
  console.log("Syntax error:", e.message);
  // Find any non-printable chars
  for (let i = 0; i < Math.min(lexer2JS.length, 500); i++) {
    const code = lexer2JS.charCodeAt(i);
    if (code < 32 && code !== 10 && code !== 9) {
      console.log(`Non-printable char at ${i}: code=${code}, context:`, JSON.stringify(lexer2JS.substring(i-20, i+20)));
    }
  }
  // Search whole string
  let found = 0;
  for (let i = 0; i < lexer2JS.length; i++) {
    const code = lexer2JS.charCodeAt(i);
    if (code < 32 && code !== 10 && code !== 9) {
      if (found < 5) {
        console.log(`Non-printable char[${found}] at ${i}: code=${code}, context:`, JSON.stringify(lexer2JS.substring(i-30, i+30)));
      }
      found++;
    }
  }
  console.log(`Total non-printable chars: ${found}`);
}
