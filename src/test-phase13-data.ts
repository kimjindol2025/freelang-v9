...
function runWith(vars: Record<string, any>, code: string): any {
...
test("json_has true", () => {
  if (D.json_has({ name: "ai" }, "name") !== true) throw new Error("Expected true");
});

test("json_has false", () => {
  if (D.json_has({ name: "ai" }, "age") !== false) throw new Error("Expected false");
});

test("json_str", () => {
  const s = D.json_str({ ok: true });
  if (JSON.parse(s).ok !== true) throw new Error(`Expected ok=true`);
});
...
test("json_merge", () => {
  ...
  if (r.x !== 1 || r.y !== 2) throw new Error(`Expected x=1 y=2`);
});

test("json_merge overwrite", () => {
  const r = D.json_merge({ x: 1, y: 0 }, { y: 99 });
  if (r.y !== 99) throw new Error(`Expected y=99, got ${r.y}`);
});
...
test("json_del", () => {
  ...
  if ("secret" in r) throw new Error("Expected secret deleted");
  if (r.name !== "ai") throw new Error("Expected name preserved");
});

test("json_get dot-path", () => {
  const r = D.json_get({ user: { name: "claude", level: 9 } }, "user.name");
  if (r !== "claude") throw new Error(`Expected "claude", got "${r}"`);
});

test("json_get array index", () => {
  const r = D.json_get({ items: ["a", "b", "c"] }, "items.1");
  if (r !== "b") throw new Error(`Expected "b", got "${r}"`);
});
...
test("json_set", () => {
  ...
  if (r.y !== 42) throw new Error(`Expected y=42`);
  if (r.x !== 1) throw new Error("Expected x preserved");
});

test("json_set immutable (original unchanged)", () => {
  const orig = { x: 1 };
  D.json_set(orig, "y", 42);
  if ("y" in orig) throw new Error("Original should not be mutated");
});
...
test("csv_parse basic", () => {
  ...
  if (rows[0][0] !== "name") throw new Error(`Expected header "name"`);
  if (rows[1][1] !== "9") throw new Error(`Expected "9"`);
});

test("csv_parse quoted cell", () => {
  const rows = D.csv_parse(`a,"hello, world",c`);
  if (rows[0][1] !== "hello, world") throw new Error(`Expected "hello, world", got "${rows[0][1]}"`);
});

test("csv_header", () => {
  const rows = [["id", "name"], ["1", "claude"]];
  const h = D.csv_header(rows);
  if (h[0] !== "id" || h[1] !== "name") throw new Error(`Expected [id, name]`);
});

test("csv_to_objects", () => {
  const rows = [["name", "phase"], ["claude", "13"], ["ai", "9"]];
  const objs = D.csv_to_objects(rows);
  if (objs.length !== 2) throw new Error(`Expected 2 objects`);
...
test("csv_write", () => {
  ...
  if (!s.includes("a,b") || !s.includes("1,2")) throw new Error(`Got: ${s}`);
});
...
test("csv_roundtrip", () => {
  ...
  if (parsed[1][0] !== "claude") throw new Error(`Roundtrip failed: ${JSON.stringify(parsed)}`);
});
...
test("str_template basic", () => {
  ...
  if (r !== "Hello claude, phase 13") throw new Error(`Got: "${r}"`);
});

test("str_template missing key stays", () => {
  const r = D.str_template("Hello {name} at {time}", { name: "ai" });
  if (!r.includes("{time}")) throw new Error(`Expected {time} to remain, got "${r}"`);
});
...
test("str_join_lines", () => {
  ...
  if (s !== "a\nb\nc") throw new Error(`Expected "a\\nb\\nc", got "${s}"`);
});

test("str_trim", () => {
  if (D.str_trim("  hello  ") !== "hello") throw new Error("Expected trimmed string");
});

test("str_words", () => {
  const words = D.str_words("  hello world  ai  ");
  if (words.length !== 3) throw new Error(`Expected 3 words, got ${words.length}`);
});

test("str_count", () => {
  if (D.str_count("abcabcabc", "abc") !== 3) throw new Error("Expected 3");
});
...
test("http_json + json_get via $var in FreeLang", () => {
  ...
  interp.interpret(parse(lex(`(http_json "http://localhost:40000/health")`)));
  ...
  if (typeof status !== "string") throw new Error(`Expected string status, got ${typeof status}`);
...
test("shell + csv_parse pipeline", () => {
  ...
  const interp = new Interpreter();
  interp.interpret(parse(lex(`(shell "printf 'name,score\\nclaude,100\\nai,99'")`)));
  const csv = interp.context.lastValue;
  const rows = D.csv_parse(csv);
  const objs = D.csv_to_objects(rows);
  if (objs[0].name !== "claude") throw new Error(`Expected claude, got ${objs[0].name}`);
  if (objs[0].score !== "100") throw new Error(`Expected 100, got ${objs[0].score}`);
});

test("str_template + http_get response formatting", () => {
  const msg = D.str_template(
    "AI language: {name} | version: {version} | phase: {phase}",
    { name: "FreeLang v9", version: "13", phase: "data-transform" }
  );
  if (!msg.includes("FreeLang v9")) throw new Error(`Template failed: ${msg}`);
  console.log(`    → ${msg}`);
});
