...
try {
  ...
  if (result instanceof FreeLangPromise && result.getState() === "rejected") {
    console.log("✅ Promise 즉시 reject: 상태 rejected");
    console.log("   에러 발생 (예상대로)\n");
  } else {
    console.log(`❌ Promise rejected 실패: ${result instanceof FreeLangPromise ? result.getState() : typeof result}\n`);
  }
} catch (e: any) {
  console.log(`❌ 에러: ${e.message}\n`);
}
...
try {
  ...
  if ((result as any)?.kind === "async-function-value") {
    console.log("✅ Async 함수 정의: async-function-value 생성됨");
    console.log("   이름: test-fn\n");
  } else {
    console.log(`❌ Async 함수 실패: ${(result as any)?.kind || typeof result}\n`);
  }
} catch (e: any) {
  console.log(`❌ 에러: ${e.message}\n`);
}
...
try {
  ...
  const p = new FreeLangPromise((resolve) => {
    resolve(5);
  });

  const chained = p.then((v) => v * 2);

  if (chained instanceof FreeLangPromise && chained.getState() === "resolved") {
    ...
    console.log(`❌ then 체이닝 실패: ${chained instanceof FreeLangPromise ? chained.getState() : typeof chained}\n`);
  }
} catch (e: any) {
  console.log(`❌ 에러: ${e.message}\n`);
}
...
try {
  ...
  const p = new FreeLangPromise((_, reject) => {
    reject(new Error("test error"));
  });

  const caught = p.catch((err) => "recovered from " + err.message);

  if (caught instanceof FreeLangPromise && caught.getState() === "resolved") {
    ...
    console.log(`❌ catch 실패: ${caught instanceof FreeLangPromise ? caught.getState() : typeof caught}\n`);
  }
} catch (e: any) {
  console.log(`❌ 에러: ${e.message}\n`);
}
...
try {
  ...
  const p1 = new FreeLangPromise((resolve) => resolve(1));
  const p2 = new FreeLangPromise((resolve) => resolve(2));
  const p3 = new FreeLangPromise((resolve) => resolve(3));

  const all = FreeLangPromise.all([p1, p2, p3]);
...
try {
  ...
  const p1 = new FreeLangPromise((resolve) => {
    setTimeout(() => resolve("first"), 10);
  });
  const p2 = new FreeLangPromise((resolve) => {
    setTimeout(() => resolve("second"), 20);
  });

  const race = FreeLangPromise.race([p1, p2]);
...
try {
  ...
  const p = new FreeLangPromise((resolve) => resolve(42));
  const result = p.finally(() => {
    finallyCalled = true;
  });

  if (result instanceof FreeLangPromise && result.getState() === "resolved") {
    const value = result.getValue();
    ...
    console.log(`❌ finally 상태 오류: ${result instanceof FreeLangPromise ? result.getState() : typeof result}\n`);
  }
} catch (e: any) {
  console.log(`❌ 에러: ${e.message}\n`);
}
...
try {
  ...
  const result = new FreeLangPromise((resolve) => resolve(2))
    .then((v) => v * 2)  // 4
    .then((v) => v + 3)  // 7
    .then((v) => v * 10); // 70
...
try {
  ...
  const result = new FreeLangPromise((_, reject) => {
