...
import { RAGStore, RAGDocument, globalRAG, tokenize, tfidf } from "./rag";
...
test("TC-1: RAGStore 인스턴스 생성", () => {
  ...
  if (!(store instanceof RAGStore)) throw new Error("RAGStore 인스턴스가 아님");
});

test("TC-2: add() 단일 문서 추가", () => {
  const store = new RAGStore();
  store.add({ id: "doc1", content: "FreeLang is an AI-native language" });
  if (store.size() !== 1) throw new Error(`size=${store.size()}, 기대=1`);
});
...
test("TC-3: addMany() 여러 문서 추가", () => {
  ...
  if (store.size() !== 3) throw new Error(`size=${store.size()}, 기대=3`);
});
...
test("TC-4: size() 문서 수 확인", () => {
  ...
  if (store.size() !== 2) throw new Error(`size=${store.size()}, 기대=2`);
});
...
test("TC-5: all() 전체 문서 반환", () => {
  ...
  const all = store.all();
  if (all.length !== 2) throw new Error(`all.length=${all.length}, 기대=2`);
  if (all[0].id !== "x" && all[1].id !== "x") throw new Error("문서 id x 없음");
});
...
test("TC-6: retrieve() 매칭 없음 → []", () => {
  ...
  if (results.length !== 0) throw new Error(`results.length=${results.length}, 기대=0`);
});
...
test("TC-7: retrieve() 매칭 → 결과 있음", () => {
  ...
  if (results.length === 0) throw new Error("결과가 없음, 기대 >= 1");
  if (results[0].id !== "d1") throw new Error(`첫 결과 id=${results[0].id}, 기대=d1`);
});
...
test("TC-8: retrieve() topK 제한", () => {
  ...
  if (results.length > 2) throw new Error(`results.length=${results.length}, 기대 <= 2`);
});
...
test("TC-9: retrieve() score 기준 정렬 (높은 순)", () => {
  ...
  if (results.length < 2) throw new Error("결과가 2개 미만");
  const s0 = results[0].score ?? 0;
  const s1 = results[1].score ?? 0;
  if (s0 < s1) throw new Error(`score 정렬 오류: ${s0} < ${s1}`);
});
...
test("TC-10: retrieve() score 0 이하 필터링", () => {
  ...
  for (const r of results) {
    if ((r.score ?? 0) <= 0) throw new Error(`score <= 0: ${r.score}`);
  }
});
...
test("TC-11: query() 기본 augment 포함 결과", () => {
  ...
  if (!result.query) throw new Error("query 필드 없음");
  if (!Array.isArray(result.retrieved)) throw new Error("retrieved가 배열이 아님");
  if (typeof result.augmented !== "string") throw new Error("augmented가 문자열이 아님");
  if (!result.augmented.includes("FreeLang")) throw new Error("augmented에 FreeLang 없음");
});
...
test("TC-12: query() 커스텀 augment 함수", () => {
  ...
  if (!result.augmented.startsWith("CUSTOM:")) throw new Error(`augmented=${result.augmented}`);
});
...
test("TC-13: query() topK 옵션 적용", () => {
  ...
  if (result.retrieved.length > 2) throw new Error(`retrieved.length=${result.retrieved.length}, 기대 <= 2`);
});
...
test("TC-14: remove() 존재하는 ID → true", () => {
  ...
  if (!ok) throw new Error("remove 반환값이 false");
  if (store.size() !== 0) throw new Error(`size=${store.size()}, 기대=0`);
});

test("TC-15: remove() 없는 ID → false", () => {
  const store = new RAGStore();
  const ok = store.remove("nonexistent");
  if (ok !== false) throw new Error(`remove 반환값이 ${ok}, 기대=false`);
});
...
test("TC-16: metadata 포함 문서 추가·검색", () => {
  ...
  if (results.length === 0) throw new Error("결과 없음");
  if (!results[0].metadata) throw new Error("metadata 없음");
  if (results[0].metadata.author !== "Vaswani") throw new Error("metadata.author 오류");
});

test("TC-17: 한국어 토크나이즈", () => {
  const tokens = tokenize("안녕하세요 프리랭 언어입니다");
  if (!tokens.includes("안녕하세요")) throw new Error(`토큰에 '안녕하세요' 없음: ${tokens}`);
  if (!tokens.includes("프리랭")) throw new Error(`토큰에 '프리랭' 없음: ${tokens}`);
});

test("TC-18: tfidf() 직접 호출 — 겹치는 토큰 있을 때 > 0", () => {
  const score = tfidf(["machine", "learning"], ["machine", "learning", "model"]);
  if (score <= 0) throw new Error(`score=${score}, 기대 > 0`);
});
...
test("TC-19: globalRAG는 싱글톤 RAGStore", () => {
  if (!(globalRAG instanceof RAGStore)) throw new Error("globalRAG가 RAGStore 아님");
  const beforeSize = globalRAG.size();
  globalRAG.add({ id: "__test__", content: "singleton test" });
  if (globalRAG.size() !== beforeSize + 1) throw new Error("싱글톤 상태 공유 실패");
  globalRAG.remove("__test__");
});
...
test("TC-20: (rag-size) — 초기 문서 수", () => {
  // globalRAG 비우기
  const all = globalRAG.all();
  for (const d of all) globalRAG.remove(d.id);
  const result = run(`(rag-size)`);
  if (typeof result !== 'number') throw new Error(`rag-size 반환타입 오류: ${typeof result}`);
});
...
test("TC-21: (rag-add \"id\" \"content\") — 문서 추가 → true", () => {
  ...
  if (result !== true) throw new Error(`rag-add 반환값=${result}, 기대=true`);
  if (globalRAG.size() < 1) throw new Error("globalRAG에 문서 없음");
});

test("TC-22: (rag-retrieve \"query\" topK) — 검색 결과 리스트", () => {
  const all2 = globalRAG.all();
  for (const d of all2) globalRAG.remove(d.id);
  globalRAG.add({ id: "r1", content: "FreeLang AI language reasoning system" });
  globalRAG.add({ id: "r2", content: "cooking food recipes kitchen" });
  const result = run(`(rag-retrieve "FreeLang AI" 3)`);
  if (!Array.isArray(result)) throw new Error(`결과가 배열이 아님: ${typeof result}`);
  if (result.length === 0) throw new Error("결과 배열이 비어있음");
  if (!result[0].id || !result[0].content) throw new Error("결과 항목에 id/content 없음");
});

test("TC-23: (rag-query \"query\") — augmented 문자열 반환", () => {
  const all3 = globalRAG.all();
  for (const d of all3) globalRAG.remove(d.id);
  globalRAG.add({ id: "q1", content: "FreeLang language model architecture" });
  const result = run(`(rag-query "FreeLang")`);
  if (typeof result !== 'string') throw new Error(`결과가 문자열이 아님: ${typeof result}`);
  if (!result.includes("FreeLang")) throw new Error("결과에 FreeLang 없음");
});

test("TC-24: (rag-remove \"id\") — 문서 삭제", () => {
  globalRAG.add({ id: "rm1", content: "to be removed document" });
  const sizeBefore = globalRAG.size();
  const result = run(`(rag-remove "rm1")`);
  if (result !== true) throw new Error(`rag-remove 반환값=${result}, 기대=true`);
  if (globalRAG.size() !== sizeBefore - 1) throw new Error("삭제 후 size 오류");
});
...
test("TC-25: add → retrieve → query 전체 파이프라인", () => {
  ...
  if (retrieved.length === 0) throw new Error("retrieve 결과 없음");
  if (!retrieved.some(d => d.id === "p1")) throw new Error("p1이 상위 결과에 없음");
  ...
  if (!result.augmented.includes("FreeLang reasoning")) throw new Error("augmented에 쿼리 없음");
  if (result.retrieved.length === 0) throw new Error("retrieved 비어있음");
});
