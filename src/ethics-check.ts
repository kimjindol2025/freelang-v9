...
export interface EthicsCheckResult {
  subject: string;          // 검사 대상 설명
  passed: boolean;
  violations: EthicsViolation[];
  score: number;            // 0~1 (1이 완전히 윤리적)
  frameworks: Record<EthicsFramework, { passed: boolean; score: number }>;
...
export interface EthicsPrinciple {
  ...
  description: string;
  framework: EthicsFramework;
  check: (subject: string, context: Record<string, unknown>) => { passed: boolean; reason: string };
}

// 기본 원칙 정의들

const doNoHarmPrinciple: EthicsPrinciple = {
  id: 'do-no-harm',
  name: '해악 금지',
  description: '행동이나 출력이 인간이나 생명체에 해를 끼쳐서는 안 된다',
  framework: 'deontological',
  check: (subject: string, _context: Record<string, unknown>) => {
    const harmKeywords = [
...
const fairnessPrinciple: EthicsPrinciple = {
  id: 'fairness',
  name: '공정성',
...
const transparencyPrinciple: EthicsPrinciple = {
  id: 'transparency',
  name: '투명성',
...
const privacyPrinciple: EthicsPrinciple = {
  id: 'privacy',
  name: '개인정보 보호',
...
const autonomyPrinciple: EthicsPrinciple = {
  id: 'autonomy',
  name: '자율성 존중',
...
const DEFAULT_PRINCIPLES: EthicsPrinciple[] = [
  doNoHarmPrinciple,
  fairnessPrinciple,
...
export class EthicsChecker {
  private principles: EthicsPrinciple[];
  ...
  check(
    ...
    const frameworkResults: Record<string, { passed: boolean; score: number; violations: number }> = {
    ...
    // 프레임워크별 점수 계산
    const principlesByFramework: Record<string, number> = {
    ...
    const frameworks: Record<EthicsFramework, { passed: boolean; score: number }> = {
  ...
  checkByFramework(
    ...
    framework: EthicsFramework,
    context: Record<string, unknown> = {}
  ): { passed: boolean; score: number; violations: EthicsViolation[] } {
  ...
  private _determineSeverity(
    ...
    // 컨텍스트에서 severity 힌트
    const ctxSeverity = context['severity'] as string | undefined;
  ...
  private _generateSuggestion(principle: EthicsPrinciple, subject: string): string {
  ...
  private _severityPenalty(severity: 'low' | 'medium' | 'high' | 'critical'): number {
  ...
  private _generateRecommendation(passed: boolean, violations: EthicsViolation[], score: number): string {
    if (passed) {
      return score >= 0.9
...
// 전역 인스턴스
export const globalEthics = new EthicsChecker();
