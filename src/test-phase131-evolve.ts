...
test("1. EvolutionEngine 클래스 인스턴스 생성", () => {
  ...
  return engine instanceof EvolutionEngine;
});
...
test("3. evolveNumbers — 타겟 [1,2,3,4,5] 수렴", () => {
  const result = evolveNumbers([1, 2, 3, 4, 5], 20, 30);
...
test("8. eliteRatio 엘리트 보존 — best fitness 단조 비감소", () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (curBest < prevBest - 0.5) { monotone = false; }
    prevBest = curBest;
  }
  return monotone;
});
...
test("11. evolveNumbers 빌트인 — result 구조 확인", () => {
  const result = evolveNumbers([3, 1, 4, 1, 5], 10, 20);
  return (
    "best" in result &&
...
test("12. evolveStrings 빌트인 — result.best.genome은 문자열", () => {
  const result = evolveStrings("hi", 10, 20);
...
test("23. elite 개체 다음 세대 유지 — best fitness 비감소", () => {
  ...
  for (let i = 1; i < fitnessHistory.length; i++) {
    if (fitnessHistory[i] < fitnessHistory[i - 1] - 10) { monotone = false; }
  }
  return monotone;
});
...
test("25. EvolutionResult — 완전한 구조 검증", () => {
  const result = evolveNumbers([0, 0, 0], 5, 5);
  return (
    "best" in result &&
    "population" in result &&
    "generations" in result &&
...
test("26. evolveStrings — 진화 후 fitness 음수가 작아짐 (타겟 접근)", () => {
  const result = evolveStrings("ab", 20, 50);
...
test("28. history gen 번호 순차 증가", () => {
  ...
  for (let i = 0; i < result.history.length; i++) {
    if (result.history[i].gen !== i + 1) return false;
  }
  return true;
});
