...
function registerWorldModelBuiltins(interp: Interpreter): void {
  ...
  const builtins: Record<string, (...args: any[]) => any> = {
    ...
    "world-add-relation": (...args) => { const kw = kwArgs(args); const r = wm.addRelation({ from: String(kw["from"] ?? ""), to: String(kw["to"] ?? ""), type: String(kw["type"] ?? "related"), strength: typeof kw["strength"] === "number" ? kw["strength"] : 1.0, bidirectional: kw["bidirectional"] === true }); return new Map<string, any>([["id", r.id], ["from", r.from], ["to", r.to], ["type", r.type], ["strength", r.strength], ["bidirectional", r.bidirectional]]); },
    "world-get-relations": (...args) => wm.getRelations(String(args[0] ?? "")).map(r => new Map<string, any>([["id", r.id], ["from", r.from], ["to", r.to], ["type", r.type], ["strength", r.strength], ["bidirectional", r.bidirectional]])),
    ...
    "world-add-rule": (...args) => { const kw = kwArgs(args); const r = wm.addRule({ condition: String(kw["condition"] ?? ""), consequence: String(kw["consequence"] ?? ""), confidence: typeof kw["confidence"] === "number" ? kw["confidence"] : 0.8 }); return new Map<string, any>([["id", r.id], ["condition", r.condition], ["consequence", r.consequence], ["confidence", r.confidence]]); },
    ...
    "world-snapshot": () => { const s = wm.snapshot(); return new Map<string, any>([["entityCount", s.entities.size], ["relationCount", s.relations.length], ["factCount", s.facts.size], ["ruleCount", s.rules.length], ["version", s.version], ["timestamp", s.timestamp.toISOString()]]); },
