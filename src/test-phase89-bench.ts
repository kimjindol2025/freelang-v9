...
import { BenchmarkSuite, BenchmarkResult } from "./benchmarks/bench-runner";
import { runVMBenchmarks } from "./benchmarks/bench-vm";
...
function assertEqual<T>(a: T, b: T, msg?: string): void {
  ...
  if (!same) throw new Error(msg ?? `기대: ${JSON.stringify(b)}, 실제: ${JSON.stringify(a)}`);
}
...
test("TC-11: toJSON — 배열 형태", () => {
  ...
  assert((json as any[]).length === 1, "JSON 배열 길이가 1이 아님");
});
...
test("TC-17: 빠른 작업 > 느린 작업 opsPerSec", () => {
  ...
  assert(fast.opsPerSec > slow.opsPerSec,
    `빠른 작업(${fast.opsPerSec.toFixed(0)})이 느린 작업(${slow.opsPerSec.toFixed(0)})보다 느림`);
});
...
test("TC-19: VM 벤치마크 — 결과 숫자값 정확성", () => {
  ...
  // VM 벤치마크 전체 실행
  const vmResults = runVMBenchmarks();
