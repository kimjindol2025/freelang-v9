...
const quality_loop_1 = require("./quality-loop");
...
async function main() {
    ...
    await test("1. evaluateQuality() 기본 동작", () => {
        const result = (0, quality_loop_1.evaluateQuality)("Hello World!", quality_loop_1.defaultCriteria, 0.7);
    ...
    await test("2. 단일 기준 평가", () => {
        ...
        const result = (0, quality_loop_1.evaluateQuality)("anything", criteria, 0.7);
    ...
    await test("3. 가중 평균 계산", () => {
        ...
        const result = (0, quality_loop_1.evaluateQuality)("x", criteria, 0.7);
    ...
    await test("4. threshold 초과 → passed=true", () => {
        ...
        const result = (0, quality_loop_1.evaluateQuality)("test", criteria, 0.7);
    ...
    await test("9. score 범위 0~1 보장", () => {
        ...
        const r1 = (0, quality_loop_1.evaluateQuality)("test", [criteria[0]], 0.7);
        const r2 = (0, quality_loop_1.evaluateQuality)("test", [criteria[1]], 0.7);
    ...
    await test("10. qualityLoop() 1라운드 성공", async () => {
        ...
        const result = await (0, quality_loop_1.qualityLoop)({
    ...
    await test("14. generate에 round 번호 전달", async () => {
        ...
        await (0, quality_loop_1.qualityLoop)({
    ...
    await test("17. defaultCriteria length 기준 — 짧은 문자열", () => {
        const lengthCrit = quality_loop_1.defaultCriteria.find(c => c.name === 'length');
    ...
    await test("18. defaultCriteria non-empty 기준", () => {
        const nonEmptyCrit = quality_loop_1.defaultCriteria.find(c => c.name === 'non-empty');
    ...
    await test("19. defaultCriteria no-error 기준", () => {
        const noErrCrit = quality_loop_1.defaultCriteria.find(c => c.name === 'no-error');
    ...
    await test("20. 빈 문자열 → non-empty 실패 → 낮은 점수", () => {
        const result = (0, quality_loop_1.evaluateQuality)("", quality_loop_1.defaultCriteria, 0.7);
    ...
    await test("21. 짧은 문자열 → length 낮은 점수", () => {
        const result = (0, quality_loop_1.evaluateQuality)("hi", quality_loop_1.defaultCriteria, 0.9);
...
//# sourceMappingURL=test-phase106-quality.js.map