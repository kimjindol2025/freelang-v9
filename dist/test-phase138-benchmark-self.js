...
const flInterp = new interpreter_1.Interpreter();
...
test("1. SelfBenchmark 생성", () => {
    const bm = new benchmark_self_1.SelfBenchmark("test-suite");
    assert(bm instanceof benchmark_self_1.SelfBenchmark, "SelfBenchmark 인스턴스여야 함");
});
// --- 2. measure 단순 함수 ---
test("2. measure 단순 함수", () => {
    const bm = new benchmark_self_1.SelfBenchmark();
...
test("11. compare 두 함수 비교", () => {
    const bm = new benchmark_self_1.SelfBenchmark();
    const result = bm.compare("fast", () => 1 + 1, "slow", () => { let x = 0; for (let i = 0; i < 100; i++)
        x += i; return x; }, 20);
...
test("13. winner 결정 로직", () => {
    ...
    assert(result.winner === 'baseline' || result.winner === 'target' || result.winner === 'tie', `winner가 유효한 값이어야 함: ${result.winner}`);
});
// --- 14. speedup 계산 ---
...
test("15. add + run 스위트 실행", () => {
    const bm = new benchmark_self_1.SelfBenchmark("my-suite");
    bm.add("fn1", () => 1 + 1);
...
test("16. BenchmarkSuite 구조", () => {
    const bm = new benchmark_self_1.SelfBenchmark("suite-struct");
    bm.add("t1", () => null);
...
test("17. summary.fastest 정확성", () => {
    const bm = new benchmark_self_1.SelfBenchmark("fastest-test");
    bm.add("slow", () => { let x = 0; for (let i = 0; i < 1000; i++)
        x += i; return x; });
    bm.add("fast", () => 42);
...
test("21. bench-suite 빌트인", () => {
    ...
    assert(result instanceof benchmark_self_1.SelfBenchmark, "bench-suite가 SelfBenchmark를 반환해야 함");
});
// --- 22. bench-add 빌트인 ---
test("22. bench-add 빌트인", () => {
    const suite = flEval('(bench-suite "add-test")');
    assert(suite instanceof benchmark_self_1.SelfBenchmark, "bench-suite가 SelfBenchmark를 반환해야 함");
...
test("28. globalBenchmark 싱글톤", () => {
    assert(benchmark_self_1.globalBenchmark instanceof benchmark_self_1.SelfBenchmark, "globalBenchmark가 SelfBenchmark여야 함");
    const result = benchmark_self_1.globalBenchmark.measure("singleton-test", () => 0, 5);
    assert(result.name === "singleton-test", "measure가 올바르게 동작해야 함");
});
// --- 29. bench 편의 함수 ---
test("29. bench 편의 함수", () => {
    const result = (0, benchmark_self_1.bench)("convenience-test", () => Math.PI, 10);
    assert(result.name === "convenience-test", `name이 "convenience-test"여야 함`);
    assert(result.runs === 10, `runs가 10이어야 함`);
});
// --- 30. 스위트 endTime 설정 ---
test("30. 스위트 endTime 설정", () => {
    const bm = new benchmark_self_1.SelfBenchmark("endtime-test");
    bm.add("fn", () => 1);
    const suite = bm.run(5);
...
//# sourceMappingURL=test-phase138-benchmark-self.js.map