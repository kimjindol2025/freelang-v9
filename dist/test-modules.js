...
`;
    ...
    const func2a = interp2.context.functions.get("test-namespace-1");
    const result2a = interp2.eval(func2a.body);
    const func2b = interp2.context.functions.get("test-namespace-2");
    const result2b = interp2.eval(func2b.body);
...
try {
    ...
    interp3.modules = new Map();
    interp3.modules.set("math", mathModule);
    const registered = interp3.modules.has("math");
...
`;
    ...
    const func4a = interp4.context.functions.get("test-alias-1");
    const result4a = interp4.eval(func4a.body);
    const func4b = interp4.context.functions.get("test-alias-2");
    const result4b = interp4.eval(func4b.body);
...
`;
    ...
    const func5a = interp5.context.functions.get("test-selective-1");
    const result5a = interp5.eval(func5a.body);
    const func5b = interp5.context.functions.get("test-selective-2");
    const result5b = interp5.eval(func5b.body);
    const mapOk = Array.isArray(result5a) && result5a[0] === 2 && result5a[1] === 4;
    const filterOk = Array.isArray(result5b) &&
        result5b.length === 3 &&
        result5b.every((x) => x > 2);
    if (mapOk && filterOk) {
...
`;
    ...
    const func6 = interp6.context.functions.get("test-chain");
