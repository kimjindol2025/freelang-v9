...
test("1. Explainer 클래스 생성", () => {
    const exp = new explain_1.Explainer();
    return exp instanceof explain_1.Explainer;
});
// ── 2. explain 결정 설명
test("2. explain() 결정 설명 반환", () => {
    const result = explain_1.globalExplainer.explain("approved", { score: 0.9, age: 0.6, income: 0.8 });
    return result !== null && result !== undefined;
});
// ── 3. DecisionExplanation 구조
test("3. DecisionExplanation 구조 확인", () => {
    const result = explain_1.globalExplainer.explain("approved", { score: 0.9, age: 0.6 });
...
test("4. reasoning 배열 비어있지 않음", () => {
    const result = explain_1.globalExplainer.explain("deny", { risk: -0.8, income: 0.3 }, "대출 심사");
...
test("6. FeatureImportance 구조 확인", () => {
    ...
    return (typeof f.feature === "string" &&
...
test("9. summary 문자열 포함", () => {
    const result = explain_1.globalExplainer.explain("approved", { score: 0.9 });
    return typeof result.summary === "string" && result.summary.length > 0;
});
// ── 10. featureImportance 계산
test("10. featureImportance() 계산", () => {
    const features = explain_1.globalExplainer.featureImportance({ x: 1.0, y: 2.0, z: 0.5 }, { out: 0.8 });
    return Array.isArray(features) && features.length === 3;
});
// ── 11. 양수/음수 direction
test("11. direction positive/negative 확인", () => {
    const result = explain_1.globalExplainer.explain("approved", { score: 0.9, risk: -0.5 });
...
test("12. localExplain() 로컬 설명 반환", () => {
    const model = (inp) => inp["age"] && Number(inp["age"]) > 18 ? "adult" : "minor";
    const local = explain_1.globalExplainer.localExplain({ age: 25, income: 50000 }, "adult", model);
    return local !== null && local !== undefined;
});
// ── 13. LocalExplanation 구조
test("13. LocalExplanation 구조 확인", () => {
    const local = explain_1.globalExplainer.localExplain({ age: 25 }, "adult", () => "adult");
    return ("input" in local &&
        "output" in local &&
        "topFactors" in local &&
...
test("14. counterfactual 문자열 생성", () => {
    const local = explain_1.globalExplainer.localExplain({ age: 25, score: 0.9 }, "approved", () => "approved");
...
test("15. toNaturalLanguage technical 출력", () => {
    const explanation = explain_1.globalExplainer.explain("approved", { score: 0.9, age: 0.6 });
    const text = explain_1.globalExplainer.toNaturalLanguage(explanation, "technical");
...
test("16. toNaturalLanguage general 출력", () => {
    const explanation = explain_1.globalExplainer.explain("approved", { score: 0.9 });
    const text = explain_1.globalExplainer.toNaturalLanguage(explanation, "general");
    return typeof text === "string" && text.length > 0 && !text.includes("[기술적 설명]");
});
// ── 17. contrastiveExplain 대조 설명
test("17. contrastiveExplain() 대조 설명", () => {
    const text = explain_1.globalExplainer.contrastiveExplain("approved", "denied", { score: 0.8, risk: -0.3 });
...
test("18. extractRules() 규칙 추출", () => {
    ...
    const rules = explain_1.globalExplainer.extractRules(examples);
...
test("20. explain-decision 빌트인", () => {
    ...
    return result instanceof Map && result.has("decision");
});
...
test("22. explain-local 빌트인", () => {
    ...
    return result instanceof Map && result.has("counterfactual");
});
...
test("24. explain-contrast 빌트인", () => {
    ...
    return typeof result === "string" && result.includes("approved");
});
...
test("27. explain-summary 빌트인", () => {
    ...
    const expl = explain_1.globalExplainer.explain("approved", { score: 0.9 }, "test");
    const explaMap = new Map([
...
test("28. confidence 0~1 범위 검증", () => {
    const result = explain_1.globalExplainer.explain("test", { a: 0.5, b: 0.3, c: 0.2 });
    return result.confidence >= 0 && result.confidence <= 1;
});
// ── Bonus 29. 빈 factors 처리
test("29. 빈 factors 안전 처리", () => {
    const result = explain_1.globalExplainer.explain("decision", {});
...
test("30. featureImportance baseline 비교", () => {
    const features = explain_1.globalExplainer.featureImportance({ x: 1.0, y: 0.5 }, { out: 0.9 }, { out: 0.3 });
...
//# sourceMappingURL=test-phase145-explain.js.map