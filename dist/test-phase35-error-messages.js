...
function testErr(name, fn, expectPat) {
    try {
        fn();
...
test("단순 토큰 col 위치", () => {
    ...
    const num1 = tokens[2];
    if (lparen.col !== 1)
        throw new Error(`( col=${lparen.col}, want 1`);
    if (plus.col !== 2)
        throw new Error(`+ col=${plus.col}, want 2`);
    if (num1.col !== 4)
        throw new Error(`1 col=${num1.col}, want 4`);
});
test("멀티라인 col 추적", () => {
    const tokens = lexerMod1.lex("(foo)\n(bar)");
    const bar = tokens.find((t) => t.value === "bar");
    if (!bar)
        throw new Error("bar token not found");
    if (bar.line !== 2)
        throw new Error(`bar line=${bar.line}, want 2`);
    if (bar.col !== 2)
        throw new Error(`bar col=${bar.col}, want 2`);
});
test("들여쓰기 col 추적", () => {
    const tokens = lexerMod1.lex("  [FUNC add");
    const lb = tokens.find((t) => t.type === "LBRACKET");
    const func = tokens.find((t) => t.value === "FUNC");
    if (!lb)
        throw new Error("[ not found");
    if (!func)
        throw new Error("FUNC not found");
    if (lb.col !== 3)
        throw new Error(`[ col=${lb.col}, want 3`);
    if (func.col !== 4)
        throw new Error(`FUNC col=${func.col}, want 4`);
});
test("문자열 토큰 col", () => {
    const tokens = lexerMod1.lex(`  "hello"`);
    const str = tokens[0];
    if (str.col !== 3)
        throw new Error(`str col=${str.col}, want 3`);
    if (str.value !== "hello")
        throw new Error(`value=${str.value}`);
});
// ── 파서 에러 메시지 형식 검증 ────────────────────────────────
console.log("\n[35-C] 파서 에러 메시지: [line:col] + ErrorType");
testErr("미닫힌 괄호", () => parserMod1.parse(lexerMod1.lex("(+ 1 2")), "SyntaxError");
testErr("미닫힌 배열 파싱 에러", () => parserMod1.parse(lexerMod1.lex("[FUNC add :params [$a $b")), "SyntaxError");
testErr("블록 타입 없음", () => parserMod1.parse(lexerMod1.lex("[123 add]")), "ParseError");
testErr("에러에 line:col 포함", () => parserMod1.parse(lexerMod1.lex("\n\n    (")), "[3:" // 3번째 라인
);
// ── 에러 위치 정확도 검증 ─────────────────────────────────────
console.log("\n[35-D] 에러 위치 정확도");
test("1:1 — 첫 글자 에러", () => {
    try {
        ...
        const m = e.message;
        if (!m.includes("[1:1]"))
            throw new Error(`got: ${m.slice(0, 60)}`);
...
test("멀티라인 에러: 2번 줄", () => {
    try {
        ...
        const m = e.message;
        if (!m.includes("[2:"))
            throw new Error(`expected [2:... but got: ${m.slice(0, 60)}`);
...
testErr("Gen2 parser: SyntaxError 형식", () => parserMod2.parse(lexerMod2.lex("(+ 1 2")), "SyntaxError");
testErr("Gen2 parser: [line:col] 형식", () => parserMod2.parse(lexerMod2.lex(")")), "[1:1]");
// ── 결과 ─────────────────────────────────────────────────────
console.log(`\n${"─".repeat(50)}`);
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase35-error-messages.js.map