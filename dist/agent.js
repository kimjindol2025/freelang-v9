...
exports.FLAgent = void 0;
exports.agentNew = agentNew;
...
exports.agentDoneSignal = agentDoneSignal;
exports.agentContinue = agentContinue;
exports.evalAgentBlock = evalAgentBlock;
exports.createAgentBuiltins = createAgentBuiltins;
/**
...
class FLAgent {
     ...
     */
    toMarkdown() {
        const lines = [`# Agent: ${this.state.goal}\n`];
...
exports.FLAgent = FLAgent;
// ── 단축 헬퍼 함수 ──────────────────────────────────────────────────────────
/** 새 에이전트 생성 */
function agentNew(opts) {
    return new FLAgent(opts);
}
/** 에이전트 루프 실행 */
function agentRun(agent, stepFn) {
...
function getMemory(state, key, defaultVal = null) {
    return key in state.memory ? state.memory[key] : defaultVal;
}
/** 에이전트 상태 통해 메모리에 값 저장 */
function setMemory(state, key, value) {
    state.memory[key] = value;
}
/** 에이전트 완료 표시 (done=true, result 설정) */
function agentDoneSignal(state, result) {
    state.done = true;
    state.result = result;
    return state;
}
/** 에이전트 계속 진행 표시 */
function agentContinue(state) {
    return state;
}
 ...
 */
function evalAgentBlock(fields, evalFn, callFnVal) {
    // :goal
    ...
    return {
        ...
        "agent-new": (...args) => {
            ...
            for (let i = 0; i < args.length; i += 2) {
                ...
                const val = args[i + 1];
                if (key === "goal")
                    opts.goal = String(val);
                else if (key === "max-steps")
                    opts.maxSteps = Number(val);
                else if (key === "tools")
                    opts.tools = Array.isArray(val) ? val.map(String) : [];
            }
            return new FLAgent(opts);
        },
        ...
        "agent-run": (agent, stepFnVal) => {
            ...
            const stepFn = (state) => {
                ...
                if (r === null || r === undefined)
                    return null;
                if (typeof r === "object" && "thought" in r)
                    return r;
                return {
                    step: state.step,
                    thought: `step-${state.step}`,
        ...
        "agent-done?": (agent) => agent.isDone(),
        // (agent-result $agent) → 최종값
        "agent-result": (agent) => agent.getState().result ?? null,
        // (agent-history $agent) → 배열
        "agent-history": (agent) => agent.getState().history,
        // (agent-state $agent) → AgentState
        "agent-state": (agent) => agent.getState(),
        // (agent-markdown $agent) → string
        "agent-markdown": (agent) => agent.toMarkdown(),
        // (get-memory $state "key" default) → any
        "get-memory": (state, key, def = null) => getMemory(state, key, def),
        // (set-memory $state "key" value) → void
        "set-memory": (state, key, value) => {
            setMemory(state, key, value);
            return null;
        },
        // (agent-done $state $result) → state (done=true, result 설정)
        "agent-done": (state, result) => agentDoneSignal(state, result),
