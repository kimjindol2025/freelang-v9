...
const doc_renderer_1 = require("./doc-renderer");
let passed = 0;
let failed = 0;
...
function assertContains(haystack, needle, msg) {
    if (!haystack.includes(needle)) {
        throw new Error(`${msg ?? "assertContains 실패"}: "${needle}" not found in output (len=${haystack.length})`);
    }
}
function assertLength(arr, n, msg) {
    if (arr.length !== n) {
        throw new Error(`${msg ?? "assertLength 실패"}: expected length ${n}, got ${arr.length}`);
    }
}
// ─────────────────────────────────────────────────────────────
...
test("TC-8-1: ## 제목 포함", () => {
    const src = `;;; 두 수를 더합니다.\n[FUNC add :params [$a $b] :body (+ $a $b)]`;
    const entries = (0, doc_extractor_1.extractDocs)(src);
    const md = (0, doc_renderer_1.renderMarkdown)(entries, "테스트 API");
    assertContains(md, "# 테스트 API");
});
test("TC-8-2: 목차 포함", () => {
    const src = `[FUNC add :params [$a $b] :body (+ $a $b)]`;
    const entries = (0, doc_extractor_1.extractDocs)(src);
    const md = (0, doc_renderer_1.renderMarkdown)(entries);
...
test("TC-11-1: 함수 목록 한 줄", () => {
    const src = `;;; 더하기\n[FUNC add :params [$a $b] :body (+ $a $b)]`;
    const entries = (0, doc_extractor_1.extractDocs)(src);
    const summary = (0, doc_renderer_1.renderSummary)(entries);
    assertContains(summary, "**add**");
    assertContains(summary, "더하기");
});
test("TC-11-2: 빈 목록 처리", () => {
    const summary = (0, doc_renderer_1.renderSummary)([]);
...
test("TC-12-1: 빈 소스 → 빈 배열", () => {
    const entries = (0, doc_extractor_1.extractDocs)("");
    assertLength(entries, 0, "빈 배열");
});
test("TC-12-2: 주석만 있는 소스 → 빈 배열", () => {
    const entries = (0, doc_extractor_1.extractDocs)("; 일반 주석\n;;; doc 주석");
    assertLength(entries, 0, "빈 배열");
});
// ─────────────────────────────────────────────────────────────
...
test("TC-15-1: 5줄 주석", () => {
    ...
    const entries = (0, doc_extractor_1.extractDocs)(lines.join("\n"));
...
test("TC-20-1: 빈 entries renderMarkdown", () => {
    const md = (0, doc_renderer_1.renderMarkdown)([], "빈 문서");
...
//# sourceMappingURL=test-phase77-doc.js.map