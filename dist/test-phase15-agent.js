...
test("agent_create", () => {
    ...
    if (agent.steps !== 0)
        throw new Error("Steps should be 0");
    if (agent.status !== "running")
        throw new Error("Status should be running");
    if (Object.keys(agent.tools).length !== 0)
        throw new Error("Should have no tools");
});
test("agent_set / agent_get", () => {
    ...
    if (A.agent_get(agent, "goal") !== "find answer")
        throw new Error("Wrong goal");
    if (A.agent_get(agent, "confidence") !== 0.9)
        throw new Error("Wrong confidence");
});
test("agent_set immutable", () => {
    const original = A.agent_create("test");
    const updated = A.agent_set(original, "x", 42);
    if (A.agent_get(original, "x") !== null)
        throw new Error("Original mutated");
    if (A.agent_get(updated, "x") !== 42)
        throw new Error("Update not applied");
});
test("agent_update multi-key", () => {
    let agent = A.agent_create("test");
    agent = A.agent_update(agent, { goal: "search", phase: 15, active: true });
    if (A.agent_get(agent, "goal") !== "search")
        throw new Error("goal missing");
    if (A.agent_get(agent, "phase") !== 15)
        throw new Error("phase missing");
    if (A.agent_get(agent, "active") !== true)
        throw new Error("active missing");
});
test("agent_get missing returns null", () => {
    const agent = A.agent_create("test");
    if (A.agent_get(agent, "nonexistent") !== null)
        throw new Error("Expected null");
});
// ── Tool Registry ─────────────────────────────────────────────
test("agent_add_tool / agent_call_tool", () => {
    let agent = A.agent_create("tool-test");
    agent = A.agent_add_tool(agent, "double", (x) => x * 2);
    agent = A.agent_add_tool(agent, "greet", (name) => `hello ${name}`);
    if (A.agent_call_tool(agent, "double", 5) !== 10)
        throw new Error("double failed");
    if (A.agent_call_tool(agent, "greet", "ai") !== "hello ai")
...
test("agent_tools lists all tools", () => {
    ...
    if (!tools.includes("a") || !tools.includes("b"))
        throw new Error(`Expected [a,b], got ${tools}`);
});
test("agent_call_tool unknown throws", () => {
    const agent = A.agent_create("test");
    let threw = false;
    try {
        A.agent_call_tool(agent, "nonexistent");
    }
    catch {
        threw = true;
    }
...
test("agent_push_history / agent_history", () => {
    ...
    if (history[0].type !== "message")
        throw new Error("Wrong type");
    if (typeof history[0].timestamp !== "number")
...
test("agent_history_type filter", () => {
    ...
    if (calls.length !== 2)
        throw new Error(`Expected 2 tool_calls, got ${calls.length}`);
});
// ── Loop Execution ────────────────────────────────────────────
test("agent_loop terminates on goal", () => {
    let agent = A.agent_create("counter");
    agent = A.agent_set(agent, "count", 0);
    const result = A.agent_loop(agent, (state) => state.count >= 5, // goal: count reaches 5
    (a) => A.agent_set(a, "count", A.agent_get(a, "count") + 1), // step: increment
    20 // max steps
    );
    if (result.status !== "done")
        throw new Error(`Expected done, got ${result.status}`);
    if (A.agent_get(result, "count") !== 5)
        throw new Error(`Expected count=5, got ${A.agent_get(result, "count")}`);
});
test("agent_loop max_steps limit", () => {
    const agent = A.agent_create("infinite");
    const result = A.agent_loop(agent, (_) => false, // goal never met
    (a) => a, // no-op step
    5 // max 5 steps
    );
    if (result.status !== "max_steps")
        throw new Error(`Expected max_steps, got ${result.status}`);
    if (result.steps !== 5)
        throw new Error(`Expected 5 steps, got ${result.steps}`);
});
test("agent_loop error handling", () => {
    const agent = A.agent_create("error-test");
    const result = A.agent_loop(agent, (_) => false, (_) => { throw new Error("step failed"); }, 10);
    if (result.status !== "error")
        throw new Error(`Expected error, got ${result.status}`);
    if (!A.agent_get(result, "_error"))
        throw new Error("Error not captured in state");
});
test("agent_done reflects terminal status", () => {
    const agent = A.agent_create("test");
    const done = A.agent_loop(agent, (_) => true, (a) => a, 5);
    if (!A.agent_done(done))
        throw new Error("Should be done");
    if (A.agent_done(agent))
        throw new Error("Fresh agent should not be done");
});
test("agent_run_until simple", () => {
    const result = A.agent_run_until(0, (n) => n >= 10, (n) => n + 3, 20);
    if (result !== 12)
        throw new Error(`Expected 12, got ${result}`); // 0→3→6→9→12
});
// ── Plan Tracking ─────────────────────────────────────────────
test("plan_create / plan_next / plan_advance", () => {
    let plan = A.plan_create(["search", "analyze", "decide", "act"]);
    if (A.plan_next(plan) !== "search")
        throw new Error("Expected first step");
    plan = A.plan_advance(plan, { found: 3 });
    if (A.plan_next(plan) !== "analyze")
        throw new Error("Expected second step");
    if (A.plan_done(plan))
        throw new Error("Should not be done yet");
});
test("plan completes when all steps done", () => {
    ...
    if (!A.plan_done(plan))
        throw new Error("Should be done");
    if (A.plan_next(plan) !== null)
        throw new Error("Next should be null when done");
});
test("plan_progress", () => {
    let plan = A.plan_create(["a", "b", "c", "d"]);
    if (A.plan_progress(plan) !== 0)
        throw new Error("Expected 0 progress");
    plan = A.plan_advance(plan, "r");
    plan = A.plan_advance(plan, "r");
    if (A.plan_progress(plan) !== 0.5)
        throw new Error(`Expected 0.5, got ${A.plan_progress(plan)}`);
});
test("plan_results", () => {
    ...
    if (!results["search"] || results["search"].hits !== 5)
        throw new Error("search result missing");
    if (!results["analyze"] || results["analyze"].score !== 0.9)
        throw new Error("analyze result missing");
});
// ── Observation / Context ─────────────────────────────────────
...
test("summarize", () => {
    ...
    if (!summary.includes("goal: search"))
        throw new Error("goal missing from summary");
    if (!summary.includes("step: 3"))
        throw new Error("step missing from summary");
});
test("context_merge", () => {
    const a = { x: 1, y: 0 };
    const b = { y: 99, z: 3 };
...
test("full agent: search → analyze → decide pipeline", () => {
    ...
    agent = A.agent_add_tool(agent, "filter_high", (items) => items.filter((x) => x.score >= 0.7));
    agent = A.agent_add_tool(agent, "pick_best", (items) => C.arr_sort_by_desc(items, "score")[0]);
    // Initialize plan
    ...
    const result = A.agent_loop(agent, (state) => A.plan_done(state.plan), (a) => {
        const currentPlan = A.agent_get(a, "plan");
    ...
    if (!decision || decision.id !== 1)
        throw new Error(`Expected best item (id=1), got ${JSON.stringify(decision)}`);
    if (result.steps !== 3)
