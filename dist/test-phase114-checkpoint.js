...
test("1. CheckpointManager 생성", () => {
    cm = new checkpoint_1.CheckpointManager();
...
test("15. globalCheckpoint 싱글톤", () => {
    checkpoint_1.globalCheckpoint.clear();
    checkpoint_1.globalCheckpoint.save("singleton-test", { hello: "world" });
    const r = checkpoint_1.globalCheckpoint.restore("singleton-test");
...
test("16. cp-save 내장함수", () => {
    checkpoint_1.globalCheckpoint.clear();
    run(`(cp-save "s1" 100)`);
    const v = checkpoint_1.globalCheckpoint.restore("s1");
    assert(v === 100);
});
// 17. cp-restore 내장함수
...
test("20. cp-drop 내장함수", () => {
    ...
    assert(checkpoint_1.globalCheckpoint.versions("dropme") === 1);
    const ok = run(`(cp-drop "dropme")`);
    assert(ok === true);
    assert(checkpoint_1.globalCheckpoint.restore("dropme") === null);
});
// 21. cp-list 내장함수
...
//# sourceMappingURL=test-phase114-checkpoint.js.map