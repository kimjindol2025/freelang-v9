...
// 성공 step 팩토리
function successStep(name, delayMs = 0) {
    return {
        name,
        run: async () => {
            if (delayMs > 0)
...
// 실패 step 팩토리
function failStep(name, delayMs = 0) {
    return {
        name,
        run: async () => {
            if (delayMs > 0)
...
test("TC-1: CIPipeline 생성 가능", () => {
    const pipeline = new ci_runner_1.CIPipeline();
    assert(pipeline instanceof ci_runner_1.CIPipeline, "CIPipeline 인스턴스여야 함");
});
...
async function runBasicTests() {
    ...
    await testAsync("TC-7: 첫 번째 step 실패 → 두 번째 skip (fail-fast 기본)", async () => {
        const pipeline = new ci_runner_1.CIPipeline({ failFast: true });
    ...
    await testAsync("TC-8: fail-fast 비활성화 → 모든 step 실행", async () => {
        const pipeline = new ci_runner_1.CIPipeline({ failFast: false });
        pipeline.addStep(failStep("step1")).addStep(successStep("step2"));
        const summary = await pipeline.run();
        assert(summary.passed === false, "passed:false여야 함");
...
//# sourceMappingURL=test-phase80-ci.js.map