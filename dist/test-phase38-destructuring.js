...
(export swap)`;
    ...
    if (!js.includes("[a, b]") && !js.includes("const [a") && !js.includes("[b,"))
        throw new Error(`no destructuring in:\n${js.slice(0, 200)}`);
    const m = makeModule(js);
    const result = m.swap([1, 2]);
    if (!Array.isArray(result) || result[0] !== 2 || result[1] !== 1)
...
(export sum3)`;
    ...
    if (m.sum3([10, 20, 30]) !== 60)
        throw new Error(`got ${m.sum3([10, 20, 30])}`);
});
test("구조 분해 + 나머지 rest", () => {
    const src = `
...
(export cross-add)`;
    const m = makeModule(standaloneCompile1(src));
    if (m["cross-add"]([1, 2], [3, 4]) !== 10)
        throw new Error(`got ${m["cross-add"]([1, 2], [3, 4])}`);
});
...
(export mixed)`;
    const m = makeModule(standaloneCompile1(src));
    if (m.mixed([3, 4], 5) !== 35)
        throw new Error(`got ${m.mixed([3, 4], 5)}`);
});
// ── 컴파일러 소스 자체 컴파일 ──────────────────────────────────
console.log("\n[38-C] 컴파일러 자체 컴파일");
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase38-destructuring.js.map