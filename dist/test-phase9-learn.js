...
try {
    ...
    const tokens = (0, lexer_1.lex)(learnCode);
    const hasLearn = tokens.some((t) => t.type === "Learn");
    if (hasLearn) {
        console.log(`✅ TEST 1 PASS: Lexer recognized "learn" keyword (${tokens.length} total tokens)`);
...
try {
    ...
    if (ast.length > 0 && ast[0].kind === "learn-block") {
        const learnBlock = ast[0];
        console.log(`✅ TEST 2 PASS: Parser created LearnBlock node`);
        console.log(`   Key: "${learnBlock.key}", Data: ${JSON.stringify(learnBlock.data)}`);
    }
    else {
        console.log(`❌ TEST 2 FAIL: Expected learn-block, got ${ast[0]?.kind || "unknown"}`);
    }
}
catch (e) {
...
try {
    ...
    const tokens = (0, lexer_1.lex)(recallCode);
    const ast = (0, parser_1.parse)(tokens);
    if (ast.length > 0 && ast[0].kind === "learn-block") {
        const learnBlock = ast[0];
        console.log(`✅ TEST 3 PASS: Parser created LearnBlock for recall`);
        console.log(`   Key: "${learnBlock.key}", Data: ${learnBlock.data}`);
    }
    else {
...
`;
    ...
    if (context.learned && context.learned.size >= 2) {
        ...
        console.log(`⚠️  TEST 6 PARTIAL: Some operations succeeded but not all verified`);
...
try {
    ...
    const tokens = (0, lexer_1.lex)(rememberCode);
    const ast = (0, parser_1.parse)(tokens);
    const interpreter = new interpreter_1.Interpreter();
    const context = interpreter.interpret(ast);
...
//# sourceMappingURL=test-phase9-learn.js.map