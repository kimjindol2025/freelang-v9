...
test('1. 기본 생성', () => {
    const s = new self_improve_1.SelfImprover({
        target: 0,
        evaluate: (v) => v,
        improve: (v) => ({ value: v + 0.1, improvement: '+0.1' })
    });
...
test('3. run() 후 done=true', () => {
    const s = new self_improve_1.SelfImprover({
        target: 0,
        evaluate: (v) => v,
        improve: (v) => ({ value: v + 0.5, improvement: '+0.5' }),
        maxIterations: 3
    });
    const result = s.run();
...
test('6. minImprovement 미달 시 종료', () => {
    const s = new self_improve_1.SelfImprover({
        target: 0.5,
        evaluate: (v) => v,
        improve: (v) => ({ value: v + 0.001, improvement: 'tiny' }),
        minImprovement: 0.01,
        maxIterations: 10
    });
    const result = s.run();
...
test('7. 점수 개선 시 current 업데이트', () => {
    const s = new self_improve_1.SelfImprover({
        target: 0,
        evaluate: (v) => v,
        improve: (v) => ({ value: v + 0.3, improvement: '+0.3' }),
        maxIterations: 1
    });
    s.step();
    assertClose(s.getState().current, 0.3, 0.0001, 'current');
});
...
test('8. 점수 하락 시 current 유지', () => {
    const s = new self_improve_1.SelfImprover({
        target: 0.8,
        evaluate: (v) => v,
        improve: (_v) => ({ value: 0.5, improvement: 'worse' }),
        maxIterations: 1
    });
    s.step();
    assertClose(s.getState().current, 0.8, 0.0001, 'current should stay 0.8');
});
// 9. history 기록 확인
test('9. history에 각 iteration 기록됨', () => {
    const s = new self_improve_1.SelfImprover({
        target: 0,
        evaluate: (v) => v,
        improve: (v) => ({ value: v + 0.2, improvement: '+0.2' }),
        maxIterations: 3
    });
    s.run();
...
test('14. done 후 step() 호출해도 state 변화 없음', () => {
    const s = new self_improve_1.SelfImprover({
        target: 0,
        evaluate: (v) => v,
        improve: (v) => ({ value: v + 0.5, improvement: 'x' }),
        maxIterations: 1
    });
    s.run();
...
test('15. history[0]는 initial 항목', () => {
    const s = new self_improve_1.SelfImprover({
        target: 42,
        evaluate: (_v) => 0.5,
        improve: (v) => ({ value: v + 1, improvement: '+1' })
    });
...
test('17. getState() 현재 상태 반환', () => {
    const s = new self_improve_1.SelfImprover({
        target: 100,
        evaluate: (v) => v / 100,
        improve: (v) => ({ value: v + 10, improvement: '+10' }),
        maxIterations: 2
    });
    s.step();
...
test('18. createSelfImprover() 팩토리 함수', () => {
    const s = (0, self_improve_1.createSelfImprover)({
        target: 0,
        evaluate: (v) => v,
        improve: (v) => ({ value: v + 0.5, improvement: '+0.5' }),
        maxIterations: 2
    });
    assert(s instanceof self_improve_1.SelfImprover, 'should return SelfImprover instance');
    const result = s.run();
...
test('21. improve 함수에 history 전달됨', () => {
    ...
    assert(receivedHistory.length >= 1, 'history should have at least 1 entry');
});
...
test('22. improvement 문자열이 history에 기록됨', () => {
    const s = new self_improve_1.SelfImprover({
        target: 0,
        evaluate: (v) => v,
        improve: (v) => ({ value: v + 0.3, improvement: 'custom-improvement-tag' }),
        maxIterations: 2
    });
    s.step();
...
test('24. minImprovement=0이면 개선 없을 때 즉시 종료', () => {
    const s = new self_improve_1.SelfImprover({
        target: 0.5,
        evaluate: (_v) => 0.5, // 항상 동일 점수
        improve: (v) => ({ value: v, improvement: 'no-change' }),
        minImprovement: 0,
        maxIterations: 10
    });
    const result = s.run();
...
//# sourceMappingURL=test-phase99-self-improve.js.map