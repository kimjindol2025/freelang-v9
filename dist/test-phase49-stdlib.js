...
function makeModule(jsCode, extra = {}) {
...
// ── Gen1 빌드 ─────────────────────────────────────────────────
console.log("\n[49-A] Gen1 빌드");
let lexerMod1, parserMod1, codegenMod1, interpMod1, stdlibMod1;
test("lexer.fl → Gen1", () => {
...
test("stdlib.fl → Gen1 컴파일", () => {
    ...
    if (typeof stdlibMod1["fl-map"] !== "function")
        throw new Error("fl-map not exported");
...
(fl-map [1 2 3] double)`;
    ...
    if (!Array.isArray(result))
        throw new Error(`not array: ${result}`);
...
test("fl-zip [1 2 3] [a b c] → pairs", () => {
    ...
    if (result[0][0] !== 1 || result[0][1] !== "a")
        throw new Error(`got ${JSON.stringify(result[0])}`);
});
test("fl-range 0 5 → [0 1 2 3 4]", () => {
    ...
    if (result.length !== 5 || result[0] !== 0 || result[4] !== 4)
        throw new Error(`got ${JSON.stringify(result)}`);
});
// ── Maybe ──────────────────────────────────────────────────────
console.log("\n[49-C] Maybe/Option");
...
function standaloneCompile2(src) {
    const interp = new interpreter_1.Interpreter();
    for (const s of [lexerSrc, parserSrc, codegenSrc])
        interp.interpret((0, parser_1.parse)((0, lexer_1.lex)(s)));
...
test("Gen2===Gen3 stdlib 고정점", () => {
    if (stdlib2JS !== stdlib3JS)
        throw new Error(`diff: Gen2 ${stdlib2JS.length} vs Gen3 ${stdlib3JS.length}`);
    console.log(`    → 고정점 확인 ✓`);
});
// ── 결과 ─────────────────────────────────────────────────────
console.log(`\n${"─".repeat(50)}`);
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase49-stdlib.js.map