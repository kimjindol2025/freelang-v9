...
const fitnessId = (x) => x; // 숫자 자체가 적합도
const nextGenKeepBest = (pop, fits) => {
    ...
    while (result.length < pop.length)
        result.push(elites[result.length % half] + Math.random() * 0.01);
    return result;
};
// ─── 1. GenerationLoop 생성 ───────────────────────────────────────────────────
test("1. GenerationLoop 클래스 인스턴스 생성", () => {
    const loop = new generation_1.GenerationLoop({ maxGenerations: 10 });
    assert(loop instanceof generation_1.GenerationLoop, "GenerationLoop 생성 실패");
});
// ─── 2. 간단한 진화 루프 실행 (숫자 최대화) ──────────────────────────────────
test("2. 간단한 숫자 최대화 진화 루프 실행", () => {
    const loop = new generation_1.GenerationLoop({ maxGenerations: 5 });
...
test("4. targetFitness 달성 시 조기 종료", () => {
    const loop = new generation_1.GenerationLoop({
        maxGenerations: 100,
        targetFitness: 5,
    });
...
test("5. stagnation 정지 (개선 없이 멈춤)", () => {
    ...
    const flatFitness = (x) => x;
    const flatNext = (pop) => [...pop]; // 변화 없음
    const loop = new generation_1.GenerationLoop({
        maxGenerations: 100,
        stagnationLimit: 3,
    });
...
test("6. GenerationStats 구조 검증", () => {
    const loop = new generation_1.GenerationLoop({ maxGenerations: 3 });
    const result = loop.run(simplePopulation, fitnessId, nextGenKeepBest);
...
test("8. history 길이 == totalGenerations", () => {
    const loop = new generation_1.GenerationLoop({ maxGenerations: 8 });
    const result = loop.run(simplePopulation, fitnessId, nextGenKeepBest);
    assert(result.history.length === result.totalGenerations, `history 길이 ${result.history.length} != totalGenerations ${result.totalGenerations}`);
});
// ─── 9. best 항상 최고 적합도 ────────────────────────────────────────────────
...
test("10. diversity 계산 (0~1 범위)", () => {
    const loop = new generation_1.GenerationLoop({ maxGenerations: 1 });
...
test("15. generation-run 빌트인", () => {
    ...
    assert(ctx.lastValue.has("best"), "best 필드 없음");
});
...
test("16. generation-stats 빌트인", () => {
    ...
    assert(ctx.lastValue.length > 0, "stats 배열이 비어 있음");
});
...
test("18. generation-history 빌트인", () => {
    ...
    assert(ctx.lastValue.length > 0, "history 배열이 비어 있음");
});
...
test("21. gen-improvement 빌트인", () => {
    ...
    assert(ctx.lastValue >= 0, "improvementRatio < 0");
});
...
test("22. gen-termination 빌트인", () => {
    ...
    assert(valid.includes(ctx.lastValue), `terminationReason 잘못됨: ${ctx.lastValue}`);
});
...
test("25. elites 계산 (상위 10%)", () => {
    ...
    assert(result.history[0].elites === expectedElites, `elites ${result.history[0].elites} != expected ${expectedElites}`);
});
// ─── 26. runGeneration 편의 함수 ─────────────────────────────────────────────
test("26. runGeneration 편의 함수 동작", () => {
    const result = (0, generation_1.runGeneration)([1, 2, 3, 4, 5], fitnessId, nextGenKeepBest, 5);
...
//# sourceMappingURL=test-phase135-generation.js.map