...
// ─── Chunk 빌더 헬퍼 ─────────────────────────────────────────────────────────
function makeChunk(instructions, constants = [], name = "test") {
...
test("TC-1: Optimizer 생성", () => {
    const opt = new optimizer_1.Optimizer();
    assert(opt instanceof optimizer_1.Optimizer, "Optimizer 인스턴스 생성 실패");
});
// TC-2: addPass 체이닝
test("TC-2: addPass 체이닝", () => {
    const opt = new optimizer_1.Optimizer();
    const result = opt.addPass(optimizer_1.constantFoldingPass).addPass(optimizer_1.deadCodeEliminationPass);
    assert(result === opt, "addPass가 this를 반환해야 함");
});
// TC-3: 패스 없으면 Chunk 그대로 반환
test("TC-3: 패스 없으면 Chunk 그대로 반환", () => {
    const opt = new optimizer_1.Optimizer();
    const chunk = makeChunk([{ op: bytecode_1.OpCode.PUSH_CONST, arg: 0 }, { op: bytecode_1.OpCode.HALT }], [42]);
...
test("TC-4: constant-folding — 2 + 3 → PUSH_CONST 5", () => {
    const chunk = makeChunk([
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 0 },
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 1 },
        { op: bytecode_1.OpCode.ADD },
        { op: bytecode_1.OpCode.HALT },
    ], [2, 3]);
    const result = optimizer_1.constantFoldingPass.run(chunk);
    // PUSH_CONST 5, HALT → 2개 명령어
    assertEqual(result.instructions.length, 2, `명령어 수: ${result.instructions.length}`);
    assertEqual(result.instructions[0].op, bytecode_1.OpCode.PUSH_CONST, "PUSH_CONST 확인");
    const foldedVal = result.constants[result.instructions[0].arg];
...
test("TC-8: dead-code-elimination — HALT 이후 명령어 제거", () => {
    const chunk = makeChunk([
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 0 },
        { op: bytecode_1.OpCode.HALT },
        { op: bytecode_1.OpCode.ADD }, // unreachable
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 1 }, // unreachable
    ], [10, 20]);
    const result = optimizer_1.deadCodeEliminationPass.run(chunk);
    assertEqual(result.instructions.length, 2, `명령어 수: ${result.instructions.length}`);
    assertEqual(result.instructions[1].op, bytecode_1.OpCode.HALT, "마지막 명령어는 HALT");
});
...
test("TC-10: push-pop-elimination — PUSH+POP 쌍 제거", () => {
    const chunk = makeChunk([
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 0 },
        { op: bytecode_1.OpCode.POP },
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 1 },
        { op: bytecode_1.OpCode.HALT },
    ], [99, 42]);
    const result = optimizer_1.pushPopEliminationPass.run(chunk);
    // PUSH_CONST 99, POP 제거 → PUSH_CONST 42, HALT 남음
    assertEqual(result.instructions.length, 2, `명령어 수: ${result.instructions.length}`);
    assertEqual(result.instructions[0].op, bytecode_1.OpCode.PUSH_CONST, "첫 명령어 PUSH_CONST");
    const val = result.constants[result.instructions[0].arg];
...
test("TC-11: push-pop-elimination — PUSH 없이 POP은 건드리지 않음", () => {
    const chunk = makeChunk([
        { op: bytecode_1.OpCode.PUSH_VAR, arg: "x" },
        { op: bytecode_1.OpCode.POP },
        { op: bytecode_1.OpCode.HALT },
    ], []);
    const result = optimizer_1.pushPopEliminationPass.run(chunk);
...
test("TC-12: jump-optimization — JUMP to next 제거", () => {
    ...
    const chunk = makeChunk([
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 0 },
        { op: bytecode_1.OpCode.JUMP, arg: 2 },
        { op: bytecode_1.OpCode.HALT },
    ], [5]);
    const result = optimizer_1.jumpOptimizationPass.run(chunk);
    assertEqual(result.instructions.length, 2, `명령어 수: ${result.instructions.length}`);
    // JUMP가 제거되어야 함
    assert(result.instructions.every(i => i.op !== bytecode_1.OpCode.JUMP), "JUMP 명령어가 남아있음");
});
...
test("TC-13: jump-optimization — 다른 JUMP는 건드리지 않음", () => {
    ...
    const result = optimizer_1.jumpOptimizationPass.run(chunk);
    assertEqual(result.instructions.length, 3, `명령어 수: ${result.instructions.length}`);
    assertEqual(result.instructions[0].op, bytecode_1.OpCode.JUMP, "JUMP 유지");
});
// TC-14: createDefaultOptimizer — 4개 패스 포함
test("TC-14: createDefaultOptimizer 4개 패스 포함", () => {
    const opt = (0, optimizer_1.createDefaultOptimizer)();
    // 간단한 최적화 적용 → stats에 4개 항목
    const chunk = makeChunk([{ op: bytecode_1.OpCode.HALT }], []);
    opt.optimize(chunk);
...
test("TC-15: getStats — passName 목록 포함", () => {
    const opt = new optimizer_1.Optimizer()
        .addPass(optimizer_1.constantFoldingPass)
        .addPass(optimizer_1.deadCodeEliminationPass);
    const chunk = makeChunk([
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 0 },
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 1 },
        { op: bytecode_1.OpCode.ADD },
        { op: bytecode_1.OpCode.HALT },
        { op: bytecode_1.OpCode.POP }, // dead code
    ], [1, 2]);
    opt.optimize(chunk);
    const stats = opt.getStats();
...
test("TC-16: optimize 후 명령어 수 감소 확인", () => {
    ...
    const result = opt.optimize(chunk);
    assert(result.instructions.length < original, `최적화 후 명령어가 줄어야 함 (${original} → ${result.instructions.length})`);
});
// TC-17: 빈 Chunk 최적화
...
test("TC-18: 중첩 최적화 — 2개 패스 연속", () => {
    ...
    const opt = new optimizer_1.Optimizer()
        .addPass(optimizer_1.constantFoldingPass)
        .addPass(optimizer_1.pushPopEliminationPass);
    const chunk = makeChunk([
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 0 }, // 2
        { op: bytecode_1.OpCode.PUSH_CONST, arg: 1 }, // 3
        { op: bytecode_1.OpCode.ADD },
        { op: bytecode_1.OpCode.POP },
    ], [2, 3]);
    const result = opt.optimize(chunk);
...
test("TC-20: Phase 56 regression — Optimizer가 기존 모듈 import를 망가뜨리지 않음", () => {
    ...
    assert(typeof optimizer_1.Optimizer === "function", "Optimizer 클래스 사용 가능");
    assert(typeof optimizer_1.createDefaultOptimizer === "function", "createDefaultOptimizer 함수 사용 가능");
    assert(typeof optimizer_1.constantFoldingPass === "object", "constantFoldingPass 사용 가능");
    assert(typeof optimizer_1.deadCodeEliminationPass === "object", "deadCodeEliminationPass 사용 가능");
    assert(typeof optimizer_1.pushPopEliminationPass === "object", "pushPopEliminationPass 사용 가능");
    assert(typeof optimizer_1.jumpOptimizationPass === "object", "jumpOptimizationPass 사용 가능");
});
// ─── 추가 테스트 (21+) ────────────────────────────────────────────────────────
...
test("TC-23: getStats — reduced 값 정확", () => {
    const opt = new optimizer_1.Optimizer().addPass(optimizer_1.deadCodeEliminationPass);
    const chunk = makeChunk([
        { op: bytecode_1.OpCode.HALT },
        { op: bytecode_1.OpCode.POP },
        { op: bytecode_1.OpCode.POP },
...
//# sourceMappingURL=test-phase84-optimizer.js.map