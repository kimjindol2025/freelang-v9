...
`;
try {
    const tokens1 = (0, lexer_1.lex)(lexerDefinitionCode);
    console.log(`✅ Lexer tokenized lexer definition code: ${tokens1.length} tokens`);
    console.log(`   Sample tokens: ${tokens1
        .slice(0, 5)
...
`;
try {
    const tokens2 = (0, lexer_1.lex)(parserDefinitionCode);
    const ast2 = (0, parser_1.parse)(tokens2);
    console.log(`✅ Parser parsed parser definition code: ${ast2.length} top-level expressions`);
    console.log(`   First node kind: ${ast2[0]?.kind || "unknown"}, op: ${ast2[0]?.op || "N/A"}`);
}
catch (e) {
...
`;
try {
    const tokens3 = (0, lexer_1.lex)(interpreterDefinitionCode);
    const ast3 = (0, parser_1.parse)(tokens3);
    console.log(`✅ Parser parsed interpreter definition code: ${ast3.length} top-level expressions`);
    const defineNodes = ast3.filter((n) => n.op === "define");
    console.log(`   Found ${defineNodes.length} define statements`);
    console.log(`   Functions defined: ${defineNodes.map((n) => n.args?.[0]?.value || "unknown").join(", ")}`);
}
catch (e) {
    console.log(`❌ Parser failed: ${e.message}`);
}
...
`;
try {
    const tokens4 = (0, lexer_1.lex)(completeProgram);
    ...
    // Analyze AST
    const modules = ast4.filter((n) => n.kind === "block" && n.type === "MODULE");
    const imports = ast4.filter((n) => n.op === "import");
    const defines = ast4.filter((n) => n.op === "define");
    const pipes = ast4.filter((n) => n.op === "pipe");
...
`;
try {
    const tokens5 = (0, lexer_1.lex)(advancedCode);
    ...
    const matchPatterns = ast5.filter((n) => {
        // Check if this define contains match expressions
        const body = n.args?.[1];
        return body && typeof body === "object" && body.op === "match";
    });
...
`;
try {
    const tokens6 = (0, lexer_1.lex)(typeSystemCode);
    ...
    const typeClassDefs = ast6.filter((n) => n.kind === "block" && n.type === "TYPECLASS");
    const instanceDefs = ast6.filter((n) => n.kind === "block" && n.type === "INSTANCE");
...
//# sourceMappingURL=test-bootstrap-verification.js.map