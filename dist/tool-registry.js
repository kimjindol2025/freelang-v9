...
class ToolRegistry {
    ...
    async execute(name, args) {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Tool timeout: ${name} (${timeout}ms)`)), timeout));
...
// ── 기본 내장 도구 등록 ──────────────────────────────────────────────────────
function makeSafeMathEval(expr) {
...
// math — 수학 계산
exports.globalToolRegistry.register({
    name: 'math',
