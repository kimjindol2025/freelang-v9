...
function assertEqual(a, b, msg) {
    ...
    if (!same)
        throw new Error(msg ?? `기대값: ${JSON.stringify(b)}, 실제: ${JSON.stringify(a)}`);
}
// ─── 컴파일+실행 단축 함수 ─────────────────────────────────────────────────
function run(node) {
...
test("define + 변수 참조", () => {
    const compiler = new compiler_1.BytecodeCompiler();
    const chunk = compiler.compile(sexpr("do", sexpr("define", varNode("x"), lit(99)), varNode("x")));
    const vm = new vm_1.VM();
...
test("define $n 10 → $n = 10", () => {
    const compiler = new compiler_1.BytecodeCompiler();
    const chunk = compiler.compile(sexpr("do", sexpr("define", varNode("n"), lit(10)), varNode("n")));
    const vm = new vm_1.VM();
    const result = vm.run(chunk);
...
test("OpCode 열거형 주요 값 존재", () => {
    assert(bytecode_1.OpCode.PUSH_CONST !== undefined, "PUSH_CONST 없음");
    assert(bytecode_1.OpCode.PUSH_VAR !== undefined, "PUSH_VAR 없음");
    assert(bytecode_1.OpCode.SET_VAR !== undefined, "SET_VAR 없음");
    assert(bytecode_1.OpCode.ADD !== undefined, "ADD 없음");
    assert(bytecode_1.OpCode.HALT !== undefined, "HALT 없음");
    assert(bytecode_1.OpCode.JUMP !== undefined, "JUMP 없음");
    assert(bytecode_1.OpCode.JUMP_IF_FALSE !== undefined, "JUMP_IF_FALSE 없음");
    assert(bytecode_1.OpCode.MAKE_LIST !== undefined, "MAKE_LIST 없음");
});
...
test("define $a 5, define $b 3, (+ $a $b) = 8", () => {
    const compiler = new compiler_1.BytecodeCompiler();
    const chunk = compiler.compile(sexpr("do", sexpr("define", varNode("a"), lit(5)), sexpr("define", varNode("b"), lit(3)), sexpr("+", varNode("a"), varNode("b"))));
    const vm = new vm_1.VM();
...
test("DUP: 스택 최상단 복제", () => {
    const chunk = {
        ...
        constants: [7],
        instructions: [
            { op: bytecode_1.OpCode.PUSH_CONST, arg: 0 }, // 스택: [7]
            { op: bytecode_1.OpCode.DUP }, // 스택: [7, 7]
            { op: bytecode_1.OpCode.ADD }, // 스택: [14]
            { op: bytecode_1.OpCode.HALT },
        ],
    };
    const vm = new vm_1.VM();
...
test("POP: 스택에서 값 제거 후 다음 값 반환", () => {
    const chunk = {
        ...
        constants: [10, 20],
        instructions: [
            { op: bytecode_1.OpCode.PUSH_CONST, arg: 0 }, // 스택: [10]
            { op: bytecode_1.OpCode.PUSH_CONST, arg: 1 }, // 스택: [10, 20]
            { op: bytecode_1.OpCode.POP }, // 스택: [10]
            { op: bytecode_1.OpCode.HALT },
        ],
    };
    const vm = new vm_1.VM();
...
test("Phase 56 렉시컬 스코프 regression 실행", () => {
    ...
    const output = execSync("npx ts-node src/test-phase56-lexical-scope.ts", { cwd: "/home/kimjin/kim/Desktop/kim/01_Active_Projects/freelang-v9", encoding: "utf8" });
    const match = output.match(/(\d+) passed, (\d+) failed/);
    if (!match)
        throw new Error("Phase 56 테스트 결과를 파싱할 수 없음");
    const pass = parseInt(match[1], 10);
    const fail = parseInt(match[2], 10);
    if (fail > 0)
        throw new Error(`Phase 56 regression: ${fail}개 실패`);
    if (pass < 14)
        throw new Error(`Phase 56 regression: ${pass}/14 PASS`);
});
// ─── 결과 요약 ─────────────────────────────────────────────────────────────
console.log("\n──────────────────────────────────────────────────");
...
//# sourceMappingURL=test-phase83-vm.js.map