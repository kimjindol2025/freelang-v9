...
exports.handleReasoningSequence = handleReasoningSequence;
function handleReasoningSequence(interp, reasoningSeq) {
    ...
    if (!ctx.reasoning)
        ctx.reasoning = new Map();
    const sequenceId = new Date().getTime();
    const executionPath = [];
    const sequenceResults = [];
    const iterationHistory = [];
    logger.info(`🔄 REASONING SEQUENCE START (${stages.length} stages, feedback: ${feedbackLoop?.enabled ? "enabled" : "disabled"})`);
    ...
    while (iteration < maxIterations) {
        iteration++;
        if (iteration > 1) {
            logger.info(`🔄 FEEDBACK LOOP ITERATION ${iteration}/${maxIterations} (damping: ${(feedbackLoop?.confidenceDamping || 0.1).toFixed(1)})`);
        }
                ...
                for (const transition of reasoningBlock.transitions) {
                    if (transition.condition) {
                        ...
                        if (conditionMet && transition.to)
                            logger.info(`    ↓ Transition to: ${transition.to}`);
                    }
                }
            }
        }
...
//# sourceMappingURL=eval-reasoning-sequence.js.map