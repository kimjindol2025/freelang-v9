...
test("(+ 1 2) → 3 리터럴", () => {
    ...
    if (!js.includes("3"))
        throw new Error(`no folded 3: ${js}`);
    if (js.includes("1 + 2") || js.includes("(1) + (2)"))
        throw new Error(`not folded: ${js}`);
    console.log(`    → ${js.replace(/\n/g, ' ').slice(40, 120)}`);
    if (makeModule(js).f() !== 3)
...
test("(concat \"hello\" \" \" \"world\") → 리터럴", () => {
    ...
    const js = standaloneCompile1(src);
    if (!js.includes('"hello world"'))
        throw new Error(`not folded: ${js}`);
    if (makeModule(js).f() !== "hello world")
        throw new Error("wrong result");
});
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase40-const-fold.js.map