...
test("TC-1: 생성 (maxTokens=4096)", () => {
    const cm = new context_window_1.ContextManager(4096);
    const s = cm.stats();
...
test("TC-2: add → id 반환", () => {
    const cm = new context_window_1.ContextManager();
...
test("TC-5: stats → { used, max, percent, count }", () => {
    const cm = new context_window_1.ContextManager(1000);
    cm.add("abc");
    const s = cm.stats();
...
test("TC-10: trim → 오래된 항목 제거 (초과 시)", () => {
    ...
    const cm = new context_window_1.ContextManager(10);
...
test("TC-18: 빈 컨텍스트 trim → 변화없음", () => {
    const cm = new context_window_1.ContextManager(100);
...
test("TC-19: 초과 add → trim 자동", () => {
    const cm = new context_window_1.ContextManager(20);
    // 각 항목 10 tokens
...
test("TC-20: priority=1.0 항목 trim에서 보존", () => {
    const cm = new context_window_1.ContextManager(15);
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-phase95-context.js.map