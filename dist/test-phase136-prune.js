...
test("1. Pruner 인스턴스 생성", () => {
    const p = new prune_1.Pruner();
    return p instanceof prune_1.Pruner;
});
...
test("7. PruneResult — kept/removed/keptRatio/stats 구조", () => {
    const p = new prune_1.Pruner();
    const result = p.pruneToTopK(nums, scorer, 3);
    return (Array.isArray(result.kept) &&
...
if (fail > 0)
    process.exit(1);
//# sourceMappingURL=test-phase136-prune.js.map