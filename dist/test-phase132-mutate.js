...
test("4. mutateString 문자열 변이 (반환 구조 확인)", () => {
    const m = new mutate_1.Mutator({ rate: 0.5 });
...
test("8. rate=0 → mutations=0", () => {
    const m = new mutate_1.Mutator({ rate: 0 });
    const r = m.mutateNumbers([1, 2, 3, 4, 5]);
...
test("10. strength 강도 적용 (큰 strength = 큰 변화)", () => {
    const m1 = new mutate_1.Mutator({ rate: 1.0, strength: 0.001 });
    const m2 = new mutate_1.Mutator({ rate: 1.0, strength: 100.0 });
...
test("19. mutate-select 빌트인", () => {
    const r = evalFL(`(mutate-select (list {:value 10 :fitness 0.9} {:value 20 :fitness 0.1} {:value 30 :fitness 0.5}) :n 1)`);
...
test("22. gaussian 타입 변이", () => {
    const m = new mutate_1.Mutator({ rate: 1.0, strength: 0.01, type: 'gaussian' });
...
test("23. insert 타입 — 문자열 길이 증가", () => {
    const m = new mutate_1.Mutator({ rate: 1.0, type: 'insert' });
    const r = m.mutateString("hello");
...
test("24. delete 타입 — 문자열 길이 감소 가능", () => {
    const m = new mutate_1.Mutator({ rate: 1.0, type: 'delete' });
    const r = m.mutateString("hello");
    // rate=1이면 모든 문자 삭제
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-phase132-mutate.js.map