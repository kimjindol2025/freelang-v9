...
test("TC-7: enter/exit 패턴 — exit 호출 시 ms 기록", (done) => {
    const p = new profiler_1.Profiler();
    p.enabled = true;
...
test("TC-15: FL 팩토리얼 실행 후 globalProfiler에 'fact' 항목 확인", () => {
    profiler_1.globalProfiler.reset();
    profiler_1.globalProfiler.enabled = true;
    const src = `
    [FUNC fact :params [$n]
  ...
  `;
    ...
    const report = profiler_1.globalProfiler.getReport();
    ...
    profiler_1.globalProfiler.reset();
    profiler_1.globalProfiler.enabled = false;
});
...
test("TC-20: Phase 56 regression — 렉시컬 스코프 격리 동작", () => {
  ...
  `;
    const interp = new interpreter_1.Interpreter();
    interp.interpret((0, parser_1.parse)((0, lexer_1.lex)(src2)));
    const xVal = interp.context.variables.get("$x");
...
//# sourceMappingURL=test-phase82-profiler.js.map