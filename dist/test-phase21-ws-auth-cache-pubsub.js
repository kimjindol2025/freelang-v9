...
const stdlib_ws_1 = require("./stdlib-ws");
const stdlib_auth_1 = require("./stdlib-auth");
const stdlib_cache_1 = require("./stdlib-cache");
const stdlib_pubsub_1 = require("./stdlib-pubsub");
let pass = 0;
let fail = 0;
...
console.log("══════════════════════════════════════════");
const A = (0, stdlib_auth_1.createAuthModule)();
// JWT sign + verify
...
console.log("══════════════════════════════════════════");
const C = (0, stdlib_cache_1.createCacheModule)();
// 기본 set/get
...
check("cache_ttl: 남은 시간 양수", C["cache_ttl"]("ttl1") > 0);
...
const PS = (0, stdlib_pubsub_1.createPubSubModule)(mockCallFn);
const subId = PS["pubsub_subscribe"]("user.login", "on_login");
...
check("pubsub_topics: 2개 토픽", PS["pubsub_topics"]().length === 2);
check("pubsub_subscribers: user.login 2개", PS["pubsub_subscribers"]("user.login") === 2);
// publish
const delivered = PS["pubsub_publish"]("user.login", { userId: "42" });
...
console.log("══════════════════════════════════════════");
const WS = (0, stdlib_ws_1.createWsModule)(mockCallFn);
...
// 연결 없는 상태
check("ws_clients: 초기 빈 배열", WS["ws_clients"]().length === 0);
...
if (fail > 0)
    process.exit(1);
//# sourceMappingURL=test-phase21-ws-auth-cache-pubsub.js.map