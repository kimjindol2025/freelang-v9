...
function hasFn(interp, name) {
    return interp.context.functions.has(name);
}
...
test("TC-1: fl-fmt.fl 파싱 성공", () => {
    ...
    if (!fmtSrc || fmtSrc.length === 0)
        throw new Error("빈 파일");
    fmtInterp = loadFl(fmtSrc);
});
test("TC-2: fl-fmt.fl — [MODULE fl-fmt] 블록 포함", () => {
    if (!fmtSrc.includes("[MODULE fl-fmt]"))
        throw new Error("[MODULE fl-fmt] 없음");
});
test("TC-3: fl-fmt.fl — indent 함수 정의됨", () => {
    if (!fmtSrc.includes("indent"))
        throw new Error("indent 없음");
    if (!fmtInterp)
        throw new Error("인터프리터 로드 실패");
    if (!hasFn(fmtInterp, "indent"))
        throw new Error("indent 함수 미등록");
});
test("TC-4: fl-fmt.fl — fmt-sexpr 함수 정의됨", () => {
    if (!fmtSrc.includes("fmt-sexpr"))
        throw new Error("fmt-sexpr 없음");
    if (!fmtInterp)
        throw new Error("인터프리터 로드 실패");
    if (!hasFn(fmtInterp, "fmt-sexpr"))
        throw new Error("fmt-sexpr 함수 미등록");
});
test("TC-5: fl-fmt.fl 로드 후 indent 함수 호출 가능", () => {
    ...
    if (result !== "    ")
        throw new Error(`indent(2) = ${JSON.stringify(result)}, 기대: "    "`);
});
// ──────────────────────────────────────────────────────────
...
test("TC-6: fl-lint.fl 파싱 성공", () => {
    ...
    if (!lintSrc || lintSrc.length === 0)
        throw new Error("빈 파일");
    lintInterp = loadFl(lintSrc);
});
test("TC-7: fl-lint.fl — [MODULE fl-lint] 블록 포함", () => {
    if (!lintSrc.includes("[MODULE fl-lint]"))
        throw new Error("[MODULE fl-lint] 없음");
});
test("TC-8: fl-lint.fl — check-empty-body 함수 정의됨", () => {
    if (!lintSrc.includes("check-empty-body"))
        throw new Error("check-empty-body 없음");
    if (!lintInterp)
        throw new Error("인터프리터 로드 실패");
    if (!hasFn(lintInterp, "check-empty-body"))
        throw new Error("check-empty-body 함수 미등록");
});
// ──────────────────────────────────────────────────────────
...
let testFlSrc = "";
let testFlInterp = null;
test("TC-9: fl-test.fl 파싱 성공", () => {
    testFlSrc = fs.readFileSync(path.join(flFilesDir, "fl-test.fl"), "utf-8");
    if (!testFlSrc || testFlSrc.length === 0)
        throw new Error("빈 파일");
    testFlInterp = loadFl(testFlSrc);
});
test("TC-10: fl-test.fl — [MODULE fl-test] 블록 포함", () => {
    if (!testFlSrc.includes("[MODULE fl-test]"))
        throw new Error("[MODULE fl-test] 없음");
});
test("TC-11: fl-test.fl — my-test 함수 정의됨", () => {
    if (!testFlSrc.includes("my-test"))
        throw new Error("my-test 없음");
    if (!testFlInterp)
        throw new Error("인터프리터 로드 실패");
    if (!hasFn(testFlInterp, "my-test"))
        throw new Error("my-test 함수 미등록");
});
test("TC-12: fl-test.fl 로드 후 my-test 호출 가능 (간단 case)", () => {
    ...
    if (result !== true)
        throw new Error(`my-test 결과: ${result}, 기대: true`);
});
...
// ──────────────────────────────────────────────────────────
console.log("\n[TC-15~17] ;;; 주석 추출\n");
function extractDocComments(src) {
    return src
        .split("\n")
...
reg56Test("함수 내 define이 전역 오염 안 함", () => {
    const interp = new interpreter_1.Interpreter();
    interp.interpret((0, parser_2.parse)((0, lexer_2.lex)(`
    (define x 10)
...
reg56Test("set! 전역 변수 수정", () => {
    ...
    (inc!)
  `)));
    const counter = interp.context.variables.get("$counter");
    if (counter !== 3)
        throw new Error(`counter=${counter}`);
});
// 4. 재귀
reg56Test("재귀 함수", () => {
    const result = run(`
...
reg56Test("let 스코프 격리", () => {
  ...
  `);
    if (result !== 99)
        throw new Error(`let result=${result}`);
});
// 7. 중첩 함수
reg56Test("중첩 함수 스코프", () => {
    const interp = new interpreter_1.Interpreter();
    ...
    (outer 5)
  `)));
    const result = interp.context.variables.get("$result");
    if (result !== 0)
        throw new Error(`result=${result}`);
});
// 8. loop/recur
...
reg56Test("slice 배열 자르기", () => {
    ...
    (slice [1 2 3 4 5] 2 4)
  `);
    if (!Array.isArray(result) || result.length !== 2 || result[0] !== 3)
...
test(`TC-19: Phase 56 regression ${reg56Passed}/14`, () => {
    if (reg56Passed < 14)
        throw new Error(`${reg56Passed}/14 PASS (${14 - reg56Passed}개 실패)`);
});
// ──────────────────────────────────────────────────────────
// 전체 .fl 파일 수 확인 (TC-20)
// ──────────────────────────────────────────────────────────
...
test("TC-20: fl-files 디렉토리에 3개 .fl 파일 존재", () => {
    ...
    if (files.length < 3)
        throw new Error(`${files.length}개 파일 (기대: 3개 이상): ${files.join(", ")}`);
    const required = ["fl-fmt.fl", "fl-lint.fl", "fl-test.fl"];
    for (const req of required) {
        if (!files.includes(req))
            throw new Error(`${req} 없음`);
    }
});
// ──────────────────────────────────────────────────────────
console.log(`\n${"─".repeat(50)}`);
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-phase88-selfhost.js.map