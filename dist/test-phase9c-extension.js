...
try {
    ...
    if (ast.length > 0 && ast[0].kind === "reasoning-sequence") {
        const seq = ast[0];
        ...
        console.log(`❌ TEST 2 FAIL: Expected reasoning-sequence, got ${ast[0]?.kind || "unknown"}`);
    }
}
catch (e) {
...
try {
    ...
    if (result?.kind === "reasoning-sequence-result" &&
        ...
        const allStagesPresent = stageNames.includes("observe") &&
        ...
        console.log(`❌ TEST 4 FAIL: Expected 5 stages, got ${result?.stages?.length || 0}`);
    }
}
catch (e) {
...
try {
    ...
    if (result?.kind === "reasoning-sequence-result" &&
        ...
        console.log(`   Sequence ID: ${result.metadata.sequenceId}`);
...
//# sourceMappingURL=test-phase9c-extension.js.map