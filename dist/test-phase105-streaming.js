...
async function runTests() {
    ...
    await test("1. FLStream 생성", () => {
        const s = new streaming_1.FLStream();
        return s !== undefined && s instanceof streaming_1.FLStream;
    });
    ...
    await test("12. 'chunk' 이벤트 emit", async () => {
        const s = new streaming_1.FLStream();
        let received = null;
        s.on('chunk', (c) => { received = c; });
        s.write("event-test");
        return received !== null && received.content === "event-test";
    });
    await test("13. 'end' 이벤트 emit", async () => {
        const s = new streaming_1.FLStream();
        let endValue = "";
        s.on('end', (v) => { endValue = v; });
        s.write("finish");
        s.end();
    ...
    await test("21. stream-write 내장함수", () => {
        ...
        const s = (0, streaming_1.getStream)(id);
...
//# sourceMappingURL=test-phase105-streaming.js.map