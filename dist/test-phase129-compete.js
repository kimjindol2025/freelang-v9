...
const compete_1 = require("./compete");
let pass = 0;
let fail = 0;
...
test("1. Competition 클래스 인스턴스 생성", () => {
    const c = new compete_1.Competition();
    return c instanceof compete_1.Competition;
});
// ─── 2. register() 경쟁자 ─────────────────────────────────────────
test("2. register() 경쟁자 등록", () => {
    const c = new compete_1.Competition();
    c.register({ id: "agent-a", solve: (p) => p * 2 });
    return c.size() === 1;
});
// ─── 3. list() 목록 ───────────────────────────────────────────────
test("3. list() 등록된 경쟁자 목록 반환", () => {
    const c = new compete_1.Competition();
    c.register({ id: "agent-x", solve: (p) => p });
    c.register({ id: "agent-y", solve: (p) => p + 1 });
    const lst = c.list();
...
test("4. size() 카운트 정확성", () => {
    const c = new compete_1.Competition();
    c.register({ id: "a1", solve: (p) => p });
    c.register({ id: "a2", solve: (p) => p });
    c.register({ id: "a3", solve: (p) => p });
    return c.size() === 3;
});
// ─── 5. run() 기본 경쟁 ───────────────────────────────────────────
test("5. run() 기본 경쟁 실행", () => {
    const c = new compete_1.Competition();
    c.register({ id: "solver", solve: (p) => p + 10 });
...
test("7. winner.rank === 1", () => {
    const c = new compete_1.Competition();
    c.register({ id: "best", solve: (p) => 90 });
    c.register({ id: "worst", solve: (p) => 10 });
...
test("8. allResults 정렬 — 점수 내림차순", () => {
    const c = new compete_1.Competition();
    c.register({ id: "low", solve: (p) => 10 });
    c.register({ id: "high", solve: (p) => 90 });
    c.register({ id: "mid", solve: (p) => 50 });
    const result = c.run("x", (out) => Number(out));
...
test("15. globalCompetition 싱글톤 존재 확인", () => {
    return compete_1.globalCompetition instanceof compete_1.Competition;
});
...
test("16. compete-register 내장함수 — 에러 없이 실행", () => {
    ...
    const localComp = new compete_1.Competition();
    localComp.register({ id: "builtin-a", solve: (p) => Number(p) * 2 });
...
test("17. compete 내장함수 — winner agentId 반환", () => {
    ...
    const localComp = new compete_1.Competition();
    localComp.register({ id: "fast", solve: (p) => Number(p) * 3 });
    localComp.register({ id: "slow", solve: (p) => Number(p) });
...
test("21. 3명 경쟁, 최고 점수 winner", () => {
    const c = new compete_1.Competition();
    c.register({ id: "alpha", solve: (p) => p + 1 });
    c.register({ id: "beta", solve: (p) => p * 2 });
    c.register({ id: "gamma", solve: (p) => p * p });
...
test("23. output이 결과에 포함됨", () => {
    const c = new compete_1.Competition();
    c.register({ id: "out-agent", solve: (p) => `result-${p}` });
...
test("25. 통합: 4명 경쟁 → 최고 점수 winner 선정", () => {
    const c = new compete_1.Competition();
    c.register({ id: "strategy-a", solve: (p) => Number(p) + 5 });
    c.register({ id: "strategy-b", solve: (p) => Number(p) * 3 });
    c.register({ id: "strategy-c", solve: (p) => Math.pow(Number(p), 2) });
...
if (fail > 0)
    process.exit(1);
//# sourceMappingURL=test-phase129-compete.js.map