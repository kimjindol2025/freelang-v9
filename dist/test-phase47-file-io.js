...
const fileIO = {
    file_read: (p) => fs.readFileSync(p, "utf-8"),
    file_write: (p, c) => { fs.writeFileSync(p, c, "utf-8"); return true; },
    file_exists: (p) => fs.existsSync(p),
    file_append: (p, c) => { fs.appendFileSync(p, c, "utf-8"); return true; },
    dir_list: (p) => fs.readdirSync(p),
};
// ── Gen1 빌드 ─────────────────────────────────────────────────
console.log("\n[47-A] Gen1 빌드");
...
test("dir-list — 배열 반환", () => {
    ...
    if (!Array.isArray(result))
        throw new Error(`not array: ${typeof result}`);
    if (result.length === 0)
...
test("dir-list — 생성한 파일 포함 확인", () => {
    ...
    const src = `(dir-list "${dirPath}")`;
    const result = flInterpret(src);
    if (!result.some((f) => f.includes("test.txt"))) {
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase47-file-io.js.map