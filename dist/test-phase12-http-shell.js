...
test("shell_pipe", () => {
    ...
    if (!result.trim().includes("hello_world"))
        throw new Error(`Expected "hello_world", got "${result}"`);
});
test("shell_capture stdout", () => {
    ...
    if (!result.stdout.includes("captured"))
        throw new Error(`Expected stdout to contain "captured"`);
    if (result.code !== 0)
        throw new Error(`Expected code 0, got ${result.code}`);
});
...
test("shell_env PATH", () => {
    ...
    if (typeof result !== "string" || result.length === 0)
        throw new Error(`Expected non-empty PATH`);
});
test("shell_cwd", () => {
    const result = run(`(shell_cwd)`);
    if (typeof result !== "string" || result.length === 0)
        throw new Error(`Expected non-empty cwd`);
});
...
test("shell writes, file reads", () => {
    ...
    if (!content.includes("phase12"))
        throw new Error(`Expected "phase12" in file, got "${content}"`);
...
test("shell_capture + error handling integration", () => {
    ...
    if (!result.stdout.includes("integration-test"))
        throw new Error("Expected integration-test in stdout");
});
console.log("\n=== Phase 12 Tests Complete ===");
//# sourceMappingURL=test-phase12-http-shell.js.map