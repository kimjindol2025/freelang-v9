...
function standaloneCompile1(flSrc) {
    if (!lexerMod1 || !parserMod1 || !codegenMod1)
        throw new Error("Gen1 not ready");
...
function standaloneCompile2(flSrc) {
    if (!lexerMod2 || !parserMod2 || !codegenMod2)
        throw new Error("Gen2 not ready");
...
test("standaloneCompile2(lexer.fl) → Gen3", () => {
    ...
    if (typeof lexerMod3.lex !== "function")
        throw new Error("lex not exported");
...
test("standaloneCompile2(parser.fl) → Gen3", () => {
    ...
    if (typeof parserMod3.parse !== "function")
        throw new Error("parse not exported");
...
test("standaloneCompile2(codegen.fl) → Gen3", () => {
    ...
    if (typeof codegenMod3["gen-js"] !== "function")
        throw new Error("gen-js not exported");
...
function standaloneCompile3(flSrc) {
    if (!lexerMod3 || !parserMod3 || !codegenMod3)
        throw new Error("Gen3 not ready");
...
test("Gen3: fib(20) = 6765", () => {
    const m = makeModule(standaloneCompile3(`[FUNC fib :params [$n] :body ((if (<= $n 1) $n (+ (fib (- $n 1)) (fib (- $n 2)))))] (export fib)`));
    if (m.fib(20) !== 6765)
        throw new Error(`got ${m.fib(20)}`);
});
test("Gen3: loop 10만 반복", () => {
    ...
    if (m["big-sum"](100000) !== 5000050000)
        throw new Error(`got ${m["big-sum"](100000)}`);
});
test("Gen3: match 문자열", () => {
    const m = makeModule(standaloneCompile3(`[FUNC grade :params [$s] :body ((match $s ("A" "excellent") ("B" "good") (_ "ok")))] (export grade)`));
    if (m.grade("A") !== "excellent")
        throw new Error(`got ${m.grade("A")}`);
    if (m.grade("B") !== "good")
        throw new Error(`got ${m.grade("B")}`);
    if (m.grade("C") !== "ok")
        throw new Error(`got ${m.grade("C")}`);
});
test("Gen3: map/filter 파이프라인", () => {
    const m = makeModule(standaloneCompile3(`[FUNC dp :params [$lst] :body ((map (filter $lst [x] (> $x 0)) [x] (* $x 2)))] (export dp)`));
    const r = m.dp([-1, 2, -3, 4, 5]);
    if (JSON.stringify(r) !== JSON.stringify([4, 8, 10]))
...
test("Gen3 lexer로 lexer.fl 렉싱", () => {
    ...
    if (tokens.length < 1000)
        throw new Error(`too few: ${tokens.length}`);
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase33-gen3-bootstrap.js.map