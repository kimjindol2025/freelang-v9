...
(export effect-sum)`;
    ...
    if (m["effect-sum"](3, 4) !== 14)
        throw new Error(`got ${m["effect-sum"](3, 4)}`);
});
...
(export last-wins)`;
    const m = makeModule(standaloneCompile1(src));
    if (m["last-wins"](5) !== 50)
        throw new Error(`got ${m["last-wins"](5)}`);
});
...
test("(do x) no IIFE wrapper", () => {
    ...
    const m = makeModule(js);
    if (m.id("hello") !== "hello")
        throw new Error(`got ${m.id("hello")}`);
});
// ── do + loop/recur 호환성 ──────────────────────────────────────
...
(export count-down)`;
    const m = makeModule(standaloneCompile1(src));
    if (m["count-down"](10) !== 10)
        throw new Error(`got ${m["count-down"](10)}`);
});
...
(export sum-loop)`;
    const m = makeModule(standaloneCompile1(src));
    if (m["sum-loop"](100) !== 5050)
        throw new Error(`got ${m["sum-loop"](100)}`);
});
// ── Gen2 + Gen3 고정점 유지 ───────────────────────────────────
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase39-do-block.js.map