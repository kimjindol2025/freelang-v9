...
exports.globalAlignment = exports.AlignmentSystem = void 0;
class AlignmentSystem {
    ...
    score(action) {
        ...
        const overallScore = Math.max(0, Math.min(1, avgGoalScore * 0.6 + avgValueScore * 0.4 - conflictPenalty));
        // 권고 결정
        let recommendation;
    ...
    suggestImprovements(action) {
        ...
        for (const [goalId, alignment] of Object.entries(result.goalAlignment)) {
            if (alignment < 0.5) {
                ...
                if (goal) {
                    suggestions.push(`목표 "${goal.description}" 기여도 향상 필요 (현재 ${(alignment * 100).toFixed(0)}%): expectedOutcomes["${goalId}"]를 높이세요`);
                }
            }
        }
        // 낮은 가치 정렬도에 대한 제안
        for (const [valueId, alignment] of Object.entries(result.valueAlignment)) {
            if (alignment < 0.7) {
                ...
                if (value) {
                    suggestions.push(`가치 "${value.name}" 위반 위험 있음 (정렬도 ${(alignment * 100).toFixed(0)}%): 관련 리스크 요인을 제거하세요`);
                }
            }
        }
        // 충돌에 대한 제안
        for (const conflict of result.conflicts) {
            ...
            if (g1 && g2) {
                suggestions.push(`충돌(${conflict.severity}): "${g1.description}"와 "${g2.description}" 사이의 절충안 고려`);
            }
        }
        // 위험 요소에 대한 제안
...
exports.AlignmentSystem = AlignmentSystem;
exports.globalAlignment = new AlignmentSystem();
