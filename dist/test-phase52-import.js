...
function makeInterp() {
    ...
    interp.currentFilePath = __dirname;
    return interp;
}
...
test("math:square 함수가 등록됨", () => {
    if (!interp1.context.functions.has("math:square"))
        throw new Error("math:square not registered");
});
test("math:cube 함수가 등록됨", () => {
    if (!interp1.context.functions.has("math:cube"))
        throw new Error("math:cube not registered");
});
test("math:clamp 함수가 등록됨", () => {
    if (!interp1.context.functions.has("math:clamp"))
...
test(":only [square] → square 직접 등록됨", () => {
    if (!interp2.context.functions.has("square"))
        throw new Error("square not registered");
});
test(":only [square] → cube는 등록 안 됨", () => {
    if (interp2.context.functions.has("cube"))
        throw new Error("cube should not be registered");
    if (interp2.context.functions.has("utils:cube"))
...
test(":as m → m:square 등록됨", () => {
    if (!interp3.context.functions.has("m:square"))
...
test("같은 파일 두 번 import → 오류 없음", () => {
    if (circularError)
        throw new Error("second import raised an error");
    if (!interp4.context.functions.has("math:square"))
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase52-import.js.map