...
try {
    ...
    const tokens = (0, lexer_1.lex)(searchCode);
    const hasSearch = tokens.some((t) => t.type === "Search");
    const hasKeywords = tokens.some((t) => t.type === "Keyword" || t.type === "Colon");
    if (hasSearch) {
        console.log(`✅ TEST 1 PASS: Lexer recognized "search" keyword (${tokens.length} total tokens)`);
...
try {
    ...
    if (ast.length > 0 && ast[0].kind === "search-block") {
        const searchBlock = ast[0];
        console.log(`✅ TEST 2 PASS: Parser created SearchBlock node`);
        console.log(`   Query: "${searchBlock.query}", Source: "${searchBlock.source}"`);
    }
    else {
        console.log(`❌ TEST 2 FAIL: Expected search-block, got ${ast[0]?.kind || "unknown"}`);
    }
}
catch (e) {
...
try {
    ...
    const tokens = (0, lexer_1.lex)(fetchCode);
    const ast = (0, parser_1.parse)(tokens);
    if (ast.length > 0 && ast[0].kind === "search-block") {
        const fetchBlock = ast[0];
        console.log(`✅ TEST 3 PASS: Parser created SearchBlock for fetch`);
        console.log(`   URL: "${fetchBlock.query}", Cache: ${fetchBlock.cache}, Source: "${fetchBlock.source}"`);
    }
    else {
...
try {
    ...
    if (ast.length > 0 && ast[0].kind === "search-block") {
        const searchBlock = ast[0];
        console.log(`✅ TEST 4 PASS: Parser handled multiple search options`);
        console.log(`   Query: "${searchBlock.query}"`);
        console.log(`   Source: "${searchBlock.source}", Limit: ${searchBlock.limit}, Cache: ${searchBlock.cache}`);
        console.log(`   Name: "${searchBlock.name}"`);
    }
    else {
...
try {
    ...
    if (context.lastValue && context.lastValue.kind === "search-result") {
        ...
        console.log(`❌ TEST 5 FAIL: Expected search-result, got ${context.lastValue?.kind}`);
    }
}
catch (e) {
...
`;
    const tokens = (0, lexer_1.lex)(programCode);
    const ast = (0, parser_1.parse)(tokens);
    const interpreter = new interpreter_1.Interpreter();
    const context = interpreter.interpret(ast);
...
//# sourceMappingURL=test-phase9-search.js.map