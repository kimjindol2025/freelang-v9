...
exports.evalModuleBlock = evalModuleBlock;
exports.evalImportBlock = evalImportBlock;
exports.evalImportFromFile = evalImportFromFile;
exports.evalOpenBlock = evalOpenBlock;
...
function evalImportBlock(interp, importBlock) {
    ...
    if (selective && selective.length > 0) {
        ...
        selective.forEach((name) => {
            if (!module.exports.includes(name)) {
                interp.logger.warn(`Function "${name}" not exported from module "${moduleName}"`);
            }
        });
    }
    else {
    ...
    interp.logger.info(`✅ Imported ${importedCount} function(s) from "${moduleName}"${selectStr}${aliasStr}`);
}
...
function evalImportFromFile(interp, relPath, prefix, selective, alias) {
    ...
    const subInterp = new interpreter_1.Interpreter();
...
function evalOpenBlock(interp, openBlock) {
    ...
    interp.logger.info(`✅ Opened module "${moduleName}" (${module.exports.length} function(s) available globally)`);
}
//# sourceMappingURL=eval-module-system.js.map