"use strict";
// FreeLang v9 Phase 9a/9b Integration Tests (Simplified)
...
test("learned data flows to decide stage", () => {
    const code = `
    (reasoning-sequence
      (learn "ai-insight" :confidence 0.9)
...
test("complete search→analyze→learn→decide pipeline", () => {
    const code = `
    (reasoning-sequence
      (search "2026 technology trends")
      (analyze :angle "technology")
      (learn "tech-insights" :confidence 0.85)
...
test("multiple search and learn stages", () => {
    ...
    (reasoning-sequence
      ...
      (learn "react-facts" :confidence 0.9)
      (learn "vue-facts" :confidence 0.85)
  ...
  `;
    ...
    const result = context.lastValue;
    const path = result?.metadata?.executionPath || [];
    const searchCount = path.filter((s) => s === "search").length;
    const learnCount = path.filter((s) => s === "learn").length;
...
//# sourceMappingURL=test-phase9ab-integration-simple.js.map