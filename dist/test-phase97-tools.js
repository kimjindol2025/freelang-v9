...
test("TC-1: globalToolRegistry 생성 확인", () => {
    assert(tool_registry_1.globalToolRegistry instanceof tool_registry_1.ToolRegistry, "globalToolRegistry is not ToolRegistry");
});
test("TC-2: register + get", () => {
    const reg = new tool_registry_1.ToolRegistry();
    const toolDef = {
        name: "test-add",
...
test("TC-6: ToolResult 구조 확인", () => {
    ...
    const result = reg.executeSync("noop", {});
...
test("TC-7: math — '2 + 3' → 5", () => {
    const result = tool_registry_1.globalToolRegistry.executeSync("math", { expr: "2 + 3" });
...
test("TC-8: str-upper — 'hello' → 'HELLO'", () => {
    const result = tool_registry_1.globalToolRegistry.executeSync("str-upper", { s: "hello" });
...
test("TC-9: str-lower — 'WORLD' → 'world'", () => {
    const result = tool_registry_1.globalToolRegistry.executeSync("str-lower", { s: "WORLD" });
    assert(result.success, `success=${result.success}`);
    assert(result.output === "world", `output=${result.output}`);
});
test("TC-10: str-len — 'hello' → 5", () => {
    const result = tool_registry_1.globalToolRegistry.executeSync("str-len", { s: "hello" });
    assert(result.success, `success=${result.success}`);
    assert(result.output === 5, `output=${result.output}`);
});
test("TC-11: json-stringify — 객체 → 문자열", () => {
    const result = tool_registry_1.globalToolRegistry.executeSync("json-stringify", { v: { x: 1, y: 2 } });
    assert(result.success, `success=${result.success}`);
...
test("TC-12: type-of — 42 → 'number'", () => {
    const result = tool_registry_1.globalToolRegistry.executeSync("type-of", { v: 42 });
    assert(result.success, `success=${result.success}`);
...
test("TC-13: (use-tool 'str-upper' {s 'test'}) → 'TEST'", () => {
    ...
    const result = tool_registry_1.globalToolRegistry.executeSync("str-upper", { s: "test" });
...
test("TC-14: (use-tool 'math' ...) — 10 * 5 → 50", () => {
    const result = tool_registry_1.globalToolRegistry.executeSync("math", { expr: "10 * 5" });
...
test("TC-18: 등록한 도구 즉시 사용", () => {
    tool_registry_1.globalToolRegistry.register({
        name: "__phase97_test_tool__",
        description: "테스트용 임시 도구",
        inputSchema: { x: "number" },
        execute: ({ x }) => Number(x) * 3,
    });
    const result = tool_registry_1.globalToolRegistry.executeSync("__phase97_test_tool__", { x: 10 });
    assert(result.success, `success=${result.success}`);
    assert(result.output === 30, `output=${result.output}`);
    // FL use-tool로도 검증 — 직접 executeSync로 확인
    const flResult = tool_registry_1.globalToolRegistry.executeSync("__phase97_test_tool__", { x: 5 });
    assert(flResult.output === 15, `fl output=${flResult.output}`);
});
// ── TC-19~24: 에지 케이스 ─────────────────────────────────────────────────
console.log("\n[TC-19~24] 에지 케이스");
test("TC-19: 없는 도구 실행 → success:false", () => {
    const result = tool_registry_1.globalToolRegistry.executeSync("__does_not_exist__", {});
...
test("TC-20: 도구 실행 durationMs 기록", () => {
    const result = tool_registry_1.globalToolRegistry.executeSync("math", { expr: "1 + 1" });
    assert(typeof result.durationMs === "number", `durationMs type=${typeof result.durationMs}`);
    assert(result.durationMs >= 0, `durationMs=${result.durationMs}`);
});
test("TC-21: ToolResult.success = true/false", () => {
    const ok = tool_registry_1.globalToolRegistry.executeSync("math", { expr: "1" });
    assert(ok.success === true, `expected true, got ${ok.success}`);
    const fail = tool_registry_1.globalToolRegistry.executeSync("__nope__", {});
...
test("TC-22: 인자 타입 확인 — str-upper에 숫자 전달 → 문자열 변환", () => {
    ...
    const result = tool_registry_1.globalToolRegistry.executeSync("str-upper", { s: 123 });
    assert(result.success, `success=${result.success}`);
...
test("TC-24: Phase 56 regression 14/14", () => {
    const interp = new interpreter_1.Interpreter();
    // 렉시컬 스코프
    interp.interpret((0, parser_1.parse)((0, lexer_1.lex)(`(define x 10)`)));
  ...
  `);
    ...
    const toolList = tool_registry_1.globalToolRegistry.listAll();
    assert(toolList.length >= 7, `tool count=${toolList.length}`);
    const mathTool = tool_registry_1.globalToolRegistry.get("math");
    assert(mathTool !== undefined, "math tool missing");
    const mathResult = tool_registry_1.globalToolRegistry.executeSync("math", { expr: "3 * 3" });
    assert(mathResult.output === 9, `math regression: ${mathResult.output}`);
    const upperResult = tool_registry_1.globalToolRegistry.executeSync("str-upper", { s: "regression" });
    assert(upperResult.output === "REGRESSION", `str-upper regression: ${upperResult.output}`);
    const lenResult = tool_registry_1.globalToolRegistry.executeSync("str-len", { s: "test" });
    assert(lenResult.output === 4, `str-len regression: ${lenResult.output}`);
    const typeResult = tool_registry_1.globalToolRegistry.executeSync("type-of", { v: true });
...
//# sourceMappingURL=test-phase97-tools.js.map