...
const chain_agents_1 = require("./chain-agents");
...
test("1. AgentChain 생성", () => {
    const chain = new chain_agents_1.AgentChain();
    assertDefined(chain, "AgentChain 인스턴스 없음");
});
test("2. add() 에이전트 추가", () => {
    const chain = new chain_agents_1.AgentChain();
    const agent = { id: "a1", transform: (x) => x + 1 };
...
test("3. length() 확인", () => {
    ...
    chain.add({ id: "a1", transform: (x) => x });
    chain.add({ id: "a2", transform: (x) => x });
    assertEq(chain.length(), 2, "2개 추가 후 length는 2");
});
test("4. run() 단일 에이전트", () => {
    const chain = new chain_agents_1.AgentChain();
    chain.add({ id: "inc", transform: (x) => x + 1 });
    const result = chain.run(10);
    assertEq(result.finalOutput, 11, "단일 에이전트 변환: 10+1=11");
});
test("5. run() 여러 에이전트 체인", () => {
    const chain = chain_agents_1.AgentChain.from([
        { id: "add1", transform: (x) => x + 1 },
        { id: "mul2", transform: (x) => x * 2 },
        { id: "sub3", transform: (x) => x - 3 },
    ]);
...
test("6. 이전 출력이 다음 입력", () => {
    const inputs = [];
    const chain = chain_agents_1.AgentChain.from([
        { id: "a1", transform: (x) => { inputs.push(x); return x + 10; } },
        { id: "a2", transform: (x) => { inputs.push(x); return x + 20; } },
    ]);
    chain.run(5);
...
test("8. links 배열", () => {
    const chain = chain_agents_1.AgentChain.from([
        { id: "step1", transform: (x) => x + 1 },
        { id: "step2", transform: (x) => x * 2 },
    ]);
...
test("9. stepsCompleted 카운트", () => {
    const chain = chain_agents_1.AgentChain.from([
        { id: "a", transform: (x) => x + 1 },
        { id: "b", transform: (x) => x + 2 },
        { id: "c", transform: (x) => x + 3 },
    ]);
...
test("11. 에러 시 success=false", () => {
    const chain = chain_agents_1.AgentChain.from([
        { id: "boom", transform: (_) => { throw new Error("폭발!"); } },
    ]);
...
test("13. validate 실패 → 이전 값 유지", () => {
    const chain = chain_agents_1.AgentChain.from([
        {
            id: "neg-reject",
            transform: (x) => x - 100, // 5-100=-95
            validate: (out) => out > 0, // -95 < 0 → 실패
        },
    ]);
...
test("16. AgentChain.from() 팩토리", () => {
    const agents = [
        { id: "x", transform: (n) => n + 5 },
        { id: "y", transform: (n) => n * 3 },
    ];
...
function run(code) {
    return interp.run(code).lastValue;
}
...
test("22. 문자열 변환 체인", () => {
    const chain = chain_agents_1.AgentChain.from([
        { id: "upper", transform: (s) => s.toUpperCase() },
        { id: "trim", transform: (s) => s.trim() },
        { id: "wrap", transform: (s) => `[${s}]` },
    ]);
...
test("23. validate=undefined → 항상 통과", () => {
    const chain = chain_agents_1.AgentChain.from([
        { id: "no-validate", transform: (x) => x + 99 },
    ]);
    const result = chain.run(1);
...
test("24. 중간 에러 후 중단", () => {
    const chain = chain_agents_1.AgentChain.from([
        { id: "step1", transform: (x) => x + 1 },
        { id: "err", transform: (_) => { throw new Error("중간 에러"); } },
        { id: "step3", transform: (x) => x * 10 },
    ]);
    const result = chain.run(5);
...
test("25. 통합: 4단계 파이프라인", () => {
    const chain = chain_agents_1.AgentChain.from([
        { id: "parse", transform: (s) => parseInt(s, 10) },
        { id: "double", transform: (x) => x * 2 },
        { id: "clamp", transform: (x) => Math.min(x, 100), validate: (o) => o <= 100 },
        { id: "format", transform: (x) => `result:${x}` },
    ]);
...
test("27. 에러 시 link.output=null", () => {
    const chain = chain_agents_1.AgentChain.from([
        { id: "fail", transform: (_) => { throw new Error("fail"); } },
    ]);
    const result = chain.run(1);
...
test("28. globalChain 인스턴스 존재", () => {
    assertDefined(chain_agents_1.globalChain, "globalChain이 export되어 있어야 함");
    assert(chain_agents_1.globalChain instanceof chain_agents_1.AgentChain, "globalChain은 AgentChain 인스턴스");
});
// ─── regression: Phase 56 Lexical Scope ──────────────────────────────────────
console.log("\n=== Regression: Phase 56 Lexical Scope ===\n");
function runReg(code) {
    const interpReg = new interpreter_1.Interpreter();
    return interpReg.run(code).lastValue;
}
function testReg(name, fn) {
    try {
        fn();
        passed++;
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase128-chain-agents.js.map