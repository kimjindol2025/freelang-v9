...
test("TC-1: Reflector 인스턴스 생성", () => {
    ...
    assert(r instanceof reflect_1.Reflector, "Reflector 생성 실패");
    assertEq(r.getCriteriaList().length, 0, "초기 criteria 수 0");
});
test("TC-2: 기본 criteria 추가 (not-null)", () => {
    const r = new reflect_1.Reflector();
    r.addCriteria(reflect_1.CRITERIA["not-null"]);
...
test("TC-4: scores 각 기준별 점수", () => {
    const r = new reflect_1.Reflector();
    r.addCriteria(reflect_1.CRITERIA["not-null"]);
    r.addCriteria(reflect_1.CRITERIA["positive"]);
...
test("TC-13: AI 답변 품질 평가 (길이, null 여부)", () => {
    const r = new reflect_1.Reflector();
    r.addCriteria(reflect_1.CRITERIA["not-null"]);
    r.addCriteria(reflect_1.CRITERIA["not-empty"]);
    r.addCriteria({
        name: "min-length",
        check: (v) => {
...
test("TC-15: feedback 배열 생성", () => {
    const r = new reflect_1.Reflector();
    r.addCriteria(reflect_1.CRITERIA["positive"]);
    r.addCriteria(reflect_1.CRITERIA["is-number"]);
...
test("TC-22: Phase 56 렉시컬 스코프 regression", () => {
    ...
    // 7. let 스코프 격리
    {
      ...
      (let [[$y 42]] $y)
    `);
        const y = interp.context.variables.get("$y");
    ...
    // 9. loop 바인딩이 전역 오염 안 함
    {
      ...
      (loop [i 0]
        (if (>= $i 3) $i (recur (+ $i 1))))
    `);
        const i = interp.context.variables.get("$i");
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase94-reflect.js.map