...
(export head)
  `.trim());
    if (exp.head([10, 20, 30]) !== 10)
        throw new Error(`got ${exp.head([10, 20, 30])}`);
});
test("rest: 나머지 원소", () => {
    const exp = compileAndRun(`
...
(export final)
  `.trim());
    if (exp.final([1, 2, 3, 99]) !== 99)
        throw new Error(`got ${exp.final([1, 2, 3, 99])}`);
});
test("reverse: 배열 뒤집기", () => {
    const exp = compileAndRun(`
...
(export find-positive)
  `.trim());
    if (exp["find-positive"]([-3, -1, 4, 6]) !== 4)
        throw new Error(`got ${exp["find-positive"]([-3, -1, 4, 6])}`);
});
test("sort: 문자열 정렬", () => {
    const exp = compileAndRun(`
...
(export nth-char)
  `.trim());
    if (exp["nth-char"]("hello", 1) !== "e")
        throw new Error(`got ${exp["nth-char"]("hello", 1)}`);
});
...
(export sub)
  `.trim());
    if (exp.sub("hello world", 6, 11) !== "world")
        throw new Error(`got ${exp.sub("hello world", 6, 11)}`);
});
...
(export starts?)
  `.trim());
    if (exp["starts?"]("hello", "hel") !== true)
        throw new Error("starts-with? failed");
    if (exp["starts?"]("hello", "xyz") !== false)
        throw new Error("starts-with? false failed");
});
...
(export ends?)
  `.trim());
    if (exp["ends?"]("hello", "llo") !== true)
        throw new Error("ends-with? failed");
});
...
(export find-idx)
  `.trim());
    if (exp["find-idx"]("hello world", "world") !== 6)
        throw new Error(`got ${exp["find-idx"]("hello world", "world")}`);
    if (exp["find-idx"]("hello", "xyz") !== -1)
        throw new Error("not found case failed");
});
test("split: 구분자로 분리", () => {
    const exp = compileAndRun(`
...
(export sq)
  `.trim());
    if (Math.abs(exp.sq(9) - 3) > 0.001)
        throw new Error(`got ${exp.sq(9)}`);
    if (Math.abs(exp.sq(2) - 1.414) > 0.001)
        throw new Error(`got ${exp.sq(2)}`);
});
test("pow: 거듭제곱", () => {
    const exp = compileAndRun(`
...
(export power)
  `.trim());
    if (exp.power(2, 10) !== 1024)
        throw new Error(`got ${exp.power(2, 10)}`);
    if (exp.power(3, 3) !== 27)
        throw new Error(`got ${exp.power(3, 3)}`);
});
// ─── [18] 타입 조건자 ─────────────────────────────────────────────
console.log("\n[18] 타입 조건자");
test("string?: 문자열 판별", () => {
...
(export str?)
  `.trim());
    if (exp["str?"]("hello") !== true)
        throw new Error("string? true failed");
    if (exp["str?"](42) !== false)
        throw new Error("string? false failed");
});
test("number?: 숫자 판별", () => {
    const exp = compileAndRun(`
...
(export num?)
  `.trim());
    if (exp["num?"](42) !== true)
        throw new Error("number? true failed");
    if (exp["num?"]("42") !== false)
        throw new Error("number? false failed");
});
test("array?: 배열 판별", () => {
    const exp = compileAndRun(`
...
(export arr?)
  `.trim());
    if (exp["arr?"]([1, 2, 3]) !== true)
        throw new Error("array? true failed");
    if (exp["arr?"]("hello") !== false)
        throw new Error("array? false failed");
});
test("map?: 맵 판별", () => {
    const exp = compileAndRun(`
...
(export obj?)
  `.trim());
    if (exp["obj?"]({ a: 1 }) !== true)
        throw new Error("map? true failed");
    if (exp["obj?"]([1, 2]) !== false)
        throw new Error("map? false (array) failed");
});
// ─── [19] 복합 실용 예제 ─────────────────────────────────────────
console.log("\n[19] 복합 실용 예제");
test("숫자 정렬 (sort + map)", () => {
...
(export palindrome?)
  `.trim());
    if (exp["palindrome?"]("racecar") !== true)
        throw new Error("racecar not palindrome");
    if (exp["palindrome?"]("hello") !== false)
        throw new Error("hello palindrome?");
});
test("단어 빈도 계산", () => {
    const exp = compileAndRun(`
...
(export word-freq)
  `.trim());
    ...
    if (freq.a !== 3 || freq.b !== 2 || freq.c !== 1)
        throw new Error(`got ${JSON.stringify(freq)}`);
});
test("깊이 우선 탐색 (트리)", () => {
    const exp = compileAndRun(`
...
(export tree-sum)
  `.trim());
    ...
    if (exp["tree-sum"](tree) !== 6)
        throw new Error(`got ${exp["tree-sum"](tree)}`);
});
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase27-stdlib-codegen.js.map