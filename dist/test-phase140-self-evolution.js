...
const self_evolution_hub_1 = require("./self-evolution-hub");
...
function makePopulation(size = 10) {
    return Array.from({ length: size }, () => Array.from({ length: 5 }, () => Math.random() * 10));
}
// ── 테스트 그룹 1: SelfEvolutionHub 기본 ─────────────────────────────────────
console.log("\n=== Phase 140: SelfEvolutionHub 기본 ===");
test("1. SelfEvolutionHub 생성", () => {
    const hub = new self_evolution_hub_1.SelfEvolutionHub();
    assert(hub !== null, "Hub should be created");
    assert(hub instanceof self_evolution_hub_1.SelfEvolutionHub, "Should be SelfEvolutionHub instance");
});
...
test("16. enableRefactor 리팩토링", () => {
    ...
    const result = hub.runCycle(pop, (item) => String(item).length / 100, (item) => item, (a, _b) => a, { generations: 3, enableRefactor: true });
...
test("21. self-evolve 빌트인", () => {
  ...
  `);
    ...
    assert(result.has("bestFitness"), "Should have bestFitness");
});
test("22. self-evolve-numbers 빌트인", () => {
    ...
    assert(result.has("best"), "Should have best");
});
...
test("24. evolution-report 빌트인", () => {
    ...
    const hub2 = new self_evolution_hub_1.SelfEvolutionHub();
...
test("25. self-improve 빌트인", () => {
    ...
    assert(result.has("improvement"), "Should have improvement");
});
...
test("28. evolution-fitness 빌트인", () => {
  ...
  `);
    ...
    assertGte(result, 0, "fitness should be >= 0");
});
// ── 테스트 그룹 3: 엣지 케이스 ───────────────────────────────────────────────
console.log("\n=== Phase 140: 엣지 케이스 ===");
...
//# sourceMappingURL=test-phase140-self-evolution.js.map