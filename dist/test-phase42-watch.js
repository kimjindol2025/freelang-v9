...
function watchCompile(filePath) {
...
test("3회 연속 파일 변경 재컴파일", () => {
    ...
    for (const { src, expected } of versions) {
        ...
        const m = watchCompile(tmpFile);
        if (m.f() !== expected)
            throw new Error(`got ${m.f()}, expected ${expected}`);
    }
    fs.unlinkSync(tmpFile);
...
test("소스 변경 → JS 출력 변경", () => {
    ...
    if (js1 === js2)
        throw new Error("different source produced identical output");
});
...
test("첫 체크 → 변경 감지", () => {
    ...
    if (!state.hasChanged(tmpFile))
        throw new Error("first check should detect change");
    fs.unlinkSync(tmpFile);
});
test("두 번째 체크 → 변경 없음", () => {
    ...
    if (state.hasChanged(tmpFile))
        throw new Error("second check should not detect change");
    fs.unlinkSync(tmpFile);
});
// ── 다중 파일 컴파일 체인 ────────────────────────────────────
console.log("\n[42-E] 다중 파일 컴파일 체인");
...
(export sum-powers)`;
    ...
    if (appMod["sum-powers"](3) !== 36)
        throw new Error(`sum-powers: ${appMod["sum-powers"](3)}`);
    if (appMod["sum-powers"](4) !== 80)
        throw new Error(`sum-powers(4): ${appMod["sum-powers"](4)}`);
});
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase42-watch.js.map