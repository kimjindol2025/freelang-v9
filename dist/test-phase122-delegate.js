...
test("16. globalDelegation 싱글톤", () => {
    assertDefined(delegate_1.globalDelegation, "globalDelegation이 없음");
    assert(delegate_1.globalDelegation instanceof delegate_1.DelegationManager, "DelegationManager 인스턴스여야 함");
});
...
test("19. delegate-all 내장함수", () => {
    const dm = new delegate_1.DelegationManager();
    dm.register({ id: "sum-agent", capabilities: ["sum"], execute: (t) => t.input.reduce((a, b) => a + b, 0) });
    const result = dm.delegateAll([
...
test("23. 태스크 input이 에이전트에 전달", () => {
    ...
    assertEq(receivedInput.value, 99, "input.value=99");
});
...
//# sourceMappingURL=test-phase122-delegate.js.map