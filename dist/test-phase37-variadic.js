...
test("rest만 있는 함수", () => {
    ...
    if (m.sum(1, 2, 3, 4, 5) !== 15)
        throw new Error(`got ${m.sum(1, 2, 3, 4, 5)}`);
});
test("앞 파라미터 + rest", () => {
    ...
    if (m["first-plus"](10, 1, 2, 3) !== 16)
        throw new Error(`got ${m["first-plus"](10, 1, 2, 3)}`);
});
test("rest를 배열로 전달", () => {
    const src = `[FUNC join-all :params [$sep & $parts] :body ((str-join $parts $sep))] (export join-all)`;
    const m = makeModule(standaloneCompile1(src));
    if (m["join-all"]("-", "a", "b", "c") !== "a-b-c")
        throw new Error(`got ${m["join-all"]("-", "a", "b", "c")}`);
});
test("가변 인자 0개 호출", () => {
    const src = `[FUNC zero-rest :params [$a & $rest] :body ((length $rest))] (export zero-rest)`;
    const m = makeModule(standaloneCompile1(src));
    if (m["zero-rest"]("x") !== 0)
        throw new Error(`got ${m["zero-rest"]("x")}`);
});
...
(export apply-fn)`;
    const js = standaloneCompile1(src);
    if (!js.includes("...args"))
        throw new Error("no ...args in fn");
    const m = makeModule(js);
    const result = m["apply-fn"]((arr) => arr.reduce((a, b) => a + b, 0), 1, 2, 3);
    if (result !== 6)
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase37-variadic.js.map