...
test("1. MessageBus 생성", () => {
    const bus = new multi_agent_1.MessageBus();
...
test("17. globalBus 싱글톤", () => {
    assert(multi_agent_1.globalBus !== null && multi_agent_1.globalBus !== undefined);
    assert(multi_agent_1.globalBus instanceof multi_agent_1.MessageBus);
});
// ────────────────────────────────────────────
// Part 2: FL 내장 함수 테스트
// ────────────────────────────────────────────
...
test("18. agent-spawn 내장함수", () => {
    ...
    const sizeBefore = multi_agent_1.globalBus.list().filter(id => id === "fl-test-spawn-18").length;
    multi_agent_1.globalBus.spawn("fl-test-spawn-18", (msg) => msg);
    const sizeAfter = multi_agent_1.globalBus.list().filter(id => id === "fl-test-spawn-18").length;
    assertEqual(sizeAfter, 1);
});
test("19. agent-send 내장함수", () => {
    // globalBus에 에이전트 미리 등록
    if (!multi_agent_1.globalBus.list().includes("fl-send-src-19")) {
        multi_agent_1.globalBus.spawn("fl-send-src-19", (m) => m);
    }
    if (!multi_agent_1.globalBus.list().includes("fl-send-dst-19")) {
        multi_agent_1.globalBus.spawn("fl-send-dst-19", (m) => m);
    }
    const interp = makeInterp();
    evalFL(interp, `(agent-send "fl-send-src-19" "fl-send-dst-19" "test-content")`);
    const sz = multi_agent_1.globalBus.inboxSize("fl-send-dst-19");
    assert(sz >= 1, `inbox size should be >= 1, got ${sz}`);
});
test("20. agent-recv 내장함수", () => {
    if (!multi_agent_1.globalBus.list().includes("fl-recv-src-20")) {
        multi_agent_1.globalBus.spawn("fl-recv-src-20", (m) => m);
    }
    if (!multi_agent_1.globalBus.list().includes("fl-recv-dst-20")) {
        multi_agent_1.globalBus.spawn("fl-recv-dst-20", (m) => m);
    }
    multi_agent_1.globalBus.send("fl-recv-src-20", "fl-recv-dst-20", "recv-test");
    const interp = makeInterp();
...
test("21. agent-broadcast 내장함수", () => {
    if (!multi_agent_1.globalBus.list().includes("fl-bc-src-21")) {
        multi_agent_1.globalBus.spawn("fl-bc-src-21", (m) => m);
    }
    if (!multi_agent_1.globalBus.list().includes("fl-bc-dst-21")) {
        multi_agent_1.globalBus.spawn("fl-bc-dst-21", (m) => m);
    }
    const interp = makeInterp();
    const result = evalFL(interp, `(agent-broadcast "fl-bc-src-21" "hello everyone")`);
    assert(Array.isArray(result));
    // fl-bc-dst-21이 받아야 함
    const sz = multi_agent_1.globalBus.inboxSize("fl-bc-dst-21");
    assert(sz >= 1);
});
test("22. agent-process 내장함수", () => {
    if (!multi_agent_1.globalBus.list().includes("fl-proc-src-22")) {
        multi_agent_1.globalBus.spawn("fl-proc-src-22", (m) => m);
    }
    if (!multi_agent_1.globalBus.list().includes("fl-proc-dst-22")) {
        multi_agent_1.globalBus.spawn("fl-proc-dst-22", (msg) => `processed:${msg.content}`);
    }
    multi_agent_1.globalBus.send("fl-proc-src-22", "fl-proc-dst-22", "data");
    const interp = makeInterp();
...
test("24. agent-inbox-size 내장함수", () => {
    if (!multi_agent_1.globalBus.list().includes("fl-inbox-src-24")) {
        multi_agent_1.globalBus.spawn("fl-inbox-src-24", (m) => m);
    }
    if (!multi_agent_1.globalBus.list().includes("fl-inbox-dst-24")) {
        multi_agent_1.globalBus.spawn("fl-inbox-dst-24", (m) => m);
    }
    multi_agent_1.globalBus.send("fl-inbox-src-24", "fl-inbox-dst-24", "m1");
    multi_agent_1.globalBus.send("fl-inbox-src-24", "fl-inbox-dst-24", "m2");
    const interp = makeInterp();
...
//# sourceMappingURL=test-phase103-multi-agent.js.map