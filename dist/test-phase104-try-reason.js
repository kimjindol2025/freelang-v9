...
const try_reason_1 = require("./try-reason");
let passed = 0;
let failed = 0;
...
async function runAllTests() {
    ...
    await test("TryReasoner 생성", () => {
        const r = new try_reason_1.TryReasoner();
    ...
    await test("lastSuccess() 마지막 성공", async () => {
        ...
        assertEqual(ls.value, 99, "lastSuccess 값 틀림");
    ...
    await test("tryReasonSync() 동기 버전 존재", () => {
        assert(typeof try_reason_1.tryReasonSync === "function", "tryReasonSync 없음");
    });
    // 13. tryReasonSync() 첫 성공
    await test("tryReasonSync() 첫 성공", () => {
        const result = (0, try_reason_1.tryReasonSync)({
            attempts: [
                { strategy: "s1", fn: () => 42 },
                { strategy: "s2", fn: () => 99 },
            ],
    ...
    await test("tryReasonSync() 두 번째 성공", () => {
        const result = (0, try_reason_1.tryReasonSync)({
            attempts: [
                { strategy: "fail", fn: (() => { throw new Error("no"); }) },
                { strategy: "win", fn: () => 77 },
            ],
        });
    ...
    await test("tryReasonSync() 모두 실패 → onAllFail", () => {
        const result = (0, try_reason_1.tryReasonSync)({
            attempts: [
                { strategy: "x", fn: (() => { throw new Error("err"); }) },
            ],
    ...
    await test("try-reason 내장함수", async () => {
        const result = await (0, try_reason_1.tryReasonBuiltin)([
            ["primary", () => "hello"],
            ["secondary", () => "world"],
        ]);
    ...
    await test("try-with-fallback 성공 케이스", async () => {
        const result = await (0, try_reason_1.tryWithFallback)(() => 123, 999);
    ...
    await test("try-with-fallback 실패 → fallback", async () => {
        const result = await (0, try_reason_1.tryWithFallback)(() => { throw new Error("fail"); return 0; }, 999);
    ...
    await test("success: true/false 구분", async () => {
        ...
        assertEqual(successEntry.value, 42, "success 값 틀림");
    });
    // ────────────────────────────────────────────
    // 추가 테스트 (26~28)
    // ────────────────────────────────────────────
    ...
    await test("tryWithFallback async fn 지원", async () => {
        const result = await (0, try_reason_1.tryWithFallback)(async () => {
            await new Promise((res) => setTimeout(res, 1));
            return "async-val";
        }, "fallback");
...
//# sourceMappingURL=test-phase104-try-reason.js.map