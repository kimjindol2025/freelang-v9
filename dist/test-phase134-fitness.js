...
test("FitnessEvaluator 생성", () => {
    const fe = new fitness_1.FitnessEvaluator();
    assert(fe instanceof fitness_1.FitnessEvaluator);
});
...
test("multiObjective 가중치 적용", () => {
    const fe = new fitness_1.FitnessEvaluator();
    const r = fe.multiObjective({ speed: 0.8, accuracy: 0.9 }, { speed: 1.0, accuracy: 1.0 }, { speed: 2.0, accuracy: 1.0 });
    assert(r.score >= 0 && r.score <= 1, `score=${r.score}`);
...
test("normalize=false 원점수 반환 (rawScore)", () => {
    const fe = new fitness_1.FitnessEvaluator({ normalize: false });
    const r = fe.proximity(4.5, 5.0, 1.0);
...
test("maximize=false 역전 (낮을수록 좋음)", () => {
    const fe = new fitness_1.FitnessEvaluator({ maximize: false });
    const r = fe.proximity(5.0, 5.0, 1.0);
...
test("globalFitness 인스턴스 사용", () => {
    const r = fitness_1.globalFitness.proximity(3.0, 3.0);
    assert(approx(r.score, 1.0), `score=${r.score}`);
});
test("fitnessScore 헬퍼 함수", () => {
    const s = (0, fitness_1.fitnessScore)(7.0, 7.0);
    assert(approx(s, 1.0), `score=${s}`);
});
test("rankItems 헬퍼 함수", () => {
    const items = ["c", "a", "b"];
    const scores = [3, 10, 7];
    const ranked = (0, fitness_1.rankItems)(items, scores);
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-phase134-fitness.js.map