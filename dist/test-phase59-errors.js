...
function runCodeExpectError(src) {
    const interp = new interpreter_1.Interpreter();
    try {
        interp.interpret((0, parser_1.parse)((0, lexer_1.lex)(src)));
...
`;
    ...
    assert(e.message.includes("Function not found") || e.message.includes("compute-tax"), `에러 메시지에 'Function not found' 또는 'compute-tax' 포함 필요: ${e.message}`);
});
...
`;
    const e = runCodeExpectError(code);
    const isFNFE = e instanceof errors_1.FunctionNotFoundError;
    const msgHasHint = e.message.includes("compute-tax");
    const hintHasSimilar = isFNFE && !!(e.hint?.includes("compute-tax"));
    assert(msgHasHint || hintHasSimilar, `힌트에 'compute-tax' 포함 필요:\nmessage=${e.message}\nhint=${isFNFE ? e.hint : "N/A"}`);
});
test("FunctionNotFoundError 타입 확인", () => {
    const e = runCodeExpectError(`(compute-missing 1)`);
    assert(e instanceof errors_1.FunctionNotFoundError || e.message.includes("Function not found"), `FunctionNotFoundError 또는 'Function not found' 메시지: ${e.message}`);
});
...
test("거리 1 — compute-taz → compute-tax", () => {
    const result = (0, error_formatter_1.suggestSimilar)("compute-taz", ["compute-tax", "compute-rate"]);
    assert(result === "compute-tax", `'compute-tax' 반환 필요: ${result}`);
});
test("거리 2 — compule-tax → compute-tax", () => {
    const result = (0, error_formatter_1.suggestSimilar)("compule-tax", ["compute-tax", "add"]);
    assert(result === "compute-tax", `'compute-tax' 반환 필요: ${result}`);
});
test("거리 3 이상 — null 반환", () => {
    const result = (0, error_formatter_1.suggestSimilar)("xyz", ["compute-tax", "compute-rate"]);
    assert(result === null, `null 반환 필요: ${result}`);
});
test("정확히 일치 — 거리 0", () => {
    const result = (0, error_formatter_1.suggestSimilar)("add", ["add", "subtract"]);
    assert(result === "add", `'add' 반환 필요: ${result}`);
});
test("빈 후보 목록 — null", () => {
    const result = (0, error_formatter_1.suggestSimilar)("foo", []);
    assert(result === null, `null 반환 필요: ${result}`);
});
test("대소문자 무시 — FOO vs foo", () => {
    const result = (0, error_formatter_1.suggestSimilar)("FOO", ["foo", "bar"]);
    assert(result === "foo", `'foo' 반환 필요: ${result}`);
});
// ─── TC-3b: levenshtein 직접 검증 ───
console.log("\n[TC-3b] levenshtein 거리 검증");
test("동일 문자열 → 0", () => {
    assert((0, error_formatter_1.levenshtein)("abc", "abc") === 0, "거리가 0이어야 합니다");
});
test("빈 문자열 → 길이", () => {
    assert((0, error_formatter_1.levenshtein)("", "abc") === 3, "거리가 3이어야 합니다");
});
test("1글자 치환 → 1", () => {
    assert((0, error_formatter_1.levenshtein)("cat", "car") === 1, "거리가 1이어야 합니다");
});
test("삽입 1회 → 1", () => {
    assert((0, error_formatter_1.levenshtein)("ab", "abc") === 1, "거리가 1이어야 합니다");
});
...
test("file/line/col 포함 시 헤더 형식", () => {
    ...
    const out = (0, error_formatter_1.formatError)(err);
...
test("FunctionNotFoundError — 필드 설정 확인", () => {
    const e = new errors_1.FunctionNotFoundError("compute-tax", "agent.fl", 42, 12, "혹시 'compute-rate'를 말씀하신 건가요?");
...
//# sourceMappingURL=test-phase59-errors.js.map