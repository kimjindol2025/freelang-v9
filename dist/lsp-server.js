...
exports.FLLanguageServer = void 0;
...
// ──────────────────────────────────────────────
function getSymbolAtPosition(src, line, character) {
    ...
    const isSymbolChar = (ch) => /[a-zA-Z0-9_\-+*/<>=!?$]/.test(ch);
    let start = character;
    let end = character;
    while (start > 0 && isSymbolChar(lineText[start - 1]))
        start--;
    while (end < lineText.length && isSymbolChar(lineText[end]))
        end++;
    if (start === end)
        return null;
    return lineText.slice(start, end);
}
function collectDefinedSymbols(src) {
    const symbols = [];
    let ast;
    try {
        ast = (0, parser_1.parse)((0, lexer_1.lex)(src));
    }
    catch {
        return symbols;
    }
    ...
    for (const node of ast) {
        ...
        if (node.kind === "sexpr" && node.op === "define" && node.args.length >= 1) {
            ...
            else if (nameNode.kind === "literal" && nameNode.type === "symbol") {
                symName = String(nameNode.value);
            }
            if (symName) {
...
// FLLanguageServer
// ──────────────────────────────────────────────
class FLLanguageServer {
    constructor() {
        this.linter = (0, linter_1.createDefaultLinter)();
        this.formatter = new formatter_1.FLFormatter();
    }
    /**
...
exports.FLLanguageServer = FLLanguageServer;
