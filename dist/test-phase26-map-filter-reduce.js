...
(export sum)
  `.trim());
    if (exp.sum([1, 2, 3, 4, 5]) !== 15)
        throw new Error(`got ${exp.sum([1, 2, 3, 4, 5])}`);
});
test("reduce: 곱셈", () => {
    const exp = compileAndRun(`
...
(export product)
  `.trim());
    if (exp.product([1, 2, 3, 4, 5]) !== 120)
        throw new Error(`got ${exp.product([1, 2, 3, 4, 5])}`);
});
test("reduce: 최대값", () => {
    const exp = compileAndRun(`
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase26-map-filter-reduce.js.map