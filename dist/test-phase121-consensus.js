...
function assertNull(v, msg) {
    if (v !== null && v !== undefined)
        throw new Error(msg ?? `Expected null, got ${JSON.stringify(v)}`);
}
...
test("1. ConsensusEngine 클래스 인스턴스 생성", () => {
    const engine = new consensus_1.ConsensusEngine();
...
test("18. globalConsensus 싱글톤 존재", () => {
    assertDefined(consensus_1.globalConsensus, "globalConsensus 없음");
    assert(consensus_1.globalConsensus instanceof consensus_1.ConsensusEngine, "ConsensusEngine 인스턴스여야 함");
});
// ─── 8. 내장 함수 테스트 ─────────────────────────────────────────────────
...
//# sourceMappingURL=test-phase121-consensus.js.map