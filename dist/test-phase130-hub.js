...
const multi_agent_hub_1 = require("./multi-agent-hub");
...
function assertDefined(v, msg) {
    if (v === undefined || v === null)
        throw new Error(msg ?? `Expected defined, got ${v}`);
}
...
test("1. MultiAgentHub 생성", () => {
    const hub = new multi_agent_hub_1.MultiAgentHub();
...
test("3. globalHub 싱글톤 존재", () => {
    assertDefined(multi_agent_hub_1.globalHub, "globalHub 없음");
    assert(multi_agent_hub_1.globalHub instanceof multi_agent_hub_1.MultiAgentHub, "globalHub는 MultiAgentHub 인스턴스여야 함");
});
...
test("6. ConsensusEngine 생성 가능", () => {
    const engine = new multi_agent_hub_1.ConsensusEngine();
...
test("8. DelegationManager 생성", () => {
    const dm = new multi_agent_hub_1.DelegationManager();
...
test("10. VotingSystem 생성", () => {
    const vs = new multi_agent_hub_1.VotingSystem();
...
test("12. Negotiator 생성", () => {
    const n = new multi_agent_hub_1.Negotiator();
...
test("14. Swarm 생성", () => {
    const s = new multi_agent_hub_1.Swarm();
    assertDefined(s, "Swarm 없음");
});
test("15. swarm optimize 동작", () => {
    const s = new multi_agent_hub_1.Swarm();
    const result = s.optimize({ objective: (x) => -(x - 0.5) * (x - 0.5) });
...
test("16. Orchestrator 생성", () => {
    const o = new multi_agent_hub_1.Orchestrator();
    assertDefined(o, "Orchestrator 없음");
});
test("17. orchestrate run 동작", () => {
    const o = new multi_agent_hub_1.Orchestrator();
    o.register({ id: "t1", run: (x) => x * 2 });
...
test("18. PeerReviewSystem 생성", () => {
    const prs = new multi_agent_hub_1.PeerReviewSystem();
...
test("19. peer-review 동작", () => {
    ...
    prs.addReviewer({
        id: "reviewer-1",
        review: (_out) => ({ reviewerId: "reviewer-1", aspect: "quality", score: 0.9, comment: "좋음" }),
    });
...
test("20. AgentChain 생성", () => {
    const ch = new multi_agent_hub_1.AgentChain();
    assertDefined(ch, "AgentChain 없음");
});
test("21. chain-agents 동작", () => {
    const ch = new multi_agent_hub_1.AgentChain();
    ch.add({ id: "double", transform: (x) => x * 2 });
    ch.add({ id: "addTen", transform: (x) => x + 10 });
...
test("22. Competition 생성", () => {
    const comp = new multi_agent_hub_1.Competition();
    assertDefined(comp, "Competition 없음");
});
test("23. compete 동작", () => {
    const comp = new multi_agent_hub_1.Competition();
    comp.register({ id: "solver-a", solve: (_p) => 80 });
    comp.register({ id: "solver-b", solve: (_p) => 95 });
    const result = comp.run("problem", (x) => Number(x));
...
test("29. hub-systems 내장함수", () => {
    ...
    assert(result.length === 9, "시스템 9개여야 함");
});
// ─── 12. 전체 시스템 임포트 및 싱글톤 확인 ──────────────────────────────
test("30. 전체 시스템 임포트 성공 — 9개 글로벌 싱글톤", () => {
    assertDefined(multi_agent_hub_1.globalConsensus, "globalConsensus 없음");
    assertDefined(multi_agent_hub_1.globalDelegation, "globalDelegation 없음");
    assertDefined(multi_agent_hub_1.globalVoting, "globalVoting 없음");
    assertDefined(multi_agent_hub_1.globalNegotiator, "globalNegotiator 없음");
    assertDefined(multi_agent_hub_1.globalSwarm, "globalSwarm 없음");
    assertDefined(multi_agent_hub_1.globalOrchestrator, "globalOrchestrator 없음");
    assertDefined(multi_agent_hub_1.globalPeerReview, "globalPeerReview 없음");
    assertDefined(multi_agent_hub_1.globalAgentChain, "globalAgentChain 없음");
    assertDefined(multi_agent_hub_1.globalCompetition, "globalCompetition 없음");
});
test("31. 각 globalX 싱글톤 타입 확인", () => {
    assert(multi_agent_hub_1.globalConsensus instanceof multi_agent_hub_1.ConsensusEngine, "ConsensusEngine 타입 불일치");
    assert(multi_agent_hub_1.globalDelegation instanceof multi_agent_hub_1.DelegationManager, "DelegationManager 타입 불일치");
    assert(multi_agent_hub_1.globalVoting instanceof multi_agent_hub_1.VotingSystem, "VotingSystem 타입 불일치");
    assert(multi_agent_hub_1.globalNegotiator instanceof multi_agent_hub_1.Negotiator, "Negotiator 타입 불일치");
    assert(multi_agent_hub_1.globalSwarm instanceof multi_agent_hub_1.Swarm, "Swarm 타입 불일치");
    assert(multi_agent_hub_1.globalOrchestrator instanceof multi_agent_hub_1.Orchestrator, "Orchestrator 타입 불일치");
    assert(multi_agent_hub_1.globalPeerReview instanceof multi_agent_hub_1.PeerReviewSystem, "PeerReviewSystem 타입 불일치");
    assert(multi_agent_hub_1.globalCompetition instanceof multi_agent_hub_1.Competition, "Competition 타입 불일치");
});
...
test("32. 통합: consensus + compete 순차 사용", () => {
    ...
    const comp = new multi_agent_hub_1.Competition();
    comp.register({ id: "solver-1", solve: (_p) => consensusResult.answer + 10 });
    comp.register({ id: "solver-2", solve: (_p) => consensusResult.answer + 50 });
    const compResult = comp.run("consensus-problem", (x) => Number(x));
...
test("35. hub route 'chain' — 에이전트 있을 때 변환", () => {
    const hub = new multi_agent_hub_1.MultiAgentHub();
    const agents = [
        { id: "double", transform: (x) => x * 2 },
        { id: "inc", transform: (x) => x + 1 },
    ];
...
test("36. hub route 'compete' — 에이전트 있을 때 최선 선택", () => {
    ...
    const problem = { task: "problem", evaluate: (x) => Number(x) };
...
//# sourceMappingURL=test-phase130-hub.js.map