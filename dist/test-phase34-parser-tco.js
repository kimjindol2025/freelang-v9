...
test("FUNC + export 파싱", () => {
    ...
    if (!Array.isArray(ast) || ast.length < 2)
        throw new Error(`ast.length=${ast.length}`);
    if (ast[0].kind !== "block" || ast[0].type !== "FUNC")
        throw new Error("not FUNC block");
});
test("loop/recur 파싱", () => {
    const src = `[FUNC sum :params [$n] :body ((loop [[$i $n] [$acc 0]] (if (<= $i 0) $acc (recur (- $i 1) (+ $acc $i)))))] (export sum)`;
    const tokens = lexerMod1.lex(src);
    const ast = parserMod1.parse(tokens);
    if (!ast[0].fields.body)
...
test("Gen1 parser로 lexer.fl 파싱", () => {
    ...
    if (!Array.isArray(ast) || ast.length < 5)
        throw new Error(`nodes=${ast.length}`);
...
test("Gen1 parser로 codegen.fl 파싱", () => {
    ...
    const ast = parserMod1.parse(tokens);
    if (!Array.isArray(ast) || ast.length < 20)
        throw new Error(`nodes=${ast.length}`);
    console.log(`    → ${tokens.length} tokens → ${ast.length} nodes`);
});
// ── Gen2 빌드 ─────────────────────────────────────────────────
...
test("lexer2JS === lexer3JS (고정점)", () => {
    if (lexer2JS !== lexer3JS)
        throw new Error(`diff at ${[...lexer2JS].findIndex((c, i) => c !== lexer3JS[i])}`);
});
test("parser2JS === parser3JS (고정점)", () => {
    if (parser2JS !== parser3JS)
        throw new Error(`diff at ${[...parser2JS].findIndex((c, i) => c !== parser3JS[i])}`);
});
test("codegen2JS === codegen3JS (고정점)", () => {
    if (codegen2JS !== codegen3JS)
        throw new Error(`diff at ${[...codegen2JS].findIndex((c, i) => c !== codegen3JS[i])}`);
});
// ── Gen1===Gen2===Gen3 동치 ───────────────────────────────────
...
function standaloneCompile3(src) {
...
for (const p of PROGS) {
    test(`동치: ${p.name}`, () => {
        ...
        if (js1 !== js2)
            throw new Error(`Gen1≠Gen2`);
        if (js2 !== js3)
            throw new Error(`Gen2≠Gen3`);
    });
}
// ── 런타임 정확성 ─────────────────────────────────────────────
console.log("\n[34-G] Gen3 런타임 정확성");
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase34-parser-tco.js.map