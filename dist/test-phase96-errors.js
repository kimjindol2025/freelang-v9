...
test("TC-23: err.category 필드", () => {
    const e = (0, result_type_1.err)("E", "msg", { category: result_type_1.ErrorCategory.TYPE_ERROR });
    assert(e.category === result_type_1.ErrorCategory.TYPE_ERROR, `category=${e.category}`);
    assert(e._tag === "Err", `_tag=${e._tag}`);
});
test("TC-24: err.recoverable 필드", () => {
    const e1 = (0, result_type_1.err)("E", "msg", { recoverable: true });
    const e2 = (0, result_type_1.err)("E", "msg", { recoverable: false });
...
test("TC-25: err.hint 필드", () => {
    const e = (0, result_type_1.err)("E", "msg", { hint: "이렇게 해보세요" });
    assert(e.hint === "이렇게 해보세요", `hint=${e.hint}`);
});
test("TC-26: AIErrorSystem.classify — 기존 throw 에러 → Err", () => {
    const system = new error_system_1.AIErrorSystem();
...
test("TC-27: AIErrorSystem.explain → 한국어 설명", () => {
    const e = (0, result_type_1.err)("E", "변수 없음", { category: result_type_1.ErrorCategory.NOT_FOUND });
    const explanation = error_system_1.defaultErrorSystem.explain(e);
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-phase96-errors.js.map