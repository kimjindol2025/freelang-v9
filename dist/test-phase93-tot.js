...
test("TC-2: branch 추가 후 체이닝", () => {
    ...
    // 체이닝: this 반환 확인
    assert(result instanceof tot_1.TreeOfThought, "branch()가 TreeOfThought 인스턴스 반환하지 않음");
});
test("TC-3: 여러 branch 독립 실행", () => {
    ...
    tot.evaluate((r) => r / 3).select("best");
    assert(log.includes("A") && log.includes("B") && log.includes("C"), `branch가 독립 실행되지 않음: ${log}`);
});
test("TC-4: select('best') → 최고 점수 반환", () => {
    ...
    tot
        ...
        .branch("중간", () => 50)
        .branch("높음", () => 100);
    tot.evaluate((r) => r / 100);
    const result = tot.select("best");
    assert(result.best.hypothesis === "높음", `best.hypothesis 오류: ${result.best.hypothesis}`);
});
...
test("TC-7: 2개 branch → best 선택", () => {
    ...
    (TOT
      ...
      :select best)
  `);
    assert(result?.best?.hypothesis === "높음", `best 오류: ${JSON.stringify(result?.best)}`);
});
test("TC-8: eval fn 점수화 동작", () => {
    ...
    (TOT
      ...
      :select best)
  `);
    assert(result?.best?.hypothesis === "B", `B가 선택되어야 함: ${result?.best?.hypothesis}`);
});
test("TC-9: prune 0.5 → 낮은 분기 제거", () => {
  ...
  `);
    ...
    const pruned = result.branches.filter((b) => b.pruned);
...
test("TC-12: best.hypothesis 필드 존재", () => {
    ...
    (TOT
      ...
      :select best)
  `);
    assert(typeof result.best.hypothesis === "string", `hypothesis 타입 오류: ${typeof result.best.hypothesis}`);
    assert(result.best.hypothesis === "유일", `hypothesis 값 오류: ${result.best.hypothesis}`);
});
// ─── TC-13~17: 실용 패턴 ──────────────────────────────────────────────────
console.log("\n[TC-13~17] 실용 패턴\n");
test("TC-13: 수학 풀이 3가지 방법 중 best", () => {
    ...
    const tot = new tot_1.TreeOfThought();
    tot
        .branch("반복 곱셈", () => { let r = 1; for (let i = 0; i < 10; i++)
        r *= 2; return r; })
        .branch("Math.pow", () => Math.pow(2, 10))
        .branch("비트시프트", () => 1 << 10);
    tot.evaluate((r) => r === 1024 ? 1.0 : 0.0);
    const result = tot.select("best");
...
test("TC-14: 문자열 처리 전략 선택", () => {
    ...
    tot.evaluate((r) => r.includes("HELLO") ? 1.0 : r.startsWith("hello") ? 0.7 : 0.5);
    const result = tot.select("best");
    assert(result.best.hypothesis === "trim+upper", `trim+upper 선택 안 됨: ${result.best.hypothesis}`);
});
test("TC-15: score fn → 결과 길이 기반", () => {
    ...
    // 길수록 높은 점수
    tot.evaluate((r) => Math.min(r.length / 50, 1.0));
    const result = tot.select("best");
    assert(result.best.hypothesis === "긴 응답", `긴 응답이 선택되어야 함: ${result.best.hypothesis}`);
});
...
test("TC-17: 가지치기 후 남은 branches", () => {
    ...
    tot.prune(0.5);
    const result = tot.select("best");
    const alive = result.branches.filter((b) => !b.pruned);
    assert(alive.length >= 1, `살아있는 branch 없음`);
    assert(result.best.score >= 0.5 || result.branches.length > 0, "가지치기 후 best 없음");
});
// ─── TC-18~22: 에지 케이스 ────────────────────────────────────────────────
console.log("\n[TC-18~22] 에지 케이스\n");
...
test("TC-21: branch 결과가 maybe 타입", () => {
    ...
    tot.evaluate((r) => {
        if (r === null)
            return 0;
        if (r?.maybeValue !== undefined)
            return r.confidence || 0;
        return 0.5;
    });
    const result = tot.select("best");
    assert(result.explored === 3, `explored 3 아님: ${result.explored}`);
    assert(result.best.hypothesis === "확실", `maybe 타입 처리 오류: ${result.best.hypothesis}`);
});
test("TC-22: Phase 56 regression 14/14", () => {
    ...
    function regRunMulti(src) {
        const interp = new interpreter_1.Interpreter();
        interp.interpret((0, parser_1.parse)((0, lexer_1.lex)(src)));
        return interp;
    }
    function regGetVar(interp, name) {
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-phase93-tot.js.map