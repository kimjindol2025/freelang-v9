...
const stdlib_test_1 = require("./stdlib-test");
let passed = 0;
let failed = 0;
...
function assertContains(str, sub) {
    if (!str.includes(sub))
        throw new Error(`Expected string to contain "${sub}", got: "${str}"`);
}
// 테스트 모듈 생성 헬퍼 (독립적인 callFn mock)
function makeTestModule() {
    // 단순 callable: fn이 JS Function이면 직접 호출, 아니면 noop
    return (0, stdlib_test_1.createTestModule)((fn, args) => {
...
function makeInterp() {
    ...
    interp.registerModule((0, stdlib_test_1.createTestModule)((fn, args) => interp.callFunctionValue(fn, args)));
    return interp;
}
...
test("TC-1b: assert false → 예외 발생", () => {
    ...
    if (!threw)
        throw new Error("assert(false) should throw");
});
...
test("TC-2b: assert-eq 다른 값 → 예외", () => {
    ...
    if (!threw)
        throw new Error("assert-eq(1,2) should throw");
});
...
test("TC-2d: assert-eq 배열 다르면 예외", () => {
    ...
    if (!threw)
        throw new Error("should throw for unequal arrays");
});
...
test("TC-2f: assert-neq 같은 값 → 예외", () => {
    ...
    if (!threw)
        throw new Error("assert-neq(5,5) should throw");
});
// ─── TC-3: assert-throws ──────────────────────────────────────────────
console.log("\n[TC-3] assert-throws");
...
test("TC-3b: assert-throws — 예외 없으면 실패", () => {
    ...
    if (!threw)
        throw new Error("assert-throws should fail when no exception");
});
...
test("TC-3d: assert-throws + expectedMsg 불일치 → 실패", () => {
    ...
    if (!threw)
        throw new Error("should fail when error message doesn't match");
});
// ─── TC-4: describe 그룹화 ────────────────────────────────────────────
console.log("\n[TC-4] describe 그룹화");
test("TC-4a: describe — suite 이름 설정", () => {
    ...
    const results = m["test-results"]();
    assertEqual(results.some((r) => r.name.includes("Math Suite")), true);
});
...
test("TC-4c: 여러 suite 격리", () => {
    ...
    assertEqual(names.some((n) => n.includes("Suite A")), true);
    assertEqual(names.some((n) => n.includes("Suite B")), true);
});
// ─── TC-5: test-report 통계 ───────────────────────────────────────────
console.log("\n[TC-5] test-report 통계");
...
test("TC-6c: FL test-report 반환값 확인", () => {
    ...
    (test-report)
  `);
    const report = interp.context.lastValue;
...
function runLexical(src) {
    const interp = new interpreter_1.Interpreter();
    interp.interpret((0, parser_1.parse)((0, lexer_1.lex)(src)));
    return interp.context.lastValue;
}
function runLexicalInterp(src) {
    const interp = new interpreter_1.Interpreter();
    interp.interpret((0, parser_1.parse)((0, lexer_1.lex)(src)));
    return interp;
}
function getVar56(interp, name) {
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-phase76-testrunner.js.map