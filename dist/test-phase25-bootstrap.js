...
function makeSandboxCompiler() {
    ...
    return (flSrc) => {
        ...
        for (const src of [lexerSrc, parserSrc, codegenSrc]) {
            interp2.interpret((0, parser_1.parse)((0, lexer_1.lex)(src)));
        }
        interp2.interpret((0, parser_1.parse)((0, lexer_1.lex)(`(gen-js (parse (lex ${JSON.stringify(flSrc)})))`)));
        return interp2.context.lastValue;
    };
}
// ── 대표 FL 프로그램 모음 ──────────────────────────────────────────
const PROGRAMS = [
...
test("sandbox: 기본 덧셈 함수 컴파일 & 실행", () => {
    ...
    if (s.module.exports.add(3, 4) !== 7)
        throw new Error("add(3,4) !== 7");
});
...
for (const prog of PROGRAMS) {
    test(`동치: ${prog.name}`, () => {
        ...
        if (!tsJS || !sbJS)
            throw new Error("outputs not ready");
        if (tsJS !== sbJS) {
            let diffIdx = 0;
...
test("fact(5) = 120", () => {
    ...
    vm.createContext(s);
    vm.runInContext(js, s);
    if (s.module.exports.fact(5) !== 120)
        throw new Error(`fact(5)=${s.module.exports.fact(5)}`);
});
test("fib(10) = 55", () => {
    ...
    vm.createContext(s);
    vm.runInContext(js, s);
    if (s.module.exports.fib(10) !== 55)
        throw new Error(`fib(10)=${s.module.exports.fib(10)}`);
});
test("sign(-5) = 'negative'", () => {
    ...
    vm.createContext(s);
    vm.runInContext(js, s);
    if (s.module.exports.sign(-5) !== "negative")
        throw new Error(`sign(-5)=${s.module.exports.sign(-5)}`);
});
test("make-adder(10)(5) = 15", () => {
    ...
    if (typeof makeAdder !== "function")
        throw new Error("make-adder not exported");
    const add10 = makeAdder(10);
    if (add10(5) !== 15)
        throw new Error(`make-adder(10)(5)=${add10(5)}`);
});
// ── 결과 ──────────────────────────────────────────────────────────
console.log(`\n${"─".repeat(50)}`);
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase25-bootstrap.js.map