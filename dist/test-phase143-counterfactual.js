...
const counterfactual_1 = require("./counterfactual");
...
function accidentOutcome(vars) {
    const rain = vars["rain"];
    const speed = vars["speed"];
    if (rain && speed > 50)
        return "accident";
    return "safe";
}
// 수치 outcome: 사고 확률 0~1
function riskScore(vars) {
    const rain = vars["rain"] ? 1 : 0;
    const speed = vars["speed"] || 0;
    return rain * 0.5 + (speed / 200);
}
console.log("\n=== Phase 143: COUNTERFACTUAL 반사실 추론 ===\n");
// 1. CounterfactualReasoner 생성
const reasoner = new counterfactual_1.CounterfactualReasoner();
test("1. CounterfactualReasoner 생성", () => reasoner instanceof counterfactual_1.CounterfactualReasoner);
...
// 3. whatIf 단순 반사실
const wf = reasoner.whatIf({ rain: true, speed: 60 }, { rain: false }, accidentOutcome);
...
// 9. analyze 다중 분석
const analysis = reasoner.analyze("s1", [
    { rain: false },
    { speed: 30 },
    { rain: false, speed: 30 },
], accidentOutcome);
...
test("10. CounterfactualAnalysis: original, counterfactuals, mostLikelyAlternative", () => {
    return (analysis.original !== undefined &&
...
const sens = reasoner.sensitivityAnalysis({ rain: true, speed: 60 }, riskScore);
...
test("19. cf-scenario 빌트인", () => {
    ...
    return result instanceof Map && result.get("id") === "s2";
});
...
test("20. cf-what-if 빌트인", () => {
  ...
  `);
    return result instanceof Map && result.has("counterfactualOutcome");
});
// 21. cf-analyze 빌트인
test("21. cf-analyze 빌트인", () => {
  ...
  `);
    return result instanceof Map && result.has("counterfactuals");
});
// 22. cf-minimal 빌트인
...
test("23. cf-sensitivity 빌트인", () => {
  ...
  `);
    return result instanceof Map && result.has("speed");
});
...
test("25. cf-best-alt 빌트인", () => {
  ...
  `);
    return result instanceof Map && result.has("probability");
});
// 26. cf-explain 빌트인
...
test("27. 빈 개입 whatIf → 동일 결과", () => {
    const wfEmpty = reasoner.whatIf({ rain: true, speed: 60 }, {}, accidentOutcome);
    return wfEmpty.counterfactualOutcome === "accident";
});
// 28. 여러 변수 동시 변경
test("28. 여러 변수 동시 변경", () => {
    const wfMulti = reasoner.whatIf({ rain: true, speed: 60, road: "wet" }, { rain: false, speed: 30 }, accidentOutcome);
...
//# sourceMappingURL=test-phase143-counterfactual.js.map