...
const eval_call_function_1 = require("./eval-call-function");
let passed = 0;
let failed = 0;
...
test("callUserFunctionTCO로 100만 카운트다운 — 스택 오버플로 없음", () => {
    ...
    [FUNC count-down :params [$n]
      :body (if (= $n 0) "done"
                (count-down (- $n 1)))]
  `)));
    const result = (0, eval_call_function_1.callUserFunctionTCO)(interp, "count-down", [1000000]);
    if (result !== "done")
        throw new Error(`Expected "done", got ${result}`);
});
...
test("fib-tail TCO — 50번째 피보나치 수", () => {
    ...
    [FUNC fib-tail :params [$n $a $b]
      :body (if (= $n 0) $a
                (fib-tail (- $n 1) $b (+ $a $b)))]
  `)));
    const result = (0, eval_call_function_1.callUserFunctionTCO)(interp, "fib-tail", [50, 0, 1]);
    // 50번째 피보나치: 12586269025
    if (result !== 12586269025)
        throw new Error(`Expected 12586269025, got ${result}`);
});
...
test("loop/recur 합산 — 1+2+...+100 = 5050", () => {
  ...
  `);
    if (result !== 5050)
        throw new Error(`Expected 5050, got ${result}`);
});
...
test("callUserFunctionTCO my-even?(10000) → true", () => {
    ...
    [FUNC my-odd? :params [$n]
      :body (if (= $n 0) false (my-even? (- $n 1)))]
  `)));
    const result = (0, eval_call_function_1.callUserFunctionTCO)(interp, "my-even?", [10000]);
    if (result !== true)
        throw new Error(`Expected true, got ${result}`);
});
...
test("callUserFunctionTCO sum(1000, 0) → 500500", () => {
    ...
    [FUNC sum-tail :params [$n $acc]
      :body (if (= $n 0) $acc
                (sum-tail (- $n 1) (+ $acc $n)))]
  `)));
    const result = (0, eval_call_function_1.callUserFunctionTCO)(interp, "sum-tail", [1000, 0]);
    if (result !== 500500)
        throw new Error(`Expected 500500, got ${result}`);
});
...
test("함수 내 define이 전역 오염 안 함", () => {
    ...
    (isolate)
  `)));
    const globalX = interp.context.variables.get("$x");
    if (globalX !== 10)
        throw new Error(`Expected $x=10, got ${globalX}`);
});
test("클로저 캡처 정상", () => {
    const result = run(`
...
test("재귀 함수 — 팩토리얼(10) = 3628800", () => {
  ...
  `);
    if (result !== 3628800)
        throw new Error(`Expected 3628800, got ${result}`);
});
// ── TC-9: 대용량 TCO — 50만 반복 ────────────────────────────────────
...
test("callUserFunctionTCO count-down(500000) → 0", () => {
    ...
    [FUNC count-to-zero :params [$n]
      :body (if (= $n 0) 0
                (count-to-zero (- $n 1)))]
  `)));
    const result = (0, eval_call_function_1.callUserFunctionTCO)(interp, "count-to-zero", [500000]);
    if (result !== 0)
...
//# sourceMappingURL=test-phase61-tco.js.map