...
test("file_read 함수 등록됨", () => {
    const interp = new interpreter_1.Interpreter();
    if (!hasFn(interp, "file_read"))
        throw new Error("file_read 미등록");
});
test("file_write 함수 등록됨", () => {
    const interp = new interpreter_1.Interpreter();
    if (!hasFn(interp, "file_write"))
        throw new Error("file_write 미등록");
});
test("file_exists 함수 등록됨", () => {
    const interp = new interpreter_1.Interpreter();
    if (!hasFn(interp, "file_exists"))
        throw new Error("file_exists 미등록");
});
test("dir_list 함수 등록됨", () => {
    const interp = new interpreter_1.Interpreter();
    if (!hasFn(interp, "dir_list"))
        throw new Error("dir_list 미등록");
});
// ════════════════════════════════════════════════════════════
...
test("http_get 함수 등록됨", () => {
    const interp = new interpreter_1.Interpreter();
    if (!hasFn(interp, "http_get"))
        throw new Error("http_get 미등록");
});
test("http_post 함수 등록됨", () => {
    const interp = new interpreter_1.Interpreter();
    if (!hasFn(interp, "http_post"))
        throw new Error("http_post 미등록");
});
// ════════════════════════════════════════════════════════════
// TC-3: db 모듈 함수 존재 확인
// ════════════════════════════════════════════════════════════
...
test("db_query 함수 등록됨", () => {
    const interp = new interpreter_1.Interpreter();
    if (!hasFn(interp, "db_query"))
        throw new Error("db_query 미등록");
});
test("db_get 함수 등록됨", () => {
    const interp = new interpreter_1.Interpreter();
    if (!hasFn(interp, "db_get"))
        throw new Error("db_get 미등록 (db_get 또는 db_query 중 하나 필요)");
});
// ════════════════════════════════════════════════════════════
...
test(`총 ${expectedFunctions.length}개 stdlib 함수 모두 등록됨`, () => {
    ...
    if (missing.length > 0)
        throw new Error(`미등록 함수: ${missing.join(", ")}`);
});
test("stdlib-loader는 20개 이상의 모듈 함수를 등록함", () => {
    const interp = new interpreter_1.Interpreter();
    const fns = interp.context.functions;
    if (fns.size < 20)
        throw new Error(`함수 수 ${fns.size}개 (20개 이상 기대)`);
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase58-module-refactor.js.map