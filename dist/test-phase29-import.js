...
function runWithRequire(mainJS, deps) {
    ...
    try {
        ...
        try {
            fs.unlinkSync(path.join(tmpDir, "main.js"));
        }
        catch { }
        try {
            fs.rmdirSync(tmpDir);
...
(export quad)
  `.trim());
    if (!js.includes('require("./math-utils.js")'))
        throw new Error("require not found");
    if (!js.includes("double"))
        throw new Error("double not found");
});
...
(export apply-both)
  `.trim());
    if (!js.includes('"add-one": add_one'))
        throw new Error(`hyphen name not properly aliased: ${js.slice(0, 300)}`);
});
...
(export double-square)
  `.trim());
    ...
    if (exp["double-square"](3) !== 18)
        throw new Error(`got ${exp["double-square"](3)}`);
    if (exp["double-square"](5) !== 50)
        throw new Error(`got ${exp["double-square"](5)}`);
});
...
(export inc6)
  `.trim());
    ...
    if (exp.inc6(0) !== 6)
        throw new Error(`got ${exp.inc6(0)}`);
    if (exp.inc6(10) !== 16)
        throw new Error(`got ${exp.inc6(10)}`);
});
...
(export sum-squares)
  `.trim());
    ...
    // 1²+2²+3²+4²+5² = 55
    if (exp["sum-squares"]([1, 2, 3, 4, 5]) !== 55)
        throw new Error(`got ${exp["sum-squares"]([1, 2, 3, 4, 5])}`);
});
// ─── 결과 ─────────────────────────────────────────────────────────
console.log(`\n${"─".repeat(50)}`);
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase29-import.js.map