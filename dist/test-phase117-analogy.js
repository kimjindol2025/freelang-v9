...
test("1. AnalogyStore 생성", () => {
    const store = new analogy_1.AnalogyStore();
    return store instanceof analogy_1.AnalogyStore;
});
// 2. store() 패턴 저장
test("2. store() 패턴 저장", () => {
    const store = new analogy_1.AnalogyStore();
...
test("15. similarity 텍스트 겹침", () => {
    const score = (0, analogy_1.similarity)("retry on timeout", "retry timeout handling");
    return score > 0;
});
// 16. similarity 동일 문자열 → 최대
test("16. similarity 동일 문자열 → 1.0", () => {
    const score = (0, analogy_1.similarity)("sort a list", "sort a list");
    return score === 1.0;
});
// 17. similarity 완전 다른 텍스트 → 0
test("17. similarity 완전 다른 텍스트 → 0", () => {
    const score = (0, analogy_1.similarity)("apple orange banana", "retry network timeout");
    return score === 0;
});
// ─── tags 가중치 ───────────────────────────────────────────────────────────────
console.log("\n=== Tags 가중치 ===");
...
test("19. globalAnalogy 싱글톤", () => {
    const sizeBefore = analogy_1.globalAnalogy.size();
    analogy_1.globalAnalogy.store("singleton test pattern", "singleton solution", []);
    return analogy_1.globalAnalogy.size() === sizeBefore + 1;
});
// ─── 내장 함수 (Interpreter) ──────────────────────────────────────────────────
console.log("\n=== 내장 함수 테스트 ===");
function evalStr(interp, code) {
...
//# sourceMappingURL=test-phase117-analogy.js.map