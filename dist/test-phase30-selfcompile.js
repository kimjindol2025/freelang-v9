...
function patternToOp(pattern) {
    if (!pattern || pattern.kind === "wildcard-pattern")
        return "_";
    if (pattern.kind === "literal-pattern") {
        // JSON.stringify로 JS 문자열 리터럴 형식(따옴표 포함)으로 변환
        // gen-match가 "\"" 포함 여부로 감싸기 여부를 결정함
        if (pattern.type === "string")
            return JSON.stringify(String(pattern.value));
...
function tsCompile(flSrc) {
    ...
    const rawAst = (0, parser_1.parse)((0, lexer_1.lex)(flSrc));
...
test("lexer.fl → standalone JS 컴파일", () => {
    ...
    if (typeof lexerJS !== "string" || lexerJS.length === 0)
        throw new Error("empty output");
    lexerMod = makeModule(lexerJS);
    if (typeof lexerMod.lex !== "function")
        throw new Error("lex not exported");
    // 기본 검증: lex가 토큰을 만들어내는지
    const tokens = lexerMod.lex("(+ 1 2)");
    if (!Array.isArray(tokens))
        throw new Error("lex returned non-array");
    if (tokens.length === 0)
        throw new Error("lex returned empty");
});
test("parser.fl → standalone JS 컴파일", () => {
    const parserJS = tsCompile(parserSrc);
    if (typeof parserJS !== "string" || parserJS.length === 0)
        throw new Error("empty output");
...
if (failed > 0)
    process.exit(1);
process.exit(0);
//# sourceMappingURL=test-phase30-selfcompile.js.map