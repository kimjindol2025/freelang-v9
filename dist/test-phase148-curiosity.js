...
const engine = new curiosity_1.CuriosityEngine();
test("1. CuriosityEngine 기본 생성", () => engine instanceof curiosity_1.CuriosityEngine);
// 2. CuriosityEngine 초기 주제로 생성
const engineWithTopics = new curiosity_1.CuriosityEngine(["양자컴퓨팅", "머신러닝", "블록체인"]);
const initialState = engineWithTopics.getState();
test("2. CuriosityEngine 초기 주제로 생성", () => initialState.frontier.length === 3 &&
...
test("5. selectNextTopic 탐색 대상 선택", () => nextTopic !== null && typeof nextTopic === "string");
// 6. explore: 탐색 수행
const exploreEngine = new curiosity_1.CuriosityEngine(["딥러닝"]);
const result = exploreEngine.explore("딥러닝", (topic) => ({
...
test("7. ExplorationResult 구조 (topic, discovered, newQuestions, ...)", () => typeof result.topic === "string" &&
...
test("11. surpriseLevel 0~1 범위", () => result.surpriseLevel >= 0 && result.surpriseLevel <= 1);
// 12. relatedTopics 생성
test("12. relatedTopics 생성", () => Array.isArray(result.relatedTopics));
// 13. identifyGaps: 지식 공백 식별
const gapEngine = new curiosity_1.CuriosityEngine();
const gaps = gapEngine.identifyGaps(["양자컴퓨팅", "머신러닝"], ["양자컴퓨팅", "머신러닝", "블록체인", "메타버스", "자율주행"]);
test("13. identifyGaps 지식 공백 반환", () => gaps.length === 3);
// 14. KnowledgeGap 구조 검증
const gap0 = gaps[0];
test("14. KnowledgeGap 구조 (topic, unknownAspects, priority, explorationCost, expectedGain)", () => typeof gap0.topic === "string" &&
...
test("15. generateQuestions 질문 생성", () => Array.isArray(questions) && questions.length >= 4);
// 16. prioritize: UCB1 정렬
const prioritizeEngine = new curiosity_1.CuriosityEngine(["A", "B", "C"]);
const prioritized = prioritizeEngine.prioritize(["A", "B", "C"]);
test("16. prioritize UCB1 정렬 반환", () => Array.isArray(prioritized) && prioritized.length === 3);
// 17. analyzeExplorationHistory: 이력 분석
const histEngine = new curiosity_1.CuriosityEngine(["수학", "물리", "화학"]);
histEngine.explore("수학", (t) => ({
    facts: [`${t}는 논리다`, `${t}는 추상이다`],
    questions: ["미적분이란?", "선형대수란?"],
}));
const analysis = histEngine.analyzeExplorationHistory();
test("17. analyzeExplorationHistory 분석 반환", () => typeof analysis.totalExplored === "number" &&
    typeof analysis.avgInfoGain === "number" &&
    typeof analysis.mostSurprising === "string" &&
    Array.isArray(analysis.recommendations));
// 18. 탐색 후 frontier 업데이트 확인
const frontierEngine = new curiosity_1.CuriosityEngine(["주제A"]);
...
const bc = new curiosity_1.CuriosityEngine(["바이오테크", "나노기술", "핵융합"]);
// globalCuriosity 대신 별도 함수를 만들어 테스트 (evalCuriosity는 globalCuriosity 사용)
// 19. curiosity-score 빌트인
const scoreResult = (0, eval_builtins_1.evalCuriosity)("curiosity-score", ["새주제", []], (_fn, _a) => null);
test("19. curiosity-score 빌트인", () => typeof scoreResult === "number" && scoreResult >= 0 && scoreResult <= 1);
// 20. curiosity-next 빌트인 (globalCuriosity에 frontier가 없을 수도 있어 null 허용)
const nextResult = (0, eval_builtins_1.evalCuriosity)("curiosity-next", [], (_fn, _a) => null);
test("20. curiosity-next 빌트인 (null or string)", () => nextResult === null || typeof nextResult === "string");
// 21. curiosity-explore 빌트인
const exploreBuiltinResult = (0, eval_builtins_1.evalCuriosity)("curiosity-explore", [
    "인공지능",
    (t) => new Map([
        ["facts", [`${t}는 머신러닝 기반`, `${t}는 데이터가 필요`]],
        ["questions", ["딥러닝이란?", "강화학습이란?"]],
    ]),
], (fn, a) => fn(a[0]) // 간단 callFn
);
test("21. curiosity-explore 빌트인 반환", () => exploreBuiltinResult instanceof Map &&
    exploreBuiltinResult.has("topic") &&
    exploreBuiltinResult.has("informationGain"));
// 22. curiosity-gaps 빌트인
const gapsResult = (0, eval_builtins_1.evalCuriosity)("curiosity-gaps", [["A", "B"], ["A", "B", "C", "D"]], (_fn, _a) => null);
test("22. curiosity-gaps 빌트인", () => Array.isArray(gapsResult) && gapsResult.length === 2);
// 23. curiosity-questions 빌트인
const questionsResult = (0, eval_builtins_1.evalCuriosity)("curiosity-questions", ["메타버스", ["VR", "AR"]], (_fn, _a) => null);
test("23. curiosity-questions 빌트인", () => Array.isArray(questionsResult) && questionsResult.length >= 4);
// 24. curiosity-prioritize 빌트인
const prioritizeResult = (0, eval_builtins_1.evalCuriosity)("curiosity-prioritize", [["X", "Y", "Z"]], (_fn, _a) => null);
test("24. curiosity-prioritize 빌트인", () => Array.isArray(prioritizeResult) && prioritizeResult.length === 3);
// 25. curiosity-analyze 빌트인
const analyzeResult = (0, eval_builtins_1.evalCuriosity)("curiosity-analyze", [], (_fn, _a) => null);
test("25. curiosity-analyze 빌트인", () => analyzeResult instanceof Map &&
...
const stateResult = (0, eval_builtins_1.evalCuriosity)("curiosity-state", [], (_fn, _a) => null);
test("26. curiosity-state 빌트인", () => stateResult instanceof Map &&
...
test("27. computeCuriosity 0~1 범위 항상 보장", () => scoreA >= 0 && scoreA <= 1 && scoreB >= 0 && scoreB <= 1);
// 28. gaps priority 내림차순 정렬
const sortedGaps = gapEngine.identifyGaps([], ["토픽1", "토픽2", "토픽3"]);
test("28. gaps priority 내림차순 정렬", () => sortedGaps.length >= 2 &&
...
//# sourceMappingURL=test-phase148-curiosity.js.map