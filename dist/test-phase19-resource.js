...
const stdlib_resource_1 = require("./stdlib-resource");
const R = (0, stdlib_resource_1.createResourceModule)();
let passed = 0;
let failed = 0;
...
check("res_cpu_load: 배열 3개", () => {
    ...
    console.log(`     Load: [${load.map((n) => n.toFixed(2)).join(", ")}]`);
...
check("res_procs: 배열 반환", () => {
    ...
    console.log(`     Top: ${procs.slice(0, 3).map((p) => p.name).join(", ")}`);
...
check("res_ports: 배열 반환", () => {
    ...
    console.log(`     Listening ports: ${ports.slice(0, 5).map((p) => p.port).join(", ")}...`);
...
check("res_net: 배열 반환", () => {
    ...
    ifaces.forEach((i) => console.log(`     ${i.name}: ${i.addr}`));
...
check("res_pm2_list: 배열 반환", () => {
    ...
    if (svcs.length > 0) {
        console.log(`     PM2 services: ${svcs.slice(0, 3).map((s) => `${s.name}(${s.status})`).join(", ")}`);
    }
    else {
...
check("res_health_check: ok/warnings/errors", () => {
    ...
    if (health.warnings.length > 0)
        console.log(`     ⚠️  ${health.warnings[0]}`);
    if (health.errors.length > 0)
        console.log(`     🔴 ${health.errors[0]}`);
...
check("res_snapshot: 필드 완전성", () => {
    ...
    if (ok)
        console.log(R["res_snapshot_report"](snap));
    return ok;
});
// ──────────────────────────────────────────
console.log("\n══════════════════════════════════════════");
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-phase19-resource.js.map